...
describe('validated player request catalog', () => {
  ...
  test('covers every cost archetype, so the tab is not thirty invoices', () => {
    const kinds = new Set(loadLaunchContent().playerRequests.requests.map(r => r.cost.kind));
  ...
  test('carries no status perk, which would collide with contractPromise', () => {
    const kinds = loadLaunchContent().playerRequests.requests.map(r => r.cost.kind as string);
    ...
    for (const banned of ['ARMBAND', 'SHIRT_TEN', 'GUARANTEED_START', 'TRAINING_PRIORITY']) {
      expect(kinds).not.toContain(banned);
    }
  });

  test('gives a themed bonus only to squad requests, never to an individual ask', () => {
    const withBonus = loadLaunchContent().playerRequests.requests
      .filter(request => request.grantBonus !== undefined);
  ...
  test('rejects artwork that names a sprite the game does not have', () => {
    ...
    expect(() => PlayerRequestCatalogSchema.parse(broken)).toThrow('unknown request art sprite');
  });
    ...
    const broken = {
      ...
      tuning: {
        ...
        cadence: {
          ...
          COZY: { ...catalog.tuning.cadence.COZY, minWeeks: 12, guaranteeWeeks: 12 },
        },
      },
    };

    expect(() => PlayerRequestCatalogSchema.parse(broken))
      .toThrow('minWeeks must be below guaranteeWeeks');
  });
  ...
  test('rejects a star window that is slower than the ordinary one', () => {
    ...
    const broken = {
      ...
      tuning: {
        ...
        cadence: {
          ...catalog.tuning.cadence,
          CHAIRMAN: { ...catalog.tuning.cadence.CHAIRMAN, starMinWeeks: 9, starGuaranteeWeeks: 12 },
        },
      },
    };
