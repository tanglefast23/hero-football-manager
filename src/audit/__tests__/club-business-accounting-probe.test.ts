...
const SEED_OFFSET = nonNegativeIntegerEnv('CLUB_BUSINESS_SEED_OFFSET', 0, 100_000);
...
const BUZZ_TUNING_CANDIDATES = [
  ...
  { id: 'OLD_WIN_5', label: 'former win +5, no hero sub-cap', buzzWinPoints: 5 },
  {
    id: 'OLD_HERO_CAP_6',
...
describe('Club Business frozen-outcome accounting probe', () => {
  it('conserves every dollar and reports each new-income source separately', () => {
    ...
    for (const cell of CELLS) {
      ...
      for (const run of runs) {
        const attributed = run.attribution.totalNewIncome - run.mandatoryPitchUplift;
        expect(run.featureCashAfterMandatoryPitchUplift
          - run.controlCashAfterMandatoryPitchUplift).toBe(attributed);
        expect(Object.values(run.attribution).every(Number.isSafeInteger)).toBe(true);
        expect(run.minimumFans).toBeGreaterThanOrEqual(0);
        expect(run.buzzHalfValues).toHaveLength(2);
      }

      if (cell.division === 5) {
        expect(runs.every(run => run.attribution.monthlySponsorDelta === 0)).toBe(true);
        expect(runs.every(run => run.attribution.sponsorObjectiveBonus === 0)).toBe(true);
      } else {
        expect(runs.every(run => run.objectiveCount === Math.min(3, 5 - cell.division)))
          .toBe(true);
      }
  ...
  it('measures sponsor profile expected value without hiding monthly tradeoffs', () => {
    ...
    for (const cell of CELLS.filter(candidate => candidate.division < 5)) {
      ...
      lines.push(
        ...
        + expectedValues.map(value => money(Math.round(value)).padStart(14)).join('')
        + `${`${(spread * 100).toFixed(1)}%`.padStart(14)}`,
      );
    }

    if (violations.length > 0) {
      lines.push('violations:', ...violations.map(violation => `- ${violation}`));
    }
    // eslint-disable-next-line no-console
    console.log(lines.join('\n'));
    if (SEEDS >= 300) {
      ...
      expect(violations.every(violation => violation.includes('profile EV spread'))).toBe(true);
    }
  });
  ...
  it('stratifies completion and expected value by objective family through D1', () => {
    ...
    for (const cell of CELLS.filter(candidate => candidate.division < 5)) {
      ...
      for (const profile of PROFILES) {
        ...
        for (const objectiveKind of OBJECTIVE_KINDS) {
          ...
          const completion = contracts.filter(contract => contract.objectiveMet).length
            / contracts.length;
          const seasonValue = mean(contracts.map(contract => contract.actualSeasonValue));
          lines.push(
      ...
      for (const objectiveKind of OBJECTIVE_KINDS) {
        const values = PROFILES.map(profile => mean(
          byProfile.get(profile)!.get(objectiveKind)!.map(contract => contract.actualSeasonValue),
        ));
        ...
        for (const alternative of PROFILES) {
          ...
          const comparisons = OBJECTIVE_KINDS.map(objectiveKind => {
            const candidateValue = mean(
              byProfile.get(candidate)!.get(objectiveKind)!
                .map(contract => contract.actualSeasonValue),
            );
            const alternativeValue = mean(
              byProfile.get(alternative)!.get(objectiveKind)!
                .map(contract => contract.actualSeasonValue),
            );
            return candidateValue - alternativeValue;
          });
          if (comparisons.every(delta => delta >= 0) && comparisons.some(delta => delta > 0)) {
    ...
    if (spreadViolations.length > 0) {
      lines.push('spread violations:', ...spreadViolations.map(violation => `- ${violation}`));
    }
    ...
    if (SEEDS >= SPONSOR_DOMINANCE_APPROVAL_SEEDS) {
      ...
      expect([...spreadViolations, ...dominanceFindings].every(violation => (
        violation.includes('profile EV spread') || violation.includes('strictly dominates')
      ))).toBe(true);
    }
  });
  ...
  it('reports the exact frozen five-division facility-sink envelope', () => {
    ...
    for (const difficulty of ['COZY', 'CHAIRMAN'] as const) {
      const byDivision = ([5, 4, 3, 2, 1] as const).map(division => sample({
        division,
        difficulty,
      }));
      const cumulativeIncome = Array.from({ length: SEEDS }, (_, index) => (
        byDivision.reduce(
          (total, divisionRuns) => total + divisionRuns[index].attribution.totalNewIncome,
          0,
        )
      ));
      expect(cumulativeIncome.every(value => Number.isSafeInteger(value) && value > 0)).toBe(true);
      const absorption = cumulativeIncome.map(value => (
        CLUB_BUSINESS_FACILITY_UPGRADE_UPLIFT / value * 100
      ));
      lines.push(
        ...
        ].map(value => `${value.toFixed(1)}%`).join('/').padStart(30)}`,
      );
    }
  ...
  it('measures simple Buzz earning candidates without changing payout terms', () => {
    ...
    for (const candidate of BUZZ_TUNING_CANDIDATES) {
      ...
      for (const cell of CELLS) {
        ...
        const capRate = halfValues.filter(value => value === 100).length / halfValues.length;
        const shippedRuns = sampleBuzz(cell, BUZZ_TUNING_CANDIDATES[0]);
        const meanPayoutDelta = mean(runs.map((run, index) => (
          run.attribution.buzzPayout - shippedRuns[index].attribution.buzzPayout
        )));
      ...
      if (violations.length > 0) {
        lines.push(`${candidate.id} violations:`, ...violations.map(value => `- ${value}`));
      }
    }

    lines.push('SHIPPED frozen five-division sink/income:');
    for (const difficulty of ['COZY', 'CHAIRMAN'] as const) {
      const byDivision = ([5, 4, 3, 2, 1] as const).map(division => sampleBuzz(
        { division, difficulty },
        BUZZ_TUNING_CANDIDATES[0],
      ));
    ...
    if (SEEDS >= 300) {
      ...
      expect(violationsByCandidate.get('SHIPPED')?.every(violation => (
...
function sample(
  ...
  const runs = Array.from({ length: SEEDS }, (_, index) => runClubBusinessBalanceScenario({
...
function sponsorContractStrata(
  ...
  const strata = new Map(OBJECTIVE_KINDS.map(kind => [
    kind,
...
function sampleBuzz(
  ...
  const cacheKey = [candidate.id, cell.division, cell.difficulty, SEED_OFFSET, SEEDS].join('/');
  ...
  const runs = Array.from({ length: SEEDS }, (_, index) => runClubBusinessBalanceScenario({
    ...
    difficulty: cell.difficulty,
    profile: 'BALANCED',
    ...('buzzWinPoints' in candidate ? { buzzWinPoints: candidate.buzzWinPoints } : {}),
...
function reportLine(
  ...
  return `${`D${cell.division} ${cell.difficulty}`.padEnd(17)}`
