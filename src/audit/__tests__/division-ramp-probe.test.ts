...
const describeProbe =
  process.env.DIVISION_RAMP_PROBE === '1' ? describe : describe.skip;
 ...
 */
function trainWithWholeBank(state: GameState): {
  state: GameState;
  drills: number;
  spent: number;
} {
  ...
  if (lineup === undefined)
    throw new Error('the ramp probe needs a user lineup');
...
function playMatchday(state: GameState): GameState {
  ...
  const side =
    userFixture.homeClubId === played.userClubId
      ? teams[userFixture.homeClubId]
      : teams[userFixture.awayClubId];
  return awakenHero(
    played,
    userFixture.id,
    side.players.map((p) => p.id),
  );
}
...
function licenseHeroes(state: GameState): GameState {
  ...
  const wanted = [
    ...licensed,
    ...powered.filter((p) => !p.licensed).map((p) => p.id),
...
function squadMeanFor(state: GameState, clubId: string): number {
  ...
  const perPlayer = team.players.map(
    (p) => ATTRS.reduce((sum, k) => sum + p.attrs[k], 0) / ATTRS.length,
  );
...
function playCareer(seed: number, train: boolean): SeasonRow[] {
  ...
  for (let season = 1; season <= SEASONS; season += 1) {
    const divisionBefore =
      state.m2?.pyramid.divisions.find((d) =>
    ...
    const divisionNow = state.m2?.pyramid.divisions.find((d) =>
      d.clubs.some((c) => c.id === state.userClubId),
    );
    ...
    rows.push({
      ...
      meanCondition: (() => {
        ...
        return (
          mine.reduce((sum, p) => sum + (p.condition ?? 100), 0) /
          Math.max(1, mine.length)
        );
      })(),
      injured: state.players.filter(
        (p) => p.clubId === state.userClubId && (p.injuryWeeks ?? 0) > 0,
      ).length,
      drills: seasonDrills,
      tpSpent: seasonSpent,
...
describeProbe('division ramp', () => {
  it('reports how many seasons Division 5 actually takes', () => {
    const lines = [
      '',
      '=== D5 RAMP: real engine for all 90 fixtures per season ===',
    ];
    ...
    for (const train of [true, false]) {
      lines.push(
        '',
        `--- manager: ${train ? 'trains weekly' : 'never trains'} ---`,
      );
      lines.push(
      ...
      if (
        (train &&
          (escaped.length !== SEEDS.length || Math.max(...escaped) > 2)) ||
