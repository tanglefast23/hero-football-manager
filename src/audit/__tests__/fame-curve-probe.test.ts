...
const describeProbe =
  process.env.FAME_CURVE_PROBE === '1' ? describe : describe.skip;
...
function newCareer(seed: number): GameState {
  return createCareer(
    createLaunchCareerSetup(seed, undefined, content, 'COZY'),
  );
}
...
function pickScorers(
  ...
  const eligible = (
    ...
    .sort(
      (left, right) =>
        SCORER_ROLE_ORDER.indexOf(left.role as never) -
          SCORER_ROLE_ORDER.indexOf(right.role as never) ||
...
function runSeasons(seed: number): SeasonRow[] {
  ...
  while (rows.length < SEASONS) {
    guard += 1;
    if (guard > SEASONS * 200)
      throw new Error('fame curve probe exceeded its transition budget');
