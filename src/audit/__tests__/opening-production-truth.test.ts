...
const CREATION = { pac: 50, sho: 65, pas: 50, def: 50, tec: 50, sta: 50 } as const;
const SEED = 4_000_000;

function freshCareer(difficulty: DifficultyMode = 'COZY', seed = SEED): GameState {
  return addCreatedPlayer(
    beginStoryOnboarding(createCareer(createLaunchCareerSetup(seed, undefined, content, difficulty))),
...
function userStarters(state: GameState) {
  ...
  if (lineup === undefined) throw new Error('the career must have a user lineup');
  return lineup.playerIds.map(id => state.players.find(player => player.id === id)!);
}
...
describe('training capacity has no weekly cap', () => {
  it('lets four distinct players train in the same week when TP permits', () => {
    ...
    const outfielders = userStarters(state).filter(player => player.role !== 'GK').slice(0, 4);
  ...
  it('lets the same player train repeatedly in one week, paying condition each time', () => {
    ...
    let taps = 0;
    while (state.trainingPoints >= resolveTrainingDrillForPath(state, 'finishing').tpCost && taps < 4) {
    ...
    expect(after.condition).toBe(startingCondition - taps * INSTANT_DRILL_CONDITION_COST);
  });
});
...
describe('Training Pitch timing', () => {
  it('is operational for week 3 DEF training but pays no TP before kickoff', () => {
    ...
    const pitchId = state.facilities.grid!.buildings
      .find(building => building.type === 'training-pitch')!.id;
    ...
    const bare = trainPlayerInstantly(freshCareerAtWeekThree(), defender.id, 'duels');
    expect(withPitch.after - defender.attrs.def)
      .toBeGreaterThan(bare.after - defender.attrs.def);
  });
});
...
describe('difficulty identity before kickoff', () => {
  it.each([
  ...
  ])('gives %s the same opener in Cozy and Chairman on the same seed', (_label, policy) => {
    const cozy = runOpening({
      seed: SEED, difficulty: 'COZY', policy, creation: CREATION, content,
    });
    const chairman = runOpening({
      seed: SEED, difficulty: 'CHAIRMAN', policy, creation: CREATION, content,
    });

    expect(chairman.actions.map(record => `${record.action}|${record.completed}`))
      .toEqual(cozy.actions.map(record => `${record.action}|${record.completed}`));
    ...
    expect(chairman.opener.opponentStrength).toBe(cozy.opener.opponentStrength);
  });
});
...
describe('the opening fixture', () => {
  it('is a week 3 league match against the strongest club in the division', () => {
    ...
    const meanFor = (clubId: string) => {
      ...
      return squad.reduce(
        (sum, player) => sum + keys.reduce((inner, key) => inner + player.attrs[key], 0) / keys.length,
        0,
      ) / squad.length;
    };
