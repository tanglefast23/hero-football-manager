...
describe('training headroom', () => {
  it('reports launch-roster headroom and the division potential curve', () => {
    ...
    lines.push(
      'player                role  arch          now  ceiling  headroom  pacNow/pacCap',
    );
    ...
    lines.push(
      'div   tier1(46-57) tier2(58-69) tier3(70-81) tier4(82-93) tier5(94-99)',
    );
    for (const division of [5, 4, 3, 2, 1]) {
      ...
      lines.push(
        `D${division}    ${counts.map((c) => `${c}%`.padEnd(13)).join('')}`,
      );
    }
