...
const ROLE_KEYS: Readonly<
  Record<Exclude<Role, 'GK'>, readonly (keyof Attrs)[]>
> = {
  DEF: ['def', 'pac', 'pas', 'tec'],
  MID: ['pas', 'tec', 'pac', 'def'],
  FWD: ['sho', 'tec', 'pac', 'pas'],
...
describe('scale-invariant automatic-substitution acceptance gate', () => {
  it('freezes the shipping D5 reference cohort and bootstrap protocol', () => {
    ...
    expect(
      [...artifact.baselineBits].every((bit) => bit === '0' || bit === '1'),
    ).toBe(true);
    expect(
      [...artifact.baselineBits].filter((bit) => bit === '1'),
    ).toHaveLength(artifact.baselineAccepted);
  });

  it('reproduces the old D5 frequency within ±3 pp and is no more eager at trained scale', () => {
    const scenarios = generateScenarios(
      artifact.scenarios,
      artifact.scenarioSeed,
    );
    const baseline = [...artifact.baselineBits].map((bit) => bit === '1');
    const d5 = scenarios.map((scenario) =>
    ...
    const trained = scenarios.map((scenario) =>
      automaticSubstitutionClearsMargin(
    ...
    const scalePointDifference =
      trained.filter(Boolean).length / trained.length -
...
function generateScenarios(count: number, seed: number): Scenario[] {
  ...
  return Array.from({ length: count }, (_, index) => {
    ...
    const outgoingRatings = ROLE_KEYS[role].map(
      () => 35 + Math.floor(random() * 21),
    );
    const replacementRatings = ROLE_KEYS[role].map(
      () => 35 + Math.floor(random() * 21),
    );
...
function scalePlayer(player: PlayerDef, scale: number): PlayerDef {
  return {
    ...
    attrs: Object.fromEntries(
      Object.entries(player.attrs).map(([key, value]) => [key, value * scale]),
    ) as unknown as Attrs,
  };
}
