...
describe('strength-gap probe', () => {
  it('maps opponent strength delta to win/draw/loss', () => {
    ...
    const lines = [
      '',
      '=== OPPONENT STRENGTH DELTA -> USER RESULT (user always home) ===',
    ];
    ...
    for (const delta of DELTAS) {
      lines.push(
        formatRow(
          `${delta >= 0 ? '+' : ''}${delta}`,
          sweep(user, scale(opponent, delta)),
        ),
      );
    }
    ...
    for (const delta of DELTAS) {
      lines.push(
        formatRow(
          `${delta >= 0 ? '+' : ''}${delta}`,
          sweep(withHero, scale(opponent, delta)),
        ),
      );
    }
...
function formatRow(label: string, outcome: Outcome): string {
  const pct = (value: number) =>
    `${(Math.round((value / SEEDS) * 1000) / 10).toFixed(1)}%`.padEnd(7);
  const mean = (value: number) =>
    (Math.round((value / SEEDS) * 100) / 100).toFixed(2);
  return (
    `${label.padEnd(8)}${pct(outcome.wins)}${pct(outcome.draws)}${pct(outcome.losses)}` +
