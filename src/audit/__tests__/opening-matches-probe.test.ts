...
describe('opening-match balance probe', () => {
  it('reports win/draw/loss for the first matches of a fresh career', () => {
    ...
    const totalTrainedWeeks = trained.reduce(
      (sum, run) => sum + run.weeksTrained,
      0,
    );
...
function playMatchday(state: GameState): {
  ...
  const results = fixtures.map((candidate) =>
  ...
  if (kind === 'league') {
    const participantIds = (
      userIsHome ? quick.replay.home : quick.replay.away
  ...
  return {
    ...
    match: {
      ...
      goalsAgainst: userIsHome
        ? quick.result.awayGoals
        : quick.result.homeGoals,
      userStrength: lineupStrength(userTeam),
      oppStrength: lineupStrength(oppTeam),
      heroCount: userTeam.players.filter((player) => player.power !== undefined)
        .length,
      home: userIsHome,
...
function lineupStrength(team: TeamDef): number {
  const keys: readonly (keyof Attrs)[] = [
    'pac',
    'sho',
    'pas',
    'def',
...
function report(label: string, runs: CareerRun[]): void {
  ...
  for (let index = 0; index < MATCHES_TRACKED; index += 1) {
    ...
    const pct = (value: number) =>
      `${Math.round((value / matches.length) * 1000) / 10}%`;
    const avg = (pick: (m: PlayedMatch) => number) =>
      Math.round(
        (matches.reduce((sum, m) => sum + pick(m), 0) / matches.length) * 100,
      ) / 100;
    lines.push(
...
function matchOutcome(match: PlayedMatch | undefined): 'W' | 'D' | 'L' {
  if (match === undefined)
    throw new Error('opening outcome export requires the first match');
