...
const describeProbe = process.env.OPENING_GAP_PROBE === '1' ? describe : describe.skip;
const content = loadLaunchContent();
const KEYS: Array<keyof Attrs> = ['pac', 'sho', 'pas', 'def', 'tec', 'sta', 'ref'];
...
function teamMean(team: TeamDef): number {
  const perPlayer = team.players.map(p => (
    KEYS.reduce((sum, k) => sum + p.attrs[k], 0) / KEYS.length
  ));
...
function attrMeans(team: TeamDef): Record<string, number> {
  return Object.fromEntries(KEYS.map(k => [
    k,
    team.players.reduce((sum, p) => sum + p.attrs[k], 0) / team.players.length,
  ]));
}

describeProbe('opening gap', () => {
  it('reports the user squad against its own Division 5 field', () => {
    const lines = ['', '=== D5 OPENING GAP (user vs the nine clubs it must beat) ==='];
    for (const seed of [4_000_000, 20_260_725, 991_237]) {
      ...
      const rivals = ids.filter(id => id !== state.userClubId).map(id => teamMean(teams[id]));
      const fieldMean = rivals.reduce((a, b) => a + b, 0) / rivals.length;

      lines.push('', `seed ${seed}: user ${userMean.toFixed(2)}`
      ...
      lines.push(`  2nd-weakest rival (the promotion bar) ${rivals.slice().sort((a, b) => a - b)[1].toFixed(2)}`);

      const userAttrs = attrMeans(teams[state.userClubId]);
      const fieldAttrs = KEYS.map(k => {
        const vals = ids.filter(id => id !== state.userClubId)
