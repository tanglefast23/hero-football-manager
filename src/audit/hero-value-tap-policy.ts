 ...
 */
export function shouldQueueWellTappedPower(state: MatchState, slot: number): boolean {
  const player = state.players[slot];
  if (player?.powerState.kind !== 'zone') return false;
  if (inUsefulContext(state, slot)) return true;
  const keeperPower = player.def.power === 'ELASTIC_KEEPER' || player.def.power === 'GIANT_GK';
