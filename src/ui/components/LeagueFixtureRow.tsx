 ...
 */
const VENUE_KEYS = { HOME: 'm2League.venueHome', AWAY: 'm2League.venueAway' } as const;
const VENUE_A11Y_KEYS = { HOME: 'm2League.a11y.venueHome', AWAY: 'm2League.a11y.venueAway' } as const;
const RESULT_KEYS = {
...
export function LeagueFixtureRow({ fixture }: { fixture: M2LeagueFixtureViewModel }) {
  ...
  const resultTone = fixture.result === 'WIN'
    ...
    : fixture.result === 'LOSS'
      ? 'danger' as const
      : 'normal' as const;
  const resultLabel = fixture.result !== undefined
    ? t(RESULT_KEYS[fixture.result])
    : fixture.currentWeek ? t('m2League.fixtureNext') : t(VENUE_KEYS[fixture.venue]);
  ...
  return (
    <View
      ...
      accessibilityLabel={t('m2League.a11y.fixtureRow', {
        ...
        result: fixture.status === 'PLAYED' ? fixture.scoreLabel : t('m2League.a11y.scheduled'),
      })}
    ...
    >
      <View className="w-16 pr-2">
        ...
        <Text className="mt-1 font-mono text-sm uppercase text-ink/50">{t(VENUE_KEYS[fixture.venue])}</Text>
      </View>
      <View className="flex-1 border-l border-ink/20 pl-3">
        <Text className="text-sm text-ink/50" numberOfLines={1}>{t(fixture.venue === 'HOME' ? 'm2League.hosts' : 'm2League.visits')}</Text>
