...
export function PostMatchBuzzCard({
  buzz,
  className = '',
}: {
  ...
  const breakdown =
    [
      buzz.win > 0
        ? t('postMatchBuzzCard.win', { points: buzz.win })
        : undefined,
      buzz.goals > 0
        ? t('postMatchBuzzCard.goals', { points: buzz.goals })
        : undefined,
      buzz.heroMoments > 0
        ? t('postMatchBuzzCard.heroes', { points: buzz.heroMoments })
        : undefined,
    ]
      .filter(Boolean)
      .join(' · ') || t('postMatchBuzzCard.noBuzzEarned');
  ...
  const accessibilityLabel = [
    ...
    buzz.payout === undefined
      ...
      : t('postMatchBuzzCard.a11y.paidOutAndReset', {
          amount: formatCurrency(t, buzz.payout),
        }),
  ]
    .filter(Boolean)
    .join(' ');
