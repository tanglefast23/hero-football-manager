...
export function FinancialStatement({
  ...
  const netTone =
    netAmount > 0 ? 'income' : netAmount < 0 ? 'expense' : 'neutral';
  ...
  return (
    <View>
      ...
      <RecordedStamp
        phase={machine.stampPhase}
        onSlammed={handleStampSettled}
      />
      <SurgeBanner
    ...
    >
      ...
      <View className="flex-row items-center">
        ...
        <View className="min-w-0 flex-1 flex-row flex-wrap items-center">
          ...
          {chipVisible ? (
            <BonusBadge
              label={multiplierLabel(reveal!)}
              reduceMotion={reduceMotion}
            />
          ) : null}
          {adjacencyBadgeVisible && reveal?.source === 'merch' ? (
            <BonusBadge
              label={`+${reveal.adjacencyPercent}%`}
              reduceMotion={reduceMotion}
            />
          ) : null}
        </View>
...
function rowAccessibilityLabel(
  ...
  const reveal = line.reveal;
  if (reveal === undefined) {
    return t('financialStatement.a11y.rowPlain', {
      label: line.label,
      amount: signed,
    });
  }
  const surgeNote = reveal.surge
    ? ` ${t('financialStatement.a11y.surgedThisWeek')}`
    : '';
