...
export type BertEyes =
  'open' | 'wide' | 'squint' | 'closed' | 'side' | 'up' | 'dot';
...
function eyes(style: BertEyes): BertPart[] {
  switch (style) {
    case 'wide':
      return [
        part('left', 35, 'top', 48, 8, 8, C.ink),
        part('right', 34, 'top', 48, 8, 8, C.ink),
      ];
    case 'squint':
      return [
        part('left', 35, 'top', 51, 7, 3, C.ink),
        part('right', 34, 'top', 51, 7, 3, C.ink),
      ];
    case 'closed':
      return [
        part('left', 34, 'top', 52, 9, 2, C.ink),
        part('right', 33, 'top', 52, 9, 2, C.ink),
      ];
    case 'side':
      return [
        part('left', 33, 'top', 50, 5, 5, C.ink),
        part('right', 37, 'top', 50, 5, 5, C.ink),
      ];
    case 'up':
      return [
        part('left', 36, 'top', 48, 5, 5, C.ink),
        part('right', 35, 'top', 48, 5, 5, C.ink),
      ];
    case 'dot':
      return [
        part('left', 37, 'top', 51, 3, 3, C.ink),
        part('right', 36, 'top', 51, 3, 3, C.ink),
      ];
    default:
      return [
        part('left', 36, 'top', 50, 5, 5, C.ink),
        part('right', 35, 'top', 50, 5, 5, C.ink),
      ];
  }
}
...
function mouth(style: BertMouth): BertPart[] {
  switch (style) {
    case 'open':
      return [part('left', 44, 'top', 81, 17, 8, C.mouth)];
    case 'wide':
      return [part('left', 41, 'top', 80, 23, 11, C.mouth)];
    case 'smile':
      return [
    ...
    case 'smirk':
      return [part('left', 44, 'top', 82, 15, 4, C.mouth, -8)];
    case 'grimace':
      return [
    ...
    case 'oh':
      return [part('left', 48, 'top', 80, 9, 9, C.mouth)];
    default:
      return [part('left', 46, 'top', 82, 13, 4, C.mouth)];
  }
}
...
export function bertPostureParts(id: BertPostureId): BertPart[] {
  ...
  if (spec.custom === 'stride')
    return [...stride(), ...ARMS[spec.arms ?? 'rest']()];
...
export const BERT_MOMENTS = {
  hello: {
    label: 'Meeting you for the first time',
    expression: 'warm',
    posture: 'stand',
  },
  introducing: {
  ...
  explaining: {
    label: 'Explaining how something works',
    expression: 'explaining',
    posture: 'both-out',
  },
  'pointing-out': {
    label: 'Pointing at the thing you need',
    expression: 'insistent',
    posture: 'lean-point',
  },
  'pointing-up': {
    label: 'One important detail',
    expression: 'stern',
    posture: 'point-up',
  },
  listing: {
    label: 'Running through the numbers',
    expression: 'deadpan',
    posture: 'clipboard',
  },
  'warning-money': {
    label: 'Warning you about money',
    expression: 'grave',
    posture: 'point',
  },
  'warning-hard': {
  ...
  'telling-off': {
    label: 'Telling you off',
    expression: 'barking',
    posture: 'lean-point',
  },
  disapproving: {
    label: 'Disapproving quietly',
  ...
  unsure: {
    label: 'Not sure what to tell you',
    expression: 'thinking',
    posture: 'scratch',
  },
  shrugging: {
    label: 'It is out of his hands',
    expression: 'unimpressed',
    posture: 'shrug',
  },
  'hold-on': {
    label: 'Asking you to wait',
    expression: 'insistent',
    posture: 'hand-up',
  },
  confiding: {
    label: 'Letting you in on something',
    expression: 'conspiring',
    posture: 'confide',
  },
  smug: { label: 'He told you so', expression: 'smug', posture: 'lean-back' },
  encouraging: {
    label: 'Talking you into it',
    expression: 'encouraging',
    posture: 'lean-in',
  },
  approving: {
    label: 'Approving of a decision',
    expression: 'warm',
    posture: 'thumbs',
  },
  celebrating: {
  ...
  relieved: {
    label: 'Relieved it worked out',
    expression: 'relieved',
    posture: 'pocket',
  },
  laughing: {
    label: 'Laughing at you, fondly',
    expression: 'chuckling',
    posture: 'lean-back',
  },
  hopeful: {
    label: 'Hoping you say yes',
    expression: 'pleading',
    posture: 'present',
  },
  'worn-out': {
    label: 'Long season, tired',
    expression: 'weary',
    posture: 'lie-prop',
  },
