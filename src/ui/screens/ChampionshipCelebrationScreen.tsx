...
export function ChampionshipCelebrationScreen({
  ...
  const celebrationVisualIds = useMemo(
    () =>
      [viewModel.star, ...viewModel.squad].map((player) =>
  ...
  useEffect(() => {
    ...
    const tossAnimation = Animated.loop(
      Animated.sequence([
    ...
    const crowdAnimation = Animated.loop(
      Animated.sequence([
    ...
    const fireworkAnimation = Animated.loop(
      Animated.sequence([
        Animated.delay(320),
  ...
  const tossedStyle = reduceMotion
    ? undefined
    : {
        transform: [
          {
    ...
    >
      ...
      <Pressable
        ...
        accessibilityLabel={t(
          'championshipCelebration.a11y.skipLeagueChampionshipCelebration',
        )}
      ...
      >
        ...
        <Text className="font-pixel text-xs uppercase text-white">
          {t('championshipCelebration.skip')}
        </Text>
      </Pressable>
      ...
      >
        <Text className="text-center font-pixel text-xs uppercase tracking-[4px] text-gold">
          {t('championshipCelebration.finalWhistle', {
            season: viewModel.seasonLabel,
          })}
        </Text>
        <Text
      ...
      <View pointerEvents="none" style={styles.celebrationStage}>
        ...
        <View style={styles.starNameplate}>
          <Text
            className="text-center font-pixel text-base uppercase text-ink"
            numberOfLines={1}
          >
            {viewModel.star.name}
          </Text>
        ...
        <View style={styles.bertLabel}>
          <Text className="font-pixel text-[10px] uppercase text-white">
            {viewModel.assistantName}
          </Text>
          <Text className="font-mono text-[8px] uppercase text-gold">
            {t('championshipCelebration.stillHere')}
          </Text>
        </View>
...
const styles = StyleSheet.create({
  ...
  pitchLine: {
    ...
    top: '56%',
    height: 3,
    backgroundColor: 'rgba(244,241,234,0.75)',
  },
  centerCircle: {
    position: 'absolute',
    left: '32%',
  ...
  skipButton: {
    ...
    backgroundColor: 'rgba(36,31,46,0.72)',
  },
  titleCard: {
    position: 'absolute',
    zIndex: 15,
  ...
  starBurst: {
    ...
    borderWidth: 4,
    borderColor: '#f6c744',
    backgroundColor: 'rgba(246,199,68,0.24)',
  },
  starCrown: {
    position: 'absolute',
    zIndex: 3,
