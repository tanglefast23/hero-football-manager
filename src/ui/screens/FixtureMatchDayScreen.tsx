...
export function FixtureMatchDayScreen({
  ...
  return (
    <SafeAreaView className="flex-1 bg-ink" edges={['top', 'left', 'right', 'bottom']}>
      <View className="flex-row items-center justify-between border-b-2 border-sky/30 px-4 py-3">
        <Pressable
          accessibilityRole="button"
          accessibilityLabel="Return to club management"
          onPress={onBack}
          className="min-h-11 min-w-11 items-center justify-center border border-paper/30"
          style={({ pressed }) => ({ opacity: pressed ? 0.65 : undefined })}
        >
          <Text className="font-mono text-lg font-bold text-paper">‹</Text>
        </Pressable>
        <View className="flex-1 px-3">
          <Text className="text-center text-xs font-bold uppercase tracking-[2px] text-sky">Match-day docket</Text>
          <Text className="mt-1 text-center text-sm font-bold uppercase text-paper">{fixture.competition}</Text>
        </View>
        <View className="min-w-11 border border-stamp px-2 py-2">
          <Text className="text-center font-mono text-xs font-bold text-stamp">{fixture.weekLabel}</Text>
        </View>
      </View>
      ...
      <ScrollView className="flex-1" contentContainerStyle={{ padding: 16, paddingBottom: 24 }}>
        <PaperPanel stamp={fixture.venueLabel}>
          <View className="flex-row items-center gap-3">
            <Text className="flex-1 text-right text-lg font-bold uppercase text-ink" numberOfLines={2}>{fixture.homeTeam}</Text>
            <View className="border-2 border-ink bg-ink px-3 py-2">
              <Text className="font-mono text-sm font-bold text-signal">V</Text>
            </View>
            <Text className="flex-1 text-lg font-bold uppercase text-ink" numberOfLines={2}>{fixture.awayTeam}</Text>
          </View>
                  ...
                  {players.map(player => (
                    <View key={player.id} className="w-14 items-center">
                      <View className={player.isHero ? 'h-9 w-9 items-center justify-center border-2 border-amber-500 bg-ink' : 'h-9 w-9 items-center justify-center border-2 border-paper bg-ink'}>
                        <Text className={player.isHero ? 'font-mono text-xs font-bold text-amber-400' : 'font-mono text-xs font-bold text-paper'}>
                          {player.shirtNumber}
                        </Text>
                      </View>
                      <Text className="mt-1 text-center text-xs font-bold text-paper" numberOfLines={1}>{player.name}</Text>
                    </View>
                  ))}
                </View>
              );
            ...
            {viewModel.tactics.map(tactic => {
              ...
              return (
                <View
                  ...
                  className={selected ? 'min-h-14 flex-row items-center border-2 border-ink bg-signal p-3' : 'min-h-14 flex-row items-center border-2 border-paper/30 bg-ink-soft p-3'}
                >
                  <View className={selected ? 'mr-3 h-5 w-5 items-center justify-center border-2 border-ink' : 'mr-3 h-5 w-5 items-center justify-center border-2 border-paper/40'}>
                    {selected ? <View className="h-2 w-2 bg-ink" /> : null}
                  </View>
                  <View className="flex-1">
                    <Text className={selected ? 'text-sm font-bold uppercase text-ink' : 'text-sm font-bold uppercase text-paper'}>{tactic.label}</Text>
                    <Text className={selected ? 'mt-1 text-xs text-ink/60' : 'mt-1 text-xs text-paper/50'}>{tactic.detail}</Text>
                  </View>
                </View>
              );
            })}
        ...
        <View className="mt-6">
            ...
            {viewModel.heroes.length === 0 ? (
              ...
              <PaperPanel key={hero.playerId} className={hero.licensed ? 'bg-amber-100' : undefined}>
                <View className="flex-row items-center gap-3">
                  ...
                  >
                    <Text className="font-mono text-lg font-bold text-ink">{hero.licensed ? '★' : '○'}</Text>
                  </Pressable>
                  <View className="flex-1">
                    <Text className="text-sm font-bold uppercase text-ink">{hero.playerName}</Text>
                    <Text className="mt-1 text-xs font-bold uppercase tracking-wide text-amber-700">{hero.powerName}</Text>
                  </View>
                  <View
                    accessible
                  ...
                  >
                    <Text className="text-xs font-bold uppercase text-ink">Watch · Tap</Text>
                    <Text className="mt-1 text-xs font-bold uppercase text-ink/50">Quick · Auto</Text>
                  </View>
                </View>
              </PaperPanel>
            ))}
          ...
          {viewModel.heroes.length > 0 ? (
            <Text className="mt-3 text-xs leading-4 text-paper/50">
