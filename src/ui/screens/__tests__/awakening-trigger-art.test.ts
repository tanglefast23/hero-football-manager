...
const ART_FILES = [
  ...SCENE_ART_FILES,
  'AwakeningTriggerCalloutIcon.tsx',
] as const;
const SMOOTH_VECTOR_PRIMITIVES =
  /\b(?:Circle|Line|Path|RoundedRect|Text|matchFont)\b/;
...
function pixelCoordinateErrors(fileName: string, source: string): string[] {
  ...
  function visit(node: ts.Node) {
    if (ts.isArrayLiteralExpression(node) && node.elements.length >= 5) {
      const firstFour = node.elements
        .slice(0, 4)
        .map((element) => numericLiteral(element as ts.Expression));
...
describe('awakening trigger pixel-art contract', () => {
  test.each(ART_FILES)(
    '%s uses the shared palette and hard pixel blocks',
    (fileName) => {
  ...
  it('places later triggers on the opaque beige card but leaves the caterpillar unframed', () => {
    const source = readFileSync(
      join(ART_DIRECTORY, 'AwakeningTriggerVisual.tsx'),
      'utf8',
    );
