...
export type FacilityPlacement =
  ...
  | {
      readonly kind: 'move';
      readonly building: ClubFacilityBuildingViewModel;
      readonly x: number;
      readonly y: number;
    };
...
export function FacilityPlacementConfirmation({
  ...
  const t = useCopy();
  const { x, y } = placement;
  const cellLabel = t('facilityPlacementConfirmation.cellLabel', {
    column: x + 1,
    row: y + 1,
  });
        ...
        >
          ...
          >
            ...
            <View className="mt-4 flex-row gap-2">
              <Metric
                label={order.costLabel}
                value={formatCurrency(t, order.cost)}
                tone="negative"
              />
              <Metric
                label={t('facilityPlacementConfirmation.upkeep')}
                value={t('clubFinances.perWeekShort', {
                  amount: formatCurrency(t, order.upkeep),
                })}
                tone="negative"
              />
            </View>
            ...
            <View className="mt-4 gap-2">
              ...
              <ActionButton
                ...
                accessibilityLabel={t(
                  'facilityPlacementConfirmation.a11y.cancelThisPlacementAnd',
                )}
                variant="paper"
                onPress={onCancel}
              />
