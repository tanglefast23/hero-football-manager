...
describe('awakening cutscene framing', () => {
  ...
  it.each(viewports)(
    'keeps the cast inside the visible strip on $label',
    ({ width, height }) => {
  ...
  it('anchors on the viewport, never on the full-pitch canvas height', () => {
    ...
    expect(source).toContainSource('<RecoverableSkiaCanvas');
    expect(source).toContainSource('style={{ width, height: viewportHeight }}');
  });
  ...
  it('takes its tap hints from the catalog, in every language', () => {
    ...
    for (const literal of [
      ...
      expect({ literal, hardcoded: source.includes(`'${literal}'`) }).toEqual({
        literal,
        hardcoded: false,
      });
    }
    ...
    for (const locale of ENABLED_LOCALES) {
      ...
      expect({
        locale,
        missing: keys.filter(
          (key) => strings[`awakening.${key}`] === undefined,
        ),
