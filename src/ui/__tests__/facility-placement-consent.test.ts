...
describe('facility placement consent and feedback', () => {
  it('confirms a relocation fee instead of spending it on the destination tap', () => {
    ...
    expect(panel).toContainSource('cost: placement.building.relocationFee');
    expect(panel).toContainSource(
  ...
  it('answers a blocked square with a refusal cue and a reason', () => {
    ...
    expect(screen).toContainSource("t('clubFinances.squareTaken')");
    expect(screen).toContainSource("t('clubFinances.squareDoesNotFit')");
    ...
    expect(screen).toContainSource('setPlacementRejection(null);');
