...
describe('tutorial cue positioning', () => {
  it('centers the cue below the measured control', () => {
    expect(
      tutorialCuePosition({ x: 160, y: 70, width: 64, height: 34 }, 390),
    ).toEqual({
      left: 119,
      top: 110,
    });
  ...
  it('keeps the cue inside either edge of the viewport', () => {
    expect(
      tutorialCuePosition({ x: 0, y: 10, width: 20, height: 20 }, 390).left,
    ).toBe(8);
    expect(
      tutorialCuePosition({ x: 370, y: 10, width: 20, height: 20 }, 390).left,
    ).toBe(236);
  });
  ...
  it('only considers a scroll target visible once it is clearly inside the viewport', () => {
    ...
    expect(
      isTutorialTargetVisible(
        { x: 20, y: 676, width: 350, height: 54 },
        viewport,
      ),
    ).toBe(true);
    expect(
      isTutorialTargetVisible(
        { x: 20, y: 690, width: 350, height: 54 },
        viewport,
      ),
    ).toBe(false);
    expect(
      isTutorialTargetVisible(
        { x: 20, y: 720, width: 350, height: 54 },
        viewport,
      ),
    ).toBe(false);
  });
...
describe('desktop viewports', () => {
  ...
  it('keeps a column-2 target visible-check working with absolute coords', () => {
    ...
    expect(
      isTutorialTargetVisible(
        { x: 700, y: 750, width: 200, height: 56 },
        viewport,
      ),
    ).toBe(true);
    expect(
      isTutorialTargetVisible(
        { x: 700, y: 900, width: 200, height: 56 },
        viewport,
      ),
    ).toBe(false);
  });
