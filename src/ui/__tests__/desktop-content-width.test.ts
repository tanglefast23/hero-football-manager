...
const read = (file: string) =>
  readFileSync(join(process.cwd(), SCREEN_DIR, file), 'utf8');
...
describe('desktop content width', () => {
  const screens = readdirSync(join(process.cwd(), SCREEN_DIR)).filter(
    (file) => file.endsWith('.tsx') && !EXEMPT.has(file),
  );
  ...
  it('clamps every scrolling management screen to the two-column measure', () => {
    ...
    const unclamped = screens.filter((file) => {
      ...
      return (
        !source.includes('useDesktopContentStyle') &&
  ...
  it('keeps one definition of the measure', () => {
    const clamp = readFileSync(
      join(process.cwd(), 'src/ui/layout/DesktopClamp.tsx'),
      'utf8',
    );
