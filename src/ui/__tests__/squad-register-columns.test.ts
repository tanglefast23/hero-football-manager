...
describe('squad register column widths', () => {
  ...
  it.each(COLUMNS)('holds the phone %s header, its arrow and a gutter', (column) => {
    const demand = headerWidthDemand(LABELS[column].phone, HEADER_FONT_SIZE.phone);
    expect(REGISTER_COLUMN_WIDTH.phone[column] - demand).toBeGreaterThanOrEqual(HEADER_MIN_GUTTER);
  });

  it.each(COLUMNS)('holds the wide %s header, its arrow and a gutter', (column) => {
    const demand = headerWidthDemand(LABELS[column].wide, HEADER_FONT_SIZE.wide);
    expect(REGISTER_COLUMN_WIDTH.wide[column] - demand).toBeGreaterThanOrEqual(HEADER_MIN_GUTTER);
  });

  it('holds the row values at the largest text size the app allows', () => {
    // Values are not capped the way headers are, so they are measured against
    // the app-wide ceiling — read from App.tsx so the two cannot drift apart.
    const app = readFileSync(join(process.cwd(), 'App.tsx'), 'utf8');
    const ceiling = app.match(/const APP_MAX_FONT_SIZE_MULTIPLIER = ([\d.]+);/);
    expect(ceiling).not.toBeNull();
    const max = Number(ceiling?.[1]);

    for (const layout of ['phone', 'wide'] as const) {
      const width = REGISTER_COLUMN_WIDTH[layout];
      expect(width.overall).toBeGreaterThanOrEqual(CELL_WIDEST_VALUE_EM.overall * CELL_FONT_SIZE.value * max);
      expect(width.potential).toBeGreaterThanOrEqual(
        CELL_WIDEST_VALUE_EM.potential * CELL_FONT_SIZE.value * max + POTENTIAL_CELL_PADDING_RIGHT,
      );
  ...
  it('caps how far a header grows with the reader s text size', () => {
    expect(screenSource()).toContain('maxFontSizeMultiplier={HEADER_MAX_FONT_MULTIPLIER}');
  });
  ...
  it('gives the train button a tap target the platform accepts', () => {
    ...
    expect(TRAIN_BUTTON_DIAMETER + 2 * TRAIN_BUTTON_HIT_SLOP)
  ...
  it('gives the header and the cells under it one width each', () => {
    ...
    expect(source).toContain('const columns = wideColumns ? ROSTER_COLUMN_STYLE.wide : ROSTER_COLUMN_STYLE.phone;');
