...
describe('squad table sorting', () => {
  it('cycles a rating column best-first, then worst-first, then default', () => {
    ...
    expect(nextSquadSort(ascending, 'player')).toEqual({
      key: 'player',
      direction: 'ascending',
    });
  });
  ...
  it.each<[SquadSort, string[]]>([
    [
      ...
      ['Gio Marsh', 'Ed Stone', 'Bo Hedges', 'Sam Mitts'],
    ],
    [
      { key: 'potential', direction: 'descending' },
      ['Gio Marsh', 'Ed Stone', 'Bo Hedges', 'Sam Mitts'],
    ],
    [
      { key: 'condition', direction: 'ascending' },
      ['Gio Marsh', 'Ed Stone', 'Sam Mitts', 'Bo Hedges'],
    ],
    [
      { key: 'player', direction: 'ascending' },
      ['Bo Hedges', 'Ed Stone', 'Gio Marsh', 'Sam Mitts'],
    ],
    [
      { key: 'role', direction: 'ascending' },
      ['Ed Stone', 'Bo Hedges', 'Sam Mitts', 'Gio Marsh'],
    ],
  ])(
    'sorts $key $direction with default order as the tie-breaker',
    (sort, expected) => {
      expect(
        sortSquadPlayers(players, sort).map((player) => player.name),
      ).toEqual(expected);
    },
  );
  ...
  it('restores the exact default order when sorting is cleared', () => {
    expect(
      sortSquadPlayers(players, null).map((player) => player.name),
