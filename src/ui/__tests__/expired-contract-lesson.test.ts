  ...
  it('stays silent while the section is still below the fold', () => {
    ...
    expect(
      isExpiredContractRevealed({
        ...
        viewportHeight: VIEWPORT,
        scrollOffset:
          SECTION_TOP - VIEWPORT + EXPIRED_CONTRACT_REVEAL_MARGIN - 1,
      }),
    ).toBe(false);
  });
...
describe('the lesson is said once per career', () => {
  ...
  it('starts unsaid and stays said', () => {
    expect(hasAssistantGuideMilestone(career, 'expired-contract-seen')).toBe(
      false,
    );
    const after = completeAssistantGuideMilestone(
      career,
      'expired-contract-seen',
    );
    expect(hasAssistantGuideMilestone(after, 'expired-contract-seen')).toBe(
      true,
    );
    // Idempotent: reaching the section again in the same career must not append
    // a second flag to the save.
    expect(
      completeAssistantGuideMilestone(after, 'expired-contract-seen'),
    ).toBe(after);
  });

  it('banks silently for an Advisor who reaches the section', () => {
    const advisor = { ...career, assistantMode: 'advisor' as const };
    expect(advisorMilestonesToBank(advisor, NO_CONTEXT)).not.toContain(
      'expired-contract-seen',
    );
    expect(
  ...
  it('banks nothing for a Teacher, who is shown it instead', () => {
    expect(
      advisorMilestonesToBank(career, {
        ...NO_CONTEXT,
        viewingExpiredContract: true,
      }),
    ).toEqual([]);
