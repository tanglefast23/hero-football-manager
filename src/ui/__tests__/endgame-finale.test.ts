...
describe('the curtain call', () => {
  ...
  it('stands them in two rows, the far one smaller and higher', () => {
    ...
    // other way round.
    expect(stage.spots.slice(0, back.length).every(spot => spot.row === 0)).toBe(true);
  });
  ...
  it('is a crowd jumping rather than one body copied', () => {
    const phases = finaleStage(squad(12), PHONE.width, PHONE.height).spots.map(spot => spot.phase);

    phases.forEach(phase => {
      expect(phase).toBeGreaterThanOrEqual(0);
      expect(phase).toBeLessThan(1);
    });
    expect(new Set(phases.map(phase => phase.toFixed(4))).size).toBe(phases.length);
  });

  it('stages the same men in the same places every time', () => {
    expect(finaleStage(squad(9), PHONE.width, PHONE.height))
      .toEqual(finaleStage(squad(9), PHONE.width, PHONE.height));
  });

  /** The fault the whole scene exists to end: a figure standing on the stand. */
  it.each(SIZES)('keeps every man on the grass at $width x $height', ({ width, height }) => {
    ...
    stage.spots.forEach(spot => {
      ...
      expect(spot.y + FINALE_CELL.height * spot.scale).toBeLessThanOrEqual(height);
      expect(spot.x).toBeGreaterThanOrEqual(0);
      expect(spot.x + FINALE_CELL.width * spot.scale).toBeLessThanOrEqual(width);
    });
  });
   ...
   */
  it.each(SIZES)('lines both rows up on one mark, clear of Bert, at $width x $height', ({ width, height }) => {
    ...
    const centres = ([0, 1] as const).flatMap(row => {
      ...
      const right = inRow[inRow.length - 1].x + FINALE_CELL.width * inRow[0].scale;
      return [(left + right) / 2];
    });
...
describe('Bert, flat out in front of them', () => {
  it.each(SIZES)('lies nearer the viewer than anybody at $width x $height', ({ width, height }) => {
    const stage = finaleStage(squad(12), width, height);
    ...
    const nearestFeet = Math.max(...stage.spots.map(
      spot => spot.y + FINALE_CELL.height * spot.scale,
    ));
  ...
  it('lies down rather than standing up', () => {
    ...
    expect(lying.bottom - lying.top).toBeLessThan(standing.bottom - standing.top);
  });
});
