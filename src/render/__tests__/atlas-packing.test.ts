...
const separation = (a: Rect, b: Rect): number => Math.max(
...
describe('sprite atlas packing', () => {
  it('sizes every slot to the largest frame plus a gutter on each side', () => {
    const widths = Object.values(fullSheet.sprites).map(frame => frame[0].length);
    const heights = Object.values(fullSheet.sprites).map(frame => frame.length);
    ...
    expect(Math.max(...rects.map(rect => (rect.x % fullLayout.slotW) + rect.w)))
      .toBe(fullLayout.slotW - ATLAS_GUTTER);
    expect(Math.max(...rects.map(rect => (rect.y % fullLayout.slotH) + rect.h)))
  ...
  it('places every cell on an integer boundary, inset by the gutter', () => {
    ...
    for (const key of fullKeys) {
      ...
      if (rect.w !== frame[0].length || rect.h !== frame.length) {
        offenders.push(`${key}: rect ${rect.w}x${rect.h} != sprite ${frame[0].length}x${frame.length}`);
      }
      if (left !== ATLAS_GUTTER || top !== ATLAS_GUTTER) {
        offenders.push(`${key}: inset ${left},${top} != gutter ${ATLAS_GUTTER}`);
      }
      // The far edges matter as much as the near ones: the widest sprite in the
      // sheet is what makes the right/bottom margin tight.
      if (left + rect.w > fullLayout.slotW - ATLAS_GUTTER) offenders.push(`${key}: overruns its slot width`);
      if (top + rect.h > fullLayout.slotH - ATLAS_GUTTER) offenders.push(`${key}: overruns its slot height`);
    }
  ...
  it('keeps every cell inside the surface buildSpriteAtlas allocates', () => {
    ...
    const outside = fullKeys.filter(key => {
      const rect = fullLayout.rectFor(key);
      return rect.x < 0 || rect.y < 0 || rect.x + rect.w > atlasW || rect.y + rect.h > atlasH;
    });
  ...
  it('gives every cell a slot of its own, so no two can overlap', () => {
    ...
    for (const key of fullKeys) {
      ...
      if (previous !== undefined) shared.push(`${previous} and ${key} share slot ${slot}`);
  ...
  it('leaves a transparent texel between every pair of cells', () => {
    ...
    const rects = Object.keys(sheet.sprites).map(key => ({ key, rect: layout.rectFor(key) }));
    ...
    for (let i = 0; i < rects.length; i += 1) {
      for (let j = i + 1; j < rects.length; j += 1) {
        ...
        if (overlaps(rects[i].rect, rects[j].rect)) touching.push(`${pair}: overlap`);
        ...
        // because a sprite scaled up samples off both sides of the boundary.
          ...
          touching.push(`${pair}: only ${separation(rects[i].rect, rects[j].rect)}px apart`);
        }
      }
    }
  ...
  const api = {
    Surface: {
      MakeOffscreen: (width: number, height: number) => {
        ...
        return {
          getCanvas: () => ({ drawRect: (rect: SkiaRectLike) => { drawn.push(rect); } }),
          flush: () => undefined,
...
describe('sprite atlas painting', () => {
  ...
  it('paints every pixel run inside its own cell, never into a gutter', () => {
    ...
    for (const run of skia.drawn) {
      ...
      const cell = cellForSlot.get(`${Math.floor(run.x / layout.slotW)},${Math.floor(run.y / layout.slotH)}`);
      ...
      if (run.height !== 1) { strays.push(`${at}: height != 1`); continue; }
      if (cell === undefined) { strays.push(`${at}: lands in no cell`); continue; }
      if (run.x < cell.rect.x || run.x + run.width > cell.rect.x + cell.rect.w) {
      ...
      if (xInSlot < ATLAS_GUTTER || xInSlot + run.width > layout.slotW - ATLAS_GUTTER) {
