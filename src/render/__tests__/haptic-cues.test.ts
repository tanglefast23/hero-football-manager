...
describe('match haptic cues', () => {
  ...
  it('makes both Gust beats tangible for the owning team', () => {
    expect(hapticCueForEvent({ t: 4, kind: 'GUST_REDIRECT', player: 2, from: 12, to: 0 }, 0)).toBe('power');
    expect(hapticCueForEvent({ t: 5, kind: 'GUST_PUNT', player: 2, from: 0, to: 9 }, 1)).toBe('rival-power');
  });
});
