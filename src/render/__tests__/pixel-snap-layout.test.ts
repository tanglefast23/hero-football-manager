...
const DEVICES: readonly Device[] = [
  ...
  },
  {
    ...
    height: 932,
    dpr: 3,
    unsnappedPlayerRatio: 3.14,
  },
  {
    name: 'Android (Pixel-class)',
    width: 411,
    height: 823,
    dpr: 2.625,
    unsnappedPlayerRatio: 2.28,
  },
  {
    name: 'iPad',
    width: 768,
  ...
  },
  {
    ...
    height: 1080,
    dpr: 1,
    unsnappedPlayerRatio: 1.29,
  },
];

describe('match pitch pixel-grid layout', () => {
  test.each(DEVICES)(
    '$name magnifies sprites by a whole number of device pixels',
    (device) => {
      const layout = matchPitchLayout(
        device.width,
        availablePitchHeight(device.height),
        device.dpr,
      );
      ...
      for (const sprite of [layout.player, layout.ball]) {
        ...
        expect(
          Math.abs(
            layout.scale * sprite.drawScale * device.dpr - sprite.devicePixels,
          ),
        ).toBeLessThan(1e-9);
      }
    },
  ...
  test.each(DEVICES)(
    '$name never samples below one device pixel per source pixel',
    (device) => {
      const layout = matchPitchLayout(
        device.width,
        availablePitchHeight(device.height),
  ...
  test.each(DEVICES)(
    '$name keeps sprites near their authored footprint',
    (device) => {
      const layout = matchPitchLayout(
        device.width,
        availablePitchHeight(device.height),
      ...
      for (let height = 560; height <= 1400; height += 31) {
        for (const dpr of [1, 1.5, 2, 2.625, 2.75, 3, 3.5]) {
          ...
          for (const sprite of [layout.player, layout.ball]) {
            ...
            expect(
              Math.abs(
                layout.scale * sprite.drawScale * dpr - sprite.devicePixels,
              ),
            ).toBeLessThan(1e-9);
          }
          expect(Number.isInteger(layout.pitchWidth)).toBe(true);
...
describe('renderer wiring contract', () => {
  ...
  test('MatchScreen reserves the chrome heights this table assumes', () => {
    ...
    expect(source).toContainSource('compactHeight ? 226 : 286');
    expect(source).toContainSource(
      'Math.max(280, height - reservedChromeHeight)',
    );
  ...
  test('MatchScreen feeds every sprite consumer the snapped draw scale', () => {
    const code = readFileSync(
      join(renderDir, 'MatchScreen.tsx'),
      'utf8',
    ...
    expect(code).not.toContainSource('PLAYER_DRAW_SCALE');
    expect(code).not.toContainSource('BALL_DRAW_SCALE');
  });
  ...
  test('every atlas transform rounds its translate to whole device pixels', () => {
    ...
    expect(source).toContainSource("from './pixel-grid'");
  });
  ...
  test('the drill stage snaps its sprite magnifications like the match pitch', () => {
    ...
    const source = readFileSync(
      join(renderDir, 'DrillSceneOverlay.tsx'),
      'utf8',
    );

    expect(source).toContainSource(
      'snapSpriteScale(1, NOMINAL_PLAYER_SCALE, devicePixelRatio)',
    );
    expect(source).toContainSource(
      'snapSpriteScale(1, NOMINAL_BALL_SCALE, devicePixelRatio)',
    );
    expect(source).toContainSource(
    ...
    for (const dpr of [1, 1.5, 2, 2.625, 3]) {
      for (const nominal of [4.1, 3.1]) {
        ...
        expect(
          Math.abs(snapped.drawScale * dpr - snapped.devicePixels),
        ).toBeLessThan(1e-9);
      }
    }
  });
  ...
  test('one device-pixel snapping rule, shared by the camera, sprites and FX', () => {
    ...
    for (const file of [
      'interpolate.ts',
      'worklet-atlas-frame.ts',
      'WorkletMatchOverlays.tsx',
    ]) {
      expect(readFileSync(join(renderDir, file), 'utf8')).not.toMatchSource(
        inlineRule,
      );
    }
