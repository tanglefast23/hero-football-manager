...
function advanceThroughAutoFire(
  match: MatchState,
  power: PowerId,
  hero: number,
): void {
  ...
  throw new Error(
    `${power} did not auto-fire in its authored showcase context`,
  );
}

describe('live power match showcase', () => {
  it.each(LAUNCH_POWER_IDS)(
    '%s auto-fires through the real best-context policy',
    (power) => {
      ...
      const arrangedPositions = match.players.map((player) => ({
        ...player.pos,
      }));
      for (
        let heldTick = 0;
        heldTick < POWER_MATCH_SHOWCASE_AUTO_FIRE_DELAY_TICKS;
        heldTick += 1
      ) {
        expect(advancePowerMatchShowcaseReady(match, power)).toBe(true);
      }
      ...
      expect(match.events).not.toContainEqual(
        expect.objectContaining({ kind: 'POWER_FIRED' }),
      );
  ...
  it('keeps the rival team unpowered so only the selected effect is under review', () => {
    expect(
      powerMatchShowcaseAway().players.every(
  ...
  it('puts the newly awakened player into the live match clip', () => {
    ...
    expect(team.players[powerMatchShowcaseHeroIndex(power)].name).toBe(
      'Zip Vela',
    );
  });
  ...
  it('shows Web Trap on a clearly separated victim who stays fixed frame by frame', () => {
    ...
    const victim = 11;
    expect(
      Math.abs(match.players[hero].pos.x - match.players[victim].pos.x),
    ...
    for (
      let attempt = 0;
      attempt < 5 && match.players[victim].webbedUntilTick === undefined;
      attempt += 1
    ) {
      tick(match);
    }
  ...
  it('gives Elastic Keeper one off-centre real shot and completes the save', () => {
    ...
    for (
      let frame = 0;
      frame < 30 && !match.events.some((event) => event.kind === 'SAVE');
      frame += 1
    ) {
      tick(match);
    }
    ...
    expect(match.events).toContainEqual(
      expect.objectContaining({ kind: 'SAVE', by: hero }),
    );
   ...
   */
  it.each(LAUNCH_POWER_IDS)(
    '%s lands its promise inside the clip on its pinned seed',
    (power) => {
      const match = createMatch(
      ...
      for (
        let frame = 0;
        frame < CLIP_TICK_BUDGET && successAt === undefined;
        frame += 1
      ) {
      ...
      expect(successAt! - firedAt!).toBeLessThanOrEqual(
        MAX_FOLLOW_THROUGH_TICKS,
      );
    },
  );
  ...
  describe('the result card after the clip freezes', () => {
    ...
    function cardDueAt(power: PowerId): number | 'never' {
      ...
      const outroStartedAt = powerCutInOutroDue(stillActive, true)
        ? FROZEN_AT
        : undefined;
    ...
    it.each(LAUNCH_POWER_IDS)(
      '%s hands over a card a beat after it freezes',
      (power) => {
        expect(cardDueAt(power)).toBe(
          FROZEN_AT + POWER_MATCH_SHOWCASE_CARD_DELAY_MS,
        );
      },
    );
    ...
    it('is due off the freeze alone when no cut-in ending was ever recorded', () => {
      expect(powerMatchShowcaseCardDueAt(FROZEN_AT, undefined)).toBe(
        FROZEN_AT + POWER_MATCH_SHOWCASE_CARD_DELAY_MS,
      );
    });

    it('waits out a cut-in that is still ending after the freeze', () => {
      expect(powerMatchShowcaseCardDueAt(FROZEN_AT, FROZEN_AT + 400)).toBe(
        FROZEN_AT + 400 + POWER_MATCH_SHOWCASE_CARD_DELAY_MS,
      );
    });
  });
