...
describe('playstyle copy', () => {
    ...
    for (const locale of ENABLED_LOCALES.filter((one) => one !== 'en')) {
      ...
      for (const mentality of MENTALITIES) {
        for (const label of [mentalityLabel(mentality, t)]) {
          ...
          expect({ locale, mentality, label }).toMatchObject({
            locale,
            mentality,
          });
          expect(label).not.toBe(mentality);
          expect(label).not.toContainSource('playstyle');
          expect(Object.values(strings)).toContainSource(label);
        }
      }
    }
  });
  ...
  test('no match surface draws the raw enum any more', () => {
    ...
    expect(match).toContainSource('mentalityLabel(displayedMentality, t)');
    expect(match).toContainSource(
      "`${t('matchScreen.playstyle')} · ${mentalityLabel(mentality, t)}`",
    );
    expect(match).toContainSource(
      "`${t('matchScreen.playstyle')} · ${mentalityLabel(e.mentality, t)}`",
    );
    expect(match).not.toContainSource('{ playstyle: displayedMentality }');
    // The icon still switches on the enum, which is correct: it is a comparison,
    // not copy.
    expect(match).toContainSource("displayedMentality === 'ATTACK' ? '▲'");
  });
});
