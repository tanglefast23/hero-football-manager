import { ATLAS_GUTTER, atlasLayout, loadSpriteSheet, webbedSpriteKey } from '../loader';
...
describe('large career match-sprite roster', () => {
  ...
  it('loads every base and derived action frame', () => {
    ...
    expect(Object.keys(sheet.sprites)).toHaveLength(BASE_KEYS.length + SLIDE_KEYS.length + webbedKeys.length);
  });

  it('builds a match-sized sheet from only the active visual identities', () => {
    const sheet = loadSpriteSheet(['r:f00', 'u:g01', 'r:c167', 'r:f00']);
    const ordinaryKeys = Object.keys(sheet.sprites).filter(key => !key.endsWith(':webbed'));
    const webbedKeys = Object.keys(sheet.sprites).filter(key => key.endsWith(':webbed'));
