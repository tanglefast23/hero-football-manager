...
const BASE_KEYS = [
  ...
  ...SIDES.flatMap((side) =>
    manifest.goalkeeper.flatMap((id) => [
      `${side}:${id}:ready0`,
      `${side}:${id}:ready1`,
      `${side}:${id}:backReady0`,
      `${side}:${id}:backReady1`,
...
const SLIDE_KEYS = VISUAL_IDS.flatMap((id) =>
  Array.from(
    { length: SLIDE_TACKLE_FRAME_COUNT },
    (_, frame) => `${id}:${slideTackleSpriteFrame(frame)}`,
  ),
);
let fullSheet: ReturnType<typeof loadSpriteSheet>;

describe('large career match-sprite roster', () => {
  beforeAll(() => {
    // Deriving every roster-facing, slide, and Web variant is intentionally
    // exhaustive and expensive. Share that immutable result across contracts.
    fullSheet = loadSpriteSheet();
  });

  it('keeps identity when lineup position or team kit changes', () => {
    expect(spriteKeyForMatchPlayer(0, 'r0', 'GK', 'ready0')).toBe(
      'r:g00:ready0',
    );
    ...
    expect(spriteKeyForMatchPlayer(4, 'academy-s3-7', 'MID', 'run0')).toBe(
      `r:${generatedLook}:run0`,
    );
    expect(
      spriteKeyForMatchPlayer(4, 'created-player', 'FWD', 'run0', 'c167'),
    ).toBe('r:c167:run0');
    expect(() => spriteKeyForMatchPlayer(24, 'player', 'MID', 'run0')).toThrow(
      '0 to 23',
    );
    expect(() => spriteKeyForMatchPlayer(1, 'player', 'MID', 'ready0')).toThrow(
      'only goalkeepers',
    );
  });
  ...
  it('loads every base and derived action frame', () => {
    ...
    for (const key of [...BASE_KEYS, ...SLIDE_KEYS])
      expect(sheet.sprites).toHaveProperty(key);
    const webbedKeys = BASE_KEYS.filter((key) => key !== 'ball').map(
      webbedSpriteKey,
    );
    for (const key of webbedKeys) expect(sheet.sprites).toHaveProperty(key);
    expect(Object.keys(sheet.sprites)).toHaveLength(
      BASE_KEYS.length + SLIDE_KEYS.length + webbedKeys.length,
    );
  });

  it('builds a match-sized sheet from only the active visual identities', () => {
    const sheet = loadSpriteSheet(['r:f00', 'u:g01', 'r:c167', 'r:f00']);
    const ordinaryKeys = Object.keys(sheet.sprites).filter(
      (key) => !key.endsWith(':webbed'),
    );
    const webbedKeys = Object.keys(sheet.sprites).filter((key) =>
      key.endsWith(':webbed'),
    );
    expect(ordinaryKeys).toHaveLength(47);
    expect(webbedKeys).toHaveLength(16);
    ...
    expect(sheet.sprites).toHaveProperty(webbedSpriteKey('r:c167:back0'));
  ...
  it('keeps hard-pixel dimensions and declared palette tokens', () => {
    ...
    for (const [key, rows] of Object.entries(sheet.sprites)) {
      ...
      expect(rows).toHaveLength(
        ball ? 6 : slide ? SLIDE_TACKLE_CELL.h : sheet.cell.h,
      );
      for (const row of rows) {
        expect(row).toHaveLength(
          ball ? 6 : slide ? SLIDE_TACKLE_CELL.w : sheet.cell.w,
        );
  ...
  it('gives every goalkeeper two distinct ready poses', () => {
    ...
    for (const side of SIDES)
      for (const id of manifest.goalkeeper) {
        const poses = ['run0', 'ready0', 'ready1'].map(
          (frame) => sheet.sprites[`${side}:${id}:${frame}`],
        );
  ...
  describe('atlas layout', () => {
    it('uses an 8-column grid with gutters and unique source origins', () => {
      ...
      expect(layout.rows).toBe(
        Math.ceil(Object.keys(sheet.sprites).length / 8),
      );
    ...
    it('is deterministic across repeated calls', () => {
      const sheet = fullSheet;
      expect(atlasLayout(sheet).rectFor('r:f08:run0')).toEqual(
        atlasLayout(sheet).rectFor('r:f08:run0'),
      );
    });
  });
  ...
  it('preserves named-star silhouette signatures', () => {
    ...
    expect(
      sheet.sprites['r:f08:run0'].some((row) => row.includes(fireToken!)),
    ).toBe(true);
    const countPainted = (row: string) =>
      [...row].filter((token) => token !== '.').length;
    expect(countPainted(sheet.sprites['u:f12:run0'][16])).toBeGreaterThan(
      countPainted(sheet.sprites['u:f10:run0'][16]),
    );
  });
});
