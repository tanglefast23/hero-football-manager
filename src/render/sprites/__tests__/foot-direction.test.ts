 ...
 * 1,016 looks trustworthy. Keeper ready poses are a planted stance, not a stride,
...
const runKeys = Object.keys(sheet.sprites).filter((key) =>
  /:run[01]$/.test(key),
);
const readyKeys = Object.keys(sheet.sprites).filter((key) =>
  /:ready[01]$/.test(key),
);
const lookIds = [
  ...new Set(runKeys.map((key) => key.replace(/:run[01]$/, ''))),
];
...
function bootGroups(rows: readonly string[]): number[][] {
  ...
  for (let column = 0; column < sheet.cell.w; column += 1) {
    ...
    if (previous !== undefined && previous[previous.length - 1] === column - 1)
...
const paintedColumns = (rows: readonly string[]) =>
  [
    ...new Set(
      rows.flatMap((row) =>
        [...row].flatMap((token, column) => (token === '.' ? [] : [column])),
      ),
    ),
  ].sort();
...
describe('vertical-pitch run cycle feet', () => {
  it('covers every look on the sheet', () => {
    expect(lookIds).toHaveLength(1016);
    expect(runKeys).toHaveLength(2032);
  ...
  it('keeps every run frame on the declared grid', () => {
    ...
    for (const key of runKeys) {
      ...
      if (rows.length !== sheet.cell.h)
        faults.push(`${key}: ${rows.length} rows`);
      rows.forEach((row, index) => {
        checkedRows += 1;
        if (row.length !== sheet.cell.w)
          faults.push(`${key} row ${index}: width ${row.length}`);
        for (const token of row) {
          if (!(token in sheet.palette))
            faults.push(`${key} row ${index}: token "${token}"`);
        }
      });
    }
  ...
  it('points the toes at the camera: no boot pixel overhangs its own shin', () => {
    ...
    const overhangs = runKeys.flatMap((key) =>
      band(sheet.sprites[key]).flatMap((row, index) =>
        bootColumns(row)
          .filter((column) => !allowed.has(column))
          .map((column) => `${key} row ${index} col ${column}`),
      ),
    );
  ...
  it('strides along the pitch axis: one foot nearer, roles swapping per frame', () => {
    for (const id of lookIds) {
      ...
      expect({ id, planted: near0 > far0, present: far0 >= 0 }).toEqual({
        id,
        planted: true,
        present: true,
      });
      expect({ id, planted: near1 > far1, present: far1 >= 0 }).toEqual({
        id,
        planted: true,
        present: true,
      });
  ...
  it('no longer sways the legs sideways between frames', () => {
    for (const id of lookIds) {
      expect({
        id,
        columns: paintedColumns(band(sheet.sprites[`${id}:run0`])),
      }).toEqual({
        id,
        columns: paintedColumns(band(sheet.sprites[`${id}:run1`])),
      });
    }
  });
  ...
  it('keeps each look its own skin and boot tokens', () => {
    ...
    const shinPairs = new Set(
      lookIds.map((id) => sheet.sprites[`${id}:run0`][27].slice(9, 11)),
    );
    expect(shinPairs.size).toBeGreaterThan(4);
    for (const id of lookIds) {
      const run0 = sheet.sprites[`${id}:run0`];
      const run1 = sheet.sprites[`${id}:run1`];
      expect(run1[27].slice(13, 15)).toBe(
        [...run0[27].slice(9, 11)].reverse().join(''),
      );
  ...
  it('turns the near boot sole-side out in every derived rear view', () => {
    for (const [frame, near, far] of [
      ...
      for (const id of lookIds) {
        ...
        const back = band(
          deriveBackFacingFrame(sheet.sprites[`${id}:${frame}`]),
        );
        expect({
          id,
          frame,
          sole: back.some((row) =>
            near.some((column) => 'Gg'.includes(row[column])),
          ),
        ...
        expect({
          ...
          frame,
          heel: back.every((row) =>
            far.every((column) => !'Gg'.includes(row[column])),
          ),
  ...
  it('leaves keeper ready poses level and untouched by the rear-sole swap', () => {
    for (const key of readyKeys) {
      ...
      const bottoms = bootGroups(bootBand).map((group) =>
        bootBottom(bootBand, group),
      );
  ...
  it('carries the reworked feet through the real load pipeline', () => {
    ...
    expect(band(loaded.sprites['r:f00:back0'])).not.toEqual(
      band(loaded.sprites['r:f00:run0']),
    );
    expect(band(loaded.sprites['r:f00:back0']).join('')).toContain('G');
    expect(band(loaded.sprites['r:g00:backReady0'])).toEqual(
      band(loaded.sprites['r:g00:ready0']),
    );
  });
});
