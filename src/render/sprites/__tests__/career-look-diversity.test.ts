...
describe('career roster visual diversity', () => {
  ...
  it('persists every assigned face through a deep-career save and reload', () => {
    ...
    expect(reloaded.players.map(player => [player.id, assignedLook(player)]))
      .toEqual(career.players.map(player => [player.id, assignedLook(player)]));
  });
  ...
  it('does not let newly promoted opponents displace surviving user faces', () => {
    for (const seed of [20260721, 20260722]) {
      ...
      for (const player of seasonSix.players.filter(player => (
        player.clubId === seasonSix.userClubId && seasonSevenLooks.has(player.id)
      ))) {
