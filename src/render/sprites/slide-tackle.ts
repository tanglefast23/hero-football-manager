...
export const SLIDE_TACKLE_HEAD_ANGLES = [6, 16, 26, 50, 72, 78, 44, 32, 22, 6] as const;
...
function emptyGrid(width: number = SLIDE_TACKLE_CELL.w, height: number = SLIDE_TACKLE_CELL.h): Grid {
...
function drawRect(grid: Grid, left: number, top: number, right: number, bottom: number, token: string): void {
...
function drawPolygon(grid: Grid, points: readonly Point[], token: string): void {
  ...
  for (let y = minY; y <= maxY; y += 1) {
    for (let x = minX; x <= maxX; x += 1) {
      if (pointInPolygon(x + 0.5, y + 0.5, points) || pointInPolygon(x, y, points)) {
...
function paste(target: Grid, source: Grid, offsetX: number, offsetY: number): void {
  for (let y = 0; y < source.length; y += 1) {
    for (let x = 0; x < source[y].length; x += 1) {
      const token = source[y][x];
      if (token !== TRANSPARENT) setPixel(target, x + offsetX, y + offsetY, token);
    }
  }
}

function paintedBounds(grid: Grid): { left: number; top: number; right: number; bottom: number } | null {
...
function rotateNearest(source: Grid, angleDegrees: number): Grid {
  ...
  const outputWidth = Math.ceil(Math.abs(sourceWidth * cosine) + Math.abs(sourceHeight * sine));
  const outputHeight = Math.ceil(Math.abs(sourceWidth * sine) + Math.abs(sourceHeight * cosine));
  ...
  for (let y = 0; y < outputHeight; y += 1) {
    for (let x = 0; x < outputWidth; x += 1) {
      ...
      if (sourceY < 0 || sourceY >= sourceHeight || sourceX < 0 || sourceX >= sourceWidth) continue;
...
function placeHead(body: Grid, head: Grid, angle: number, centerX: number, centerY: number): Grid {
...
function mostCommonToken(source: Grid, candidates: readonly string[]): string {
  ...
  return [...counts.entries()].sort((a, b) => b[1] - a[1])[0]?.[0] ?? candidates[0];
}
...
function kitTokens(source: Grid): { base: string; shadow: string; light: string } {
  const redCount = source.flat().filter(token => token === 'R' || token === 'r' || token === 'E').length;
  const blueCount = source.flat().filter(token => token === 'B' || token === 'b' || token === 'C').length;
...
function uprightBody(source: Grid): Grid {
  ...
  paste(result, source.slice(15, 30).map(row => [...row]), 5, 15);
  return result;
}
...
function kneelBody(source: Grid, high: boolean): Grid {
  ...
  drawPolygon(result, [[11, hipY], [17, hipY], [18, 26], [16, 28], [11, 27], [10, 23]], 'K');
  drawRect(result, 12, hipY + 1, 16, 25, kit.shadow);
  drawRect(result, 12, 25, 16, 27, skin.base);
  drawPolygon(result, [[10, 27], [18, 27], [19, 30], [10, 30]], 'K');
  drawRect(result, 12, 28, 17, 29, 'W');

  drawPolygon(result, [[16, hipY], [23, hipY + 1], [25, 23], [reach, 23], [reach + 1, 27], [22, 27], [17, 24]], 'K');
  drawPolygon(result, [[17, hipY + 1], [22, hipY + 2], [23, 24], [18, 23]], kit.shadow);
...
function slideBody(source: Grid): Grid {
  ...
  drawPolygon(result, [[20, 18], [28, 18], [29, 21], [24, 23], [19, 21]], 'K');
  drawPolygon(result, [[21, 18], [27, 19], [23, 21], [20, 20]], kit.shadow);
  drawPolygon(result, [[26, 20], [33, 20], [33, 24], [27, 24], [24, 22]], 'K');
  drawRect(result, 28, 21, 32, 22, 'W');
  setPixel(result, 28, 23, 'w');

  drawPolygon(result, [[17, 20], [25, 22], [24, 25], [16, 23]], 'K');
  drawPolygon(result, [[18, 21], [24, 23], [20, 24], [17, 22]], kit.base);
  drawPolygon(result, [[23, 24], [30, 24], [31, 27], [23, 27]], 'K');
  drawRect(result, 25, 25, 29, 26, 'W');

  drawPolygon(result, [[9, 16], [6, 20], [5, 24], [8, 25], [10, 20], [12, 18]], 'K');
  drawPolygon(result, [[9, 17], [7, 20], [7, 23], [8, 23], [10, 19], [12, 18]], kit.light);
...
function deriveFrames(sourceRows: string[]): string[][] {
  ...
  const centers: readonly Point[] = [
    [17, 8], [14, 9], [11, 10], [11, 11], [11, 13],
    [11, 13], [11, 11], [14, 9], [15, 9], [17, 8],
  ];
  return bodies.map((body, index) => gridToRows(placeHead(
    body,
    head,
...
export function slideTackleSpriteFrame(index: number): SlideTackleSpriteFrame {
  const clamped = Math.max(0, Math.min(SLIDE_TACKLE_FRAME_COUNT - 1, Math.floor(index))) as SlideTackleFrameIndex;
 ...
 */
export function withSlideTackleSprites<T extends DerivableSpriteSheet>(sheet: T): T {
