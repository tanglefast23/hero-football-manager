...
export function CupTitleCard({ card, onDone }: CupTitleCardProps) {
  ...
  return (
    <Pressable
      ...
      accessibilityLabel={t('matchScreen.a11y.cupTitleCard', {
        label: card.accessibilityLabel,
      })}
      onPress={finishOnce}
    ...
    >
              ...
              transform: [
                ...
                },
                {
                  rotate: flight.interpolate({
                    inputRange: BALL_FRAMES,
                    outputRange: BALL_PATH.map(
                      (point) => `${Math.round(point.spin)}deg`,
                    ),
                  }),
                },
              ],
      ...
      <View style={styles.plaque}>
        ...
        <Text
          style={[styles.title, narrow ? styles.titleNarrow : null]}
          numberOfLines={1}
        >
          {card.title}
        </Text>
