...
import { DESK_STORY_ALERT_ID, homeViewModel, isHomeDeskClear } from '../view-models';
...
describe('stories on the desk', () => {
  ...
  it('is never desk-clear outside the manage phase', () => {
    expect(isHomeDeskClear({ ...deskClearCareer(20261002), week: 9, phase: 'matchday' })).toBe(false);
  });

  it('puts the offered story on the desk with its whole opening on the card', () => {
    const offered = offerCareerEvent({ ...deskClearCareer(20261003), week: 9 }, spider.id);
  ...
  it('outranks the standing keep-building nudge', () => {
    ...
    expect(homeViewModel(clear).alerts.map(alert => alert.id)).toEqual(['build-reminder']);

    const offered = offerCareerEvent(clear, spider.id);

    expect(homeViewModel(offered).alerts.map(alert => alert.id)).toEqual([DESK_STORY_ALERT_ID]);
  });

  it('leaves the desk once the story has been answered', () => {
    const offered = offerCareerEvent({ ...deskClearCareer(20261005), week: 9 }, spider.id);
    const resolved = {
      ...offered,
