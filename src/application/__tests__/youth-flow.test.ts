...
describe('M2 youth intake store flow', () => {
  ...
  test('reveals the Week 2 choice, signs one prospect, and preserves the scout slot', () => {
    ...
    expect(beforeReveal.youthIntake).toMatchObject({
      status: 'CLOSED',
      offers: [],
    });
    useM1Store.setState({
    ...
    const cashBefore = useM1Store
      .getState()
      .career!.clubs.find((club) => club.id === initial.userClubId)!.cash;

    useM1Store.getState().signYouth(offer.player.id);

    const signed = useM1Store.getState().career!;
    expect(
      signed.players.find((player) => player.id === offer.player.id),
    ).toEqual(offer.player);
    expect(
      signed.clubs.find((club) => club.id === signed.userClubId)?.cash,
    ...
    expect(
      signed.players.filter((player) => player.clubId === signed.userClubId),
    ).toHaveLength(16);
