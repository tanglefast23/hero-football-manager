...
describe('keep-building inbox nudge', () => {
    ...
    expect(home.alerts).toEqual([
      {
        ...
        title: 'Keep building',
        detail:
          'Remember to construct new buildings every week if you have the finances to do so.',
        tone: 'info',
      },
    ]);
  });
  ...
  it('stays quiet while the works crew is already on a project', () => {
    ...
    const building = buildCareerFacility(clear, 'coaching-office', {
      x: 5,
      y: 4,
    }).state;
  ...
  it('never crowds out a real inbox item', () => {
    ...
    expect(
      home.alerts.some((alert) => alert.id === `injury-${injuredId}`),
    ).toBe(true);
    expect(home.alerts.some((alert) => alert.id === BUILD_REMINDER_ID)).toBe(
      false,
    );
  });
  ...
  it('does not repeat in a later season', () => {
    ...
    expect(homeViewModel({ ...clear, season: 2, week: 7 }).alerts).toHaveLength(
      0,
    );
  });
});
