...
const view = (negotiation: ContractNegotiation) =>
  marketNegotiationViewModel({
    state: negotiation,
    playerName: 'Jobo',
...
describe('the wage the panel quotes', () => {
    ...
    for (const term of [1, 2, 3]) {
      for (const perk of [
        ...
        expect({
          term,
          perk,
          quoted: quotes[offerQuoteKey(term, perk)] !== undefined,
  ...
  it('goes down as the manager gives more away', () => {
    const quotes = view(opened()).requiredWeeklyWageByOffer;
    expect(quotes[offerQuoteKey(3, 'GUARANTEED_STARTER')]!).toBeLessThan(
      quotes[offerQuoteKey(1, 'JERSEY_10')]!,
    );
  });
...
describe('the agent s closing ultimatum', () => {
  ...
  it('says the same thing every time the panel re-renders', () => {
    const negotiation = toFinalRound();
    expect(view(negotiation).finalDemand!.line).toBe(
      view(negotiation).finalDemand!.line,
    );
  });
