...
describe('M2 struggling-manager recovery soak', () => {
  it('survives six losing seasons, cold relaunches, and repeated board rescues deterministically', () => {
    ...
    expect(
      first.resolutions.some((resolution) => resolution.kind === 'FORCED_SALE'),
    ).toBe(true);
    expect(
      buildCareerTeamDef(first.state, first.state.userClubId).players,
    ).toHaveLength(11);
    expect(
      first.state.players.filter(
        (player) => player.clubId === first.state.userClubId,
      ),
    ).toHaveLength(16);
  });
});
...
function runStrugglingManagerCareer(seed: number): {
  ...
  let state = coldRelaunch(createCareer(createLaunchCareerSetup(seed)));
  let transitions = 0;
  ...
  while (!(
    state.phase === 'season-end' && state.season === COMPLETED_SEASONS
  )) {
    transitions += 1;
    ...
    if (state.phase === 'manage') {
      ...
      if (
        ...
        const hero = ultimatum.candidates.find(
          (candidate) =>
            state.players.find((player) => player.id === candidate.playerId)
              ?.power !== undefined,
        );
      ...
      if (matchday === undefined)
        throw new Error('soak entered matchday without a fixture');
    ...
    if (resolution !== undefined) {
      ...
      if (firstSeen && resolution.kind === 'FORCED_SALE') {
        expect(resolution.playerId).not.toBe(
          protectedByUltimatum.get(resolution.id),
        );
        expect(
          state.players.find(
