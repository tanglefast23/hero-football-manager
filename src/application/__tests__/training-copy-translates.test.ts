...
describe('training copy reaches a non-English player', () => {
  test('drill shop rows name the path in the player s language', () => {
    ...
    const english = squadTrainingViewModel(
      state,
      content,
      undefined,
    ).drillUpgrades;
  ...
  test('a locked drill tier explains itself in German, division name included', () => {
    ...
    expect(german.blockedReason).toBe(
      'Übungen Stufe 2 gibt es ab D4 · Kreisliga.',
    );
  });
  ...
  test('an unaffordable upgrade says so in German', () => {
    ...
    expect(
      squadTrainingViewModel(broke, content, undefined, de).drillUpgrades[0]
        .blockedReason,
  ...
  test('a facility level held back by the division explains itself in German', () => {
    ...
    const atLevelTwo = {
      ...
      facilities: {
        ...
        grid: {
          ...
          buildings: gym.facilities.grid!.buildings.map((building) => ({
            ...building,
            level: 2 as const,
          })),
        },
    ...
    expect(facilityUpgradeBlockedReason(atLevelTwo, 3)).toMatchObject({
      ...
      textParams: {
        level: 3,
        divisionLevel: 2,
        divisionNameKey: 'division.national',
      },
    });
    const building = clubFinancesViewModel(atLevelTwo, de).facilities
      .buildings[0];
    expect(building.upgradeBlockedReason).toBe(
      'Anlagen Stufe 3 gibt es ab D2 · Nationalliga.',
    );
    ...
    for (const locale of ENABLED_LOCALES) {
      ...
      expect({ locale, division: drawn.upgradeBlockedDivision }).toEqual({
        locale,
        division: 2,
      });
      ...
      expect({
        locale,
        rung: drawn.upgradeBlockedReason?.match(/D[1-5]/)?.[0],
