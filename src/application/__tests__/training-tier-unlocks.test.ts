...
describe('training drill tier purchases', () => {
  ...
  test('shows the OWNED tier for a stat option, not the best the division allows', () => {
    const divisionFive = createCareer(
      createLaunchCareerSetup(20260722, undefined, content),
    );
    const player = divisionFive.players.find(
      (candidate) => candidate.clubId === divisionFive.userClubId,
    )!;
    const optionAt = (highestDivisionReached: 5 | 4 | 2, ownedTier?: number) =>
      squadTrainingViewModel(
        {
          ...
          ...(ownedTier === undefined
            ? {}
            : { ownedTrainingTiers: { sprints: ownedTier } }),
        },
        content,
        player.id,
  ...
  it('offers every path its own next tier, priced, with the reason it is unavailable', () => {
    ...
    const rich = {
      ...
      clubs: state.clubs.map((club) =>
        club.id === state.userClubId ? { ...club, cash: 100_000 } : club,
      ),
    };

    const atDivisionFive = squadTrainingViewModel(
      rich,
      content,
      undefined,
    ).drillUpgrades;
    ...
    const promoted = {
      ...rich,
      m2: { ...rich.m2!, highestDivisionReached: 4 as const },
    };
    expect(
      squadTrainingViewModel(promoted, content, undefined).drillUpgrades[0]
        .blockedReason,
    ).toBeUndefined();
    ...
    expect(
      squadTrainingViewModel(broke, content, undefined).drillUpgrades[0]
        .blockedReason,
  ...
  it('buys one path at a time, charging the club and leaving the others alone', () => {
    ...
    expect(
      bought.clubs.find((club) => club.id === bought.userClubId)?.cash,
    ).toBe(0);
    ...
    expect(() => purchaseCareerTrainingUpgrade(bought, 'sprints')).toThrow(
      'Tier 3 drills unlock in D3 · Regional League.',
    );
    expect(() =>
      purchaseCareerTrainingUpgrade(bought, 'keeper-drills'),
