...
describe('completeFaceOff', () => {
  ...
  it('honours an awakening held behind the scene', () => {
    useM1Store.setState({ screen: 'faceoff', pendingPostFaceOffScreen: 'awakening' });
    useM1Store.getState().completeFaceOff();
  ...
  it('is harmless when called twice', () => {
    useM1Store.setState({ screen: 'faceoff', pendingPostFaceOffScreen: 'postmatch' });
    useM1Store.getState().completeFaceOff();
    useM1Store.getState().completeFaceOff();
    expect(useM1Store.getState().screen).toBe('postmatch');
  });
  ...
  it('mutates nothing at all when the face-off is not the screen', () => {
    useM1Store.setState({ screen: 'management', pendingPostFaceOffScreen: 'postmatch' });
    useM1Store.getState().completeFaceOff();
...
describe('quickResult raises the face-off', () => {
   ...
   */
  const source = () => readFileSync(join(process.cwd(), 'src/application/store.ts'), 'utf8');

  it('builds the scene from the pre-settlement capture, never by re-entering currentMatchday', () => {
    const quickResult = source().split('  quickResult(')[1].split('\n  watchMatch(')[0];

    expect(quickResult).toContain('quickResultFaceOffViewModel({');
    // The capture destructured at the top of the call — the only teams that
    // belong to the fixture just played.
    expect(quickResult).toContain('const { kind, fixture, fixtures, teams } = currentMatchday(before);');
    ...
    expect(quickResult).not.toContain('currentMatchday(next)');
    expect(quickResult).not.toContain('currentMatchday(after)');
    expect(quickResult).not.toContain('currentMatchday(completed)');
  });

  it('reads the verdict, not the scoreline, so a penalty shoot-out is never a draw', () => {
    const quickResult = source().split('  quickResult(')[1].split('\n  watchMatch(')[0];
    expect(quickResult).toContain('outcomeLabel: postMatch.result.outcomeLabel');
  });

  it('holds the screen the result would have opened, and skips the scene if it cannot be built', () => {
    const quickResult = source().split('  quickResult(')[1].split('\n  watchMatch(')[0];
    expect(quickResult).toContain("screen: faceOff === null ? destination : 'faceoff'");
    expect(quickResult).toContain('pendingPostFaceOffScreen: faceOff === null ? null : destination');
    // The awakening still comes before the ledger; the face-off only ever sits
    // ahead of that chain, never inside it.
    expect(quickResult).toContain("const destination = awakening.awakened ? 'awakening' as const : 'postmatch' as const;");
  });
});
...
describe('ephemeral screen state is never inherited across careers', () => {
  ...
  it('pairs every postMatch reset with the face-off and match-day banner', () => {
    ...
    for (const reset of resets) {
      ...
      expect(`${where} ${block.includes('faceOff: null,')}`).toBe(`${where} true`);
      expect(`${where} ${block.includes('pendingPostFaceOffScreen: null,')}`).toBe(`${where} true`);
      expect(`${where} ${block.includes('matchDayBanner: null,')}`).toBe(`${where} true`);
    }
  });
});
