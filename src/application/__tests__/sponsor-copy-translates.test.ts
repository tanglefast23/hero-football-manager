...
function withSponsorship(state: GameState, sponsorship: SponsorshipState): GameState {
...
describe('the sponsor desk reads in the player s language', () => {
  test('offer lines, targets and profile chips all resolve', () => {
    ...
    cards.forEach((card, index) => {
      ...
      expect({ id: offer.sponsorContentId, name: card.sponsorName })
        .toEqual({ id: offer.sponsorContentId, name: offer.sponsorName });
      expect(card.offerLine)
        .toBe(german(`sponsor.brand.${offer.sponsorContentId}.offerLine`));
      expect(card.offerLine).not.toBe(offer.offerLine);
      expect(card.objectiveLabel).toBe(
        german(`sponsor.objective.${objectiveIdOf(offer.objective.kind)}.label`)
    ...
    expect(cards.map(card => card.profileLabel)).toEqual(offers.map(offer => german(
      `clubFinances.sponsorProfile${offer.profile[0]}${offer.profile.slice(1).toLowerCase()}`,
    )));
    expect(new Set(cards.map(card => card.profileLabel)))
      .toEqual(new Set(['Solide', 'Ausgewogen', 'Mutig']));
  });
  ...
  test('the continuity placeholder is chrome, not a brand', () => {
    ...
    expect(slots.map(slot => slot.sponsorName))
      .toEqual(['Aktueller Sponsor 1', 'Aktueller Sponsor 2']);
    ...
    expect(state.clubBusiness.sponsorship.activeContracts.map(contract => contract.sponsorName))
  ...
  test('a single-slot club drops the number', () => {
    ...
    const single = withSponsorship(state, {
      ...
      offers: state.clubBusiness.sponsorship.offers.filter(offer => offer.slot === first!.slot),
    });

    expect(clubFinancesViewModel(single, de).sponsorship!.slots[0]!.sponsorName)
  ...
  test('a signed deal keeps its target translated all season', () => {
    ...
    const contract = signed.clubBusiness.sponsorship.activeContracts
      .find(candidate => !candidate.provisional)!;

    expect(contract.objective?.labelKey).toBe('sponsor.objective.league-wins.label');
    expect(contract.objective?.labelParams).toEqual({ target: contract.objective!.target });

    const slot = clubFinancesViewModel(signed, de).sponsorship!.slots
      .find(candidate => !candidate.provisional)!;
    expect(slot.objectiveLabel).toBe(
      german('sponsor.objective.league-wins.label')
        .replace('{target}', String(contract.objective!.target)),
    );
...
describe('what a save written before any of this still shows', () => {
  ...
  test('an objective with no labelKey falls back to the English in the save', () => {
    ...
    const old = clubFinancesViewModel(restored, de).sponsorship!.slots
      .find(candidate => !candidate.provisional)!;
    const current = clubFinancesViewModel(signed, de).sponsorship!.slots
      .find(candidate => !candidate.provisional)!;

    const englishInTheSave = restored.clubBusiness.sponsorship.activeContracts
      .find(candidate => !candidate.provisional)!.objective!.label;
    ...
    expect(old.offerLine).not.toBe(restored.clubBusiness.sponsorship.activeContracts
      .find(candidate => !candidate.provisional)!.offerLine);
  });
  ...
  test('a brand the shipped content no longer has renders its persisted line', () => {
    ...
    const slot = clubFinancesViewModel(withRetiredBrand, de).sponsorship!.slots[0]!;
...
function objectiveIdOf(kind: 'LEAGUE_WINS' | 'LEAGUE_GOALS' | 'LEAGUE_FINISH'): string {
  const definition = content.sponsors.objectives.find(candidate => candidate.kind === kind);
  expect({ kind, found: definition !== undefined }).toEqual({ kind, found: true });
  return definition!.id;
}

function signFirstOffer(state: GameState): GameState {
  const offer = state.clubBusiness.sponsorship.offers
    .find(candidate => candidate.objective.kind === 'LEAGUE_WINS')!;
  return withSponsorship(state, acceptSponsorOffer(state.clubBusiness.sponsorship, {
