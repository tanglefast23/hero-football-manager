...
describe('open-ended potential in the Squad desk', () => {
  test('shows role-aware current ratings and exact training-speed grades', () => {
    ...
    const archetypeByPlayerId = new Map(
      PLAYER_ARCHETYPES.map(
        (archetype, index) => [roster[index].id, archetype] as const,
      ),
    );
    const state = {
      ...initial,
    ...
    for (const [playerId, archetype] of archetypeByPlayerId) {
      ...
      expect(player.overall).toBe(
        roleOverall(careerPlayer.role, careerPlayer.attrs),
      );
      ...
      expect(player.superChancePercent).toBe(
        superTrainingChancePercent(playerPotentialGrade(careerPlayer)),
      );
    }
  });
});
