...
export function effectiveStat(state: MatchState, idx: number, stat: keyof Attrs): number {
  const p = state.players[idx];
  const conditionScale = 1 - 0.25 * (1 - clamp(p.condition, 0, 100) / 100);
  return Math.round(p.def.attrs[stat] * conditionScale);
}
...
export function drainStamina(p: SimPlayer, movedFar: boolean): void {
  ...
  p.condition = clamp(p.condition - (movedFar ? 0.02 : 0.005) * drainMultiplier, 0, 100);
}
