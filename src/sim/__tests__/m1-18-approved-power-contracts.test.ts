...
describe('m1.18 approved power contracts', () => {
  it('keeps the authored next-tick wind-ups synchronized with the simulator', () => {
    const windups = Object.fromEntries(
      loadLaunchContent().powers.powers.map((power) => [
        power.id,
        power.windupTicks,
      ]),
    );
    ...
    for (const [power, slot] of [
      ...
      powerTick(match);
      expect(match.players[slot].powerState).toMatchObject({
        kind: 'winding',
        untilTick: 1,
      });
    }
  ...
  it('Ice keeps carrier and ball together while sliding toward the carrier own goal', () => {
    ...
    expect(
      match.events.some(
        (event) => event.kind === 'PASS' || event.kind === 'SHOT',
      ),
    ).toBe(false);
  });
  ...
  it('Rally queues one refill for a ready hero and never grants a fifth Zone', () => {
    ...
    match.players[recipient].powerState = {
      kind: 'active',
      untilTick: 10,
      strength: 1,
  ...
  it('Decoy creates a separate forward receiver that receives without replacing the original', () => {
    ...
    for (let idx = 11; idx < 22; idx += 1)
      if (idx !== marker) match.players[idx].pos = { x: 6500, y: 9000 };
    ...
    expect(match.ball).toMatchObject({
      kind: 'pass',
      to: 22,
      decoyReceiverPlayerId: match.decoyClones[0]!.def.id,
    });
    forceArrival(match);
  ...
  it('Decoy fails soft with no forward and cancels if its captured forward is substituted', () => {
    ...
    expect(noForward.events.some((event) => event.kind === 'POWER_FIRED')).toBe(
      false,
    );
  ...
  it('Future steals then immediately guarantees the furthest-forward onside outlet', () => {
    ...
    for (let idx = 11; idx < 22; idx += 1)
      match.players[idx].pos = { x: 2400, y: 2500 };
    ...
    expect(match.players[2].powerState).toMatchObject({
  ...
  it('Portal protects the receiver until their first pass or shot', () => {
    ...
    match.players[12].pos = {
      x: match.players[receiver].pos.x + 50,
      y: match.players[receiver].pos.y,
    };
    match.tick = 19;
    powerTick(match);
    tackleTick(match);
  ...
  it('Portal remembers its valid receiver and exit through the full windup', () => {
    ...
    expect(
      match.events.filter(
        (event) =>
          event.kind === 'POWER_FIRED' && event.power === 'PORTAL_PASS',
      ),
    ).toHaveLength(1);
    expect(
      match.events.filter(
        (event) =>
          event.kind === 'POWER_IMPACT' && event.power === 'PORTAL_PASS',
      ),
    ).toHaveLength(1);
  });
  ...
  it('Gravity clears defenders to a safe rim and commits a runner into the abandoned lane', () => {
    ...
    if (
      winding.kind !== 'winding' ||
      winding.runnerIdx === undefined ||
      winding.runnerAnchor === undefined
    )
      throw new Error('expected runner capture');
    ...
    expect(
      Math.hypot(
        match.players[runner].pos.x - runnerAnchor.x,
        match.players[runner].pos.y - runnerAnchor.y,
      ),
    ).toBeLessThan(
      Math.hypot(before.x - runnerAnchor.x, before.y - runnerAnchor.y),
    );
    tackleTick(match);
    ...
    expect(attackingDecision(match, carrier)).toMatchObject({
      kind: 'pass',
      to: runner,
    });
    match.tick = 5;
    ...
    expect(
      ...
      Math.hypot(inFlight.x - runnerAnchor.x, inFlight.y - runnerAnchor.y),
    );
  ...
  it('lets a planted Gravity well follow the current lane when the carrier shifts formation', () => {
    ...
    if (winding.kind !== 'winding')
      throw new Error('expected planted Gravity well');
    expect(winding.untilTick - match.tick).toBe(1);
  ...
  it.each([
    ...
    (tier, expected) => {
      ...
      expect(
        match.events.filter((event) => event.kind === 'IGNITED'),
      ).toHaveLength(expected);
      expect(
        match.events
          .filter((event) => event.kind === 'IGNITED')
          .every(
            (event) =>
              event.kind === 'IGNITED' &&
              match.players[event.player].def.role === 'DEF',
          ),
      ).toBe(true);
    },
  );
  ...
  it('Gust guarantees the redirect and open-forward punt while grade improves its landing', () => {
    ...
    expect(match.ball).toMatchObject({
      ...
      gustGrade: 1.15,
      willSucceed: true,
      arrivalPos: manualLanding!.pos,
    });
    expect(match.events).toContainEqual(
      expect.objectContaining({ kind: 'GUST_REDIRECT', player: 2 }),
    );
    expect(match.events).toContainEqual(
      expect.objectContaining({
        kind: 'GUST_PUNT',
