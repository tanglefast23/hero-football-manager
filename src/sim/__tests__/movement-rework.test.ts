...
import { CORRELATION_SEEDS, meanOffBallYVelocityCorrelation } from './helpers/y-correlation';
...
describe('formation tables: validity', () => {
  it('has 10 slots x 35 cells x 2 phases of in-bounds fractions, and a 10-entry kickoff list', () => {
    ...
    for (const phase of [tables.phases.inPossession, tables.phases.outOfPossession]) {
    ...
    for (const [name, phase] of Object.entries(tables.phases)) {
      for (const row of [5, 6]) {
        for (let col = 0; col < GRID_COLS; col++) {
          ...
          const meanY = (slots: number[]) => slots.reduce((a, s) => a + phase[s][cell][1], 0) / slots.length;
          const def = meanY([0, 1, 2, 3]);
          const mid = meanY([4, 5, 6, 7]);
          if (!(def > mid)) throw new Error(`coherence: ${name} cell (${col},${row}) def ${def} !> mid ${mid}`);
        }
      }
    }
  });
...
describe('sampling convention', () => {
  ...
  for (const x of [0, 137, 1700, 3400, 5100, 6663, 6800]) {
    for (const y of [0, 251, 2625, 5250, 7875, 10249, 10500]) BALLS.push({ x, y });
  }
    ...
    for (const inPoss of [true, false]) {
      for (let slot = 1; slot <= 10; slot++) {
        for (const ball of BALLS) {
          expect(tableTarget(1, slot, inPoss, ball)).toEqual(rot(tableTarget(0, slot, inPoss, rot(ball))));
        }
      }
    }
  });
  ...
  it('cell-center convention: a ball at a cell center samples that cell entry exactly', () => {
    ...
    for (const [col, row] of [[0, 0], [2, 3], [4, 6], [1, 5]]) {
      const ball = { x: (col + 0.5) * cellW, y: (row + 0.5) * cellH };
      for (let slot = 1; slot <= 10; slot++) {
        const entry = tables.phases.outOfPossession[slot - 1][row * GRID_COLS + col];
...
describe('asymmetry (ball far left)', () => {
  ...
  it('near-side x-response is much larger than far-side, both phases', () => {
    for (const [inPoss, minRatio] of [[false, 1.5], [true, 1.4]] as Array<[boolean, number]>) {
      const resp = (slot: number) =>
        Math.abs(tableTarget(0, slot, inPoss, left).x - tableTarget(0, slot, inPoss, center).x);
  ...
  it('the far winger tucks inward, both phases', () => {
    for (const inPoss of [false, true]) {
      expect(tableTarget(0, 8, inPoss, left).x).toBeLessThan(tableTarget(0, 8, inPoss, center).x);
    }
  });
});
...
describe('possession phases', () => {
  it('same cell, opposite phases: materially different shapes (measured 12.4k-14.8k cm L1 across probes)', () => {
    for (const ball of [{ x: 3400, y: 5250 }, { x: 3400, y: 8250 }, { x: 1700, y: 2250 }]) {
      let l1 = 0;
      for (let slot = 1; slot <= 10; slot++) {
  ...
  it('turnover blend: targets travel the old->new phase segment smoothly and settle within the staggered window', () => {
    ...
    for (let k = 1; k <= MAX_WINDOW + 2; k++) {
      ...
      watched.forEach((slot, i) => {
        ...
        expect(dist(cur[slot], to[i])).toBeLessThanOrEqual(dist(prev[slot], to[i]) + 2);
      });
      prev = cur.slice();
    }
...
describe('spacing and support pressure', () => {
  it('role-band spacing holds; crowding and support pressure reported as metrics', () => {
    ...
    for (let seed = 1; seed <= 5; seed++) {
      ...
      while (m.phase !== 'fulltime') {
          ...
          for (const [line, slots] of Object.entries(LINES)) {
            for (const s of slots) {
              ...
              for (let j = base + 1; j < base + 11; j++) {
                ...
                best = Math.min(best, dist(m.players[base + s].pos, m.players[j].pos));
              }
              sums[line][0] += best;
              sums[line][1]++;
              crowdN++;
        ...
        if (m.ball.kind === 'held') {
          const c = m.ball.by;
          for (let i = 0; i < 22; i++) {
            if (m.players[i].team === m.players[c].team || i === m.movement.presserIdx) continue;
    ...
    console.log(`SPACING metrics: crowding<300cm=${((crowd / crowdN) * 100).toFixed(2)}% supportPressure=${supportPressure.toFixed(3)}/held-tick`);
...
describe('presser lease', () => {
  function rig(): MatchState {
    ...
    for (const idx of [11, 12, 13, 15, 17, 18, 19, 20, 21]) m.players[idx].pos = { x: 400, y: 900 };
    return m;
  }
  ...
  it('holds the selected presser for the lease window even when a rival becomes nearer', () => {
    ...
    for (let k = 0; k < PRESSER_LEASE_TICKS - 1; k++) {
      ...
      m.tick++;
      movementTick(m);
      if (m.tick < since + PRESSER_LEASE_TICKS) expect(m.movement.presserIdx).toBe(14);
    }
...
describe('determinism', () => {
  it('double-run: positions and movement bookkeeping are byte-identical after 400 ticks', () => {
    ...
    for (let i = 0; i < 400; i++) { tick(a); tick(b); }
