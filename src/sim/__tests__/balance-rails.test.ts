  ...
  describe('balance rails', () => {
    it('a normal matchup stays within sane bounds (200 seeds)', () => {
      ...
      for (let seed = 1; seed <= N; seed++) {
        ...
        totalShots += shots;
        totalSaves += saves;
        totalSlides += r.events.filter(
          (e) => e.kind === 'SLIDE_STARTED',
        ).length;
        totalSlideWins += r.events.filter(
          (e) => e.kind === 'TACKLE' && e.style === 'slide' && e.won,
        ).length;
      ...
      console.log(
        `BALANCE RAILS normal: goals/match=${goalsPerMatch.toFixed(3)} maxGoalsInAMatch=${maxGoals} shots/match=${shotsPerMatch.toFixed(3)} saveRate=${saveRate.toFixed(4)} slides/match=${(totalSlides / N).toFixed(3)} slideWinRate=${(totalSlideWins / Math.max(1, totalSlides)).toFixed(3)} over ${N} seeds`,
      );
      ...
      expect(saveRate).toBeLessThanOrEqual(0.9);
    ...
    it('a +20-stat team dominates from either side without a runaway blowout (200 paired seeds)', () => {
      ...
      const N = 200;
      let homeStrongWins = 0,
        awayStrongWins = 0;
      let homeWeakWins = 0,
        awayWeakWins = 0;
      let homeStrongGoals = 0,
        awayStrongGoals = 0,
        weakGoals = 0;
      let strongShots = 0,
        strongPasses = 0;
      ...
      for (let seed = 1; seed <= N; seed++) {
        ...
        strongShots += home.events.filter(
          (e) => e.kind === 'SHOT' && e.by < 11,
        ).length;
        strongShots += away.events.filter(
          (e) => e.kind === 'SHOT' && e.by >= 11,
        ).length;
        strongPasses += home.events.filter(
          (e) => e.kind === 'PASS' && e.from < 11,
        ).length;
        strongPasses += away.events.filter(
          (e) => e.kind === 'PASS' && e.from >= 11,
        ).length;
      }
      ...
      const percentile = (p: number): number =>
        goalMargins[Math.ceil(goalMargins.length * p) - 1];
      ...
      expect(
        Math.abs(homeStrongGoalsPerMatch - awayStrongGoalsPerMatch),
