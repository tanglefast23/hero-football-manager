...
const markTable = JSON.parse(
  readFileSync(
    join(__dirname, '..', '..', '..', 'content', 'fonts', 'vi-marks.json'),
    'utf8',
  ),
) as MarkTable;
...
describe('gate 6.1 — the face covers Vietnamese', () => {
  test('134 letters, in both cuts', () => {
    ...
    for (const cut of CUTS) {
      ...
      expect({
        cut,
        missing: missingGlyphs(VIETNAMESE_LETTERS.join(''), covered),
      }).toEqual({ cut, missing: [] });
    }
  });
  ...
  test('stock Silkscreen still cannot — so the gate is not vacuous', () => {
    expect(
      missingGlyphs(VIETNAMESE_LETTERS.join(''), glyphSet(stock('400Regular')))
        .length,
    ).toBe(102);
  });
  ...
  test('both cuts cover exactly the same codepoints', () => {
    const [regular, bold] = CUTS.map((cut) =>
      [...glyphSet(derived(cut))].sort((a, b) => a - b).join(','),
    );
...
function shapeOf(glyph: Glyph): unknown {
  return glyph.components === null
    ? {
        kind: 'simple',
        contours: glyph.contours.map((points) => points.map(round)),
      }
...
describe('gate 6.2 — the original face is untouched', () => {
  ...
  test.each(CUTS)(
    '%s — every pass-through table is present and byte-identical',
    (cut) => {
      ...
      for (const tag of PASS_THROUGH) {
        ...
        expect({
          cut,
          tag,
          present: original !== undefined && copied !== undefined,
        }).toEqual({ cut, tag, present: true });
        expect({ cut, tag, equal: original!.equals(copied!) }).toEqual({
          cut,
          tag,
          equal: true,
        });
  ...
  test.each(CUTS)(
    '%s — glyph IDs 0..226 keep their geometry, metrics and hinting',
    (cut) => {
      const before = readFace(stock(cut));
      const after = readFace(derived(cut));
      ...
      for (let id = 0; id < before.numGlyphs; id += 1) {
        ...
        if (
          JSON.stringify(shapeOf(original)) !== JSON.stringify(shapeOf(copied))
        ) {
          differences.push(`glyph ${id}: geometry`);
        }
        if (original.instructions !== copied.instructions)
          differences.push(`glyph ${id}: instructions`);
        if (before.metrics[id]!.advance !== after.metrics[id]!.advance)
          differences.push(`glyph ${id}: advance`);
        if (before.metrics[id]!.lsb !== after.metrics[id]!.lsb)
          differences.push(`glyph ${id}: lsb`);
      }
  ...
  test.each(CUTS)(
    '%s — every original codepoint keeps its glyph, advance and sidebearing',
    (cut) => {
      const before = readFace(stock(cut));
      const after = readFace(derived(cut));
      const differences: string[] = [];
      for (const [code, glyph] of before.cmap) {
        ...
        if (moved !== glyph)
          differences.push(
            `U+${code.toString(16)}: glyph ${glyph} -> ${String(moved)}`,
          );
        else if (
          before.metrics[glyph]!.advance !== after.metrics[glyph]!.advance ||
          before.metrics[glyph]!.lsb !== after.metrics[glyph]!.lsb
        ) {
          differences.push(`U+${code.toString(16)}: metrics`);
        }
      }
      expect({ cut, count: before.cmap.size, differences }).toEqual({
        cut,
        count: 226,
        differences: [],
      });
  ...
  test.each(CUTS)('%s — new glyphs are appended, never inserted', (cut) => {
    const before = readFace(stock(cut));
    const after = readFace(derived(cut));
    const appended = [...after.cmap.entries()].filter(
      ([code]) => !before.cmap.has(code),
    );
    expect(appended).toHaveLength(102);
    for (const [code, glyph] of appended) {
      expect({ code, appended: glyph >= before.numGlyphs }).toEqual({
        code,
        appended: true,
      });
    }
...
describe('gate 6.3 — declared metrics are unchanged and no ink escapes the box', () => {
  test.each(CUTS)(
    '%s — the vertical metrics every layout constant rests on',
    (cut) => {
    ...
    for (let id = 227; id < face.numGlyphs; id += 1) {
      for (const contour of face.glyph(id).outline) {
        for (const point of contour) {
          if (point.y < -250 || point.y > 1000)
            offenders.push(`glyph ${id}: y=${point.y}`);
        }
      }
    }
  ...
  test.each(CUTS)('%s — the append checklist landed', (cut) => {
    ...
    expect(face.os2.xAvgCharWidth).not.toBe(
      readFace(stock(cut)).os2.xAvgCharWidth,
    );
  });
});
...
describe('gate 6.4 — advances', () => {
  test.each(CUTS)('%s — an accented form keeps its base advance', (cut) => {
    ...
    const advance = (character: string) =>
      face.metrics[face.cmap.get(character.codePointAt(0)!)!]!.advance;
    const offenders: string[] = [];
    for (const entry of markTable.cuts[cut]!) {
      ...
      if (advance(entry.char) !== advance(entry.base)) {
        offenders.push(
          `${entry.char}: ${advance(entry.char)} vs ${entry.base} ${advance(entry.base)}`,
        );
      }
    }
    expect({ cut, offenders }).toEqual({ cut, offenders: [] });
  ...
  test.each(CUTS)(
    '%s — the horn costs nothing, or one pixel, and nothing else',
    (cut) => {
      ...
      for (const [horned, base] of [
        ...
        expect({ cut, horned, advance: advance(horned) }).toEqual({
          cut,
          horned,
  ...
  test('the measured column tables still fit, in the face they will be drawn in', () => {
    ...
    for (const character of 'ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789.,-$') {
      ...
      expect({
        character,
        advance: face.metrics[face.cmap.get(code)!]!.advance,
      }).toEqual({
        character,
        advance: before.metrics[before.cmap.get(code)!]!.advance,
      });
    }
  });
});
...
describe('gate 6.6 — the sub-pixel rows survive the smallest shipped type', () => {
  ...
  test('the sub-row heights are the integer table the spec fixed', () => {
    ...
    const bands = (
      markTable as unknown as { bands: Record<string, [number, number]> }
    ).bands;
    ...
    for (const [name, [low, high]] of Object.entries(bands)) {
      expect({
        name,
        integral: Number.isInteger(low) && Number.isInteger(high),
...
describe('gate 6.8 — Vietnamese copy is NFC and drawable', () => {
  test('every string in the vi catalog, plus the endonym, is NFC', () => {
    ...
    expect(localeMeta('vi').endonym.normalize('NFC')).toBe(
      localeMeta('vi').endonym,
    );
  });
  ...
  test('every character of the vi catalog is in the face', () => {
    ...
    const text = [
      localeMeta('vi').endonym,
      ...Object.values(loadCatalog('vi').strings),
    ].join('');
...
describe('gate 6.9 — every new glyph round-trips against its coordinate table', () => {
  ...
  function expectedCells(
    base: Set<string>,
    added: readonly [number, number, number, number][],
  ): Set<string> {
  ...
  test.each(CUTS)(
    '%s — 102 glyphs equal base outline plus their listed rectangles',
    (cut) => {
      const face = readFace(derived(cut));
      ...
      for (const entry of entries) {
        ...
        if (missing.length > 0 || extra.length > 0) {
          offenders.push(
            `${entry.char} (${entry.codepoint}): missing ${missing.join(' ')} extra ${extra.join(' ')}`,
          );
        }
        if (face.metrics[id]!.advance !== entry.advance) {
          offenders.push(
            `${entry.char}: advance ${face.metrics[id]!.advance} vs table ${entry.advance}`,
          );
        }
      }
      expect({ cut, offenders }).toEqual({ cut, offenders: [] });
  ...
  test.each(CUTS)(
    '%s — no two letters share a tone, and no tone is the wrong one',
    (cut) => {
      ...
      for (const entry of entries) {
        ...
        if (seen === undefined) canonical.set(key, tone);
          ...
          offenders.push(
            `${entry.char}: ${key} drawn as "${tone}", elsewhere "${seen}"`,
          );
      }
      expect({ cut, offenders }).toEqual({ cut, offenders: [] });
...
describe('licence and naming', () => {
  test('the OFL text ships beside the TTFs', () => {
    const licence = readFileSync(
      join(__dirname, '..', '..', '..', 'assets', 'fonts', 'OFL.txt'),
      'utf8',
    );
  ...
  test.each(CUTS)(
    '%s — name IDs 1/3/4/6 are distinct and 13/14 are the original OFL records',
    (cut) => {
      const names = readNameTable(derived(cut));
      const original = readNameTable(stock(cut));
      for (const nameID of [1, 3, 4, 6]) {
        expect({ nameID, name: names.get(nameID) }).not.toEqual({
          nameID,
          name: original.get(nameID),
        });
...
function readNameTable(path: string): Map<number, string> {
  ...
  for (let index = 0; index < count; index += 1) {
    ...
    found.set(
      ...
      Buffer.from(bytes.subarray(storage + offset, storage + offset + length))
        .swap16()
        .toString('utf16le'),
    );
  }
