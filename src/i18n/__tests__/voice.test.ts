...
describe('voiceOf', () => {
  ...
  test('long prose is body, which is not a pixel face at all', () => {
    ...
    expect(
      faceForKey('event.derby-night.body', localeMeta('en').faces),
    ).toBeNull();
  });

  test('a display key resolves to the locale s display face', () => {
    expect(faceForKey('settings.language.title', localeMeta('vi').faces)).toBe(
      'HFMSilkscreen_700Bold',
    );
    expect(faceForKey('col.league.points', localeMeta('vi').faces)).toBe(
