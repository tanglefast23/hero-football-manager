...
describe('retired weekly-training migration', () => {
  it('loads a save carrying a weekly plan, cap notices, a promise bump, and the slot cap', () => {
    ...
    const loaded = parseStoredGameState(legacy) as unknown as Record<
      string,
      unknown
    >;
    ...
    expect(
      (loaded.trainingRules as Record<string, unknown>).maxFocusDrillsPerWeek,
    ).toBeUndefined();
    expect(
      (loaded.trainingRules as { focusDrills: unknown[] }).focusDrills,
    ).toHaveLength(35);
