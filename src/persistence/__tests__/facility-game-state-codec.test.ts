...
describe('facility game-state persistence', () => {
  test('round-trips the grid, adjacency discoveries, and facility ledger lines', () => {
    ...
    const gym = completeProject(
      buildCareerFacility(initial, 'gym', { x: 0, y: 0 }).state,
    );
    const dorm = completeProject(
      buildCareerFacility(gym, 'dorm', { x: 1, y: 0 }).state,
    );
    const state = {
      ...dorm,
      players: dorm.players.map((player, index) =>
        index === 0 ? { ...player, facilityStaBonusRemainder: 70 } : player,
      ),
      ledgers: [
        {
          season: 1,
    ...
    expect(restored.facilities.grid?.discoveredAdjacencies).toEqual([
      'gym-dorm',
    ]);
  ...
  test('round-trips a closure logged while the club was in the red', () => {
    ...
    expect(
      closed.clubs.find((club) => club.id === closed.userClubId)?.cash,
    ).toBe(-6_500);
    ...
    expect(
      parseStoredGameState(serializeGameState(closed)).cashTransactions?.at(-1),
  ...
  test('loads an M1 save without a grid or stamina-bonus carry', () => {
    ...
    for (const player of legacy.players)
      delete player.facilityStaBonusRemainder;
    ...
    expect(
      restored.players.every(
        (player) => player.facilityStaBonusRemainder === undefined,
      ),
    ).toBe(true);
  });
  ...
  test.each([
    ...
    if (_label === 'outside the facility grid') {
      ...
      Object.assign(stored.facilities.grid.buildings[1], {
        id: 'facility-2',
        type: 'gym',
      });
    }
  ...
  test.each([
    ...
    (_label, buildingPatch, constructionPatch, expectedBasis) => {
      ...
      expect(migrated.facilities.grid?.buildings[0].capitalInvested).toBe(
        expectedBasis,
      );
      expect(
        parseStoredGameState(JSON.stringify(migrated)).facilities.grid
          ?.buildings[0],
      ).toMatchObject({
        type: 'training-pitch',
        capitalInvested: expectedBasis,
      });
    },
  );
  ...
  test('requires a nonnegative safe capital basis in every schema-4 building', () => {
    ...
    expect(() => serializeGameState(invalid)).toThrow(
