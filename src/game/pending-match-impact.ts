...
export function pendingImpactFromProduction(input: {
  ...
  if (production.fixtureResult.fixtureId !== fixture.id) {
    throw new Error(
      `production result ${production.fixtureResult.fixtureId} does not match ${fixture.id}`,
    );
  }
  const userIsHome = fixture.homeClubId === state.userClubId;
  if (!userIsHome && fixture.awayClubId !== state.userClubId) {
    throw new Error(
      `fixture ${fixture.id} does not contain user club ${state.userClubId}`,
    );
  }

  const participants = uniqueIds(
    production.participantPlayerIds,
    `${fixture.id} participant`,
  );
  const powerFired = uniqueIds(
    production.powerFiredPlayerIds,
    `${fixture.id} power owner`,
  );
  ...
  for (const playerId of powerFired) {
    if (playerById.get(playerId)?.power === undefined) {
      throw new Error(
        `fixture ${fixture.id} power owner ${playerId} is not a hero`,
      );
    }
  }

  const heroAppearancePlayerIds = participants.filter(
    (playerId) => playerById.get(playerId)?.power !== undefined,
  );
...
export function appendPendingUserMatchImpact(
  ...
  const existing = pending.find(
    (candidate) => candidate.fixtureId === impact.fixtureId,
  );
  if (existing === undefined) return [...pending, impact];
  if (JSON.stringify(existing) !== JSON.stringify(impact)) {
    throw new Error(
      `fixture ${impact.fixtureId} already has a different pending match impact`,
    );
  }
  return [...pending];
}
...
function userOutcome(input: {
  ...
  if (
    input.cupWinnerClubId !== input.homeClubId &&
    input.cupWinnerClubId !== input.awayClubId
  ) {
