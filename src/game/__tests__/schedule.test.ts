...
describe('generateSeasonFixtures', () => {
  ...
  test('gives every club nine home and nine away matches', () => {
    ...
    for (const clubId of CLUB_IDS) {
      expect(
        fixtures.filter((fixture) => fixture.homeClubId === clubId),
      ).toHaveLength(9);
      expect(
        fixtures.filter((fixture) => fixture.awayClubId === clubId),
      ).toHaveLength(9);
    }
  });
  ...
  test('starts the opening season in Week 3 and brings Match 2 forward to Week 4', () => {
    ...
    expect(
      weeks.every((week, index) => index === 0 || week > weeks[index - 1]),
    ).toBe(true);
  ...
  test('changes fixture seeds when the career seed or season changes', () => {
    const base = generateSeasonFixtures(CLUB_IDS, 1, 100).map(
      (fixture) => fixture.matchSeed,
    );
    const otherCareer = generateSeasonFixtures(CLUB_IDS, 1, 101).map(
      (fixture) => fixture.matchSeed,
    );
    const otherSeason = generateSeasonFixtures(CLUB_IDS, 2, 100).map(
      (fixture) => fixture.matchSeed,
    );
    ...
    const opening = [1, 2, 3, 4, 5].map((round) => {
      const fixture = fixtures.find(
        (candidate) =>
          ...
          (candidate.homeClubId === CLUB_IDS[0] ||
            candidate.awayClubId === CLUB_IDS[0]),
      )!;
  ...
  test('rejects malformed schedule inputs', () => {
    expect(() => generateSeasonFixtures(CLUB_IDS.slice(0, 9), 1, 1)).toThrow(
      'exactly 10 clubs',
    );
    expect(() =>
      generateSeasonFixtures([...CLUB_IDS.slice(0, 9), CLUB_IDS[0]], 1, 1),
    ).toThrow('unique');
    expect(() =>
      generateSeasonFixtures([...CLUB_IDS.slice(0, 9), '  '], 1, 1),
    ).toThrow('non-empty');
    expect(() => generateSeasonFixtures(CLUB_IDS, 0, 1)).toThrow(
      'positive integer',
    );
    expect(() => generateSeasonFixtures(CLUB_IDS, 1.5, 1)).toThrow(
      'positive integer',
    );
    expect(() => generateSeasonFixtures(CLUB_IDS, 1, Number.NaN)).toThrow(
      'safe integer',
    );
