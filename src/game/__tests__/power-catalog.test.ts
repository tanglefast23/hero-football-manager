...
const EXPECTED_ROLE_POOLS: Readonly<Record<Role, readonly PowerId[]>> = {
  ...
  MID: ['SUPER_SPEED', 'BLINK_RUN', 'FIRE_TORCH', 'PHASE_RUN', 'PORTAL_PASS',
    ...
    'SHADOW_MARK', 'GRAVITY_WELL', 'GUST'],
  FWD: ['SUPER_SPEED', 'BLINK_RUN', 'THUNDER_STRIKE', 'FIRE_TORCH', 'PHASE_RUN',
    'PORTAL_PASS', 'RALLY_CRY', 'GRAVITY_WELL'],
};
...
describe('generated club power variety', () => {
  it('uses the exact role pools and deterministically exposes every member of each pool', () => {
    for (const role of ['GK', 'DEF', 'MID', 'FWD'] as const) {
      const compatible = LAUNCH_POWER_IDS.filter(power => powerIsCompatibleWithRole(power, role));
