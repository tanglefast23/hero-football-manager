...
describe('cash transaction ids', () => {
  const entry = { kind: 'scouting' as const, label: 'Scout trip', amount: -100 };
  ...
  test('ids are monotonic and survive a pruned history without colliding', () => {
    ...
    expect(second.cashTransactions?.map(transaction => transaction.id))
    ...
    expect(afterPrune.cashTransactions?.map(transaction => transaction.id))
  ...
  test('a save from before the counter seeds it from the ids it already issued', () => {
    ...
    const second = recordCashTransaction(recordCashTransaction(initial, entry), entry);
    const { cashTransactionIdCounter: _legacyOmits, ...legacy } = second;

    const migrated = recordCashTransaction(legacy, entry);

    expect(migrated.cashTransactions?.map(transaction => transaction.id))
      .toEqual(['cash-transaction-1', 'cash-transaction-2', 'cash-transaction-3']);
