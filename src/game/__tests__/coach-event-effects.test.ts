import { applyCoachEventEffect, selectCareerEventCoach } from '../career-events';
...
function coach(id: string, specialties: [CoachSpecialty, CoachSpecialty]): CoachCandidate {
  return {
    id,
    name: id,
    specialties,
...
function careerWithStaff(head?: CoachCandidate, assistant?: CoachCandidate): GameState {
  return {
    userClubId: 'user',
...
describe('pointing a story at a coach', () => {
  it('selects a slot the club actually staffs', () => {
    ...
    expect(selectCareerEventCoach(state, 'HEAD').pendingEvent?.selectedCoachRole).toBe('HEAD');
  });

  it('refuses a slot the club has left empty', () => {
    const state = careerWithStaff(coach('head', ['ATTACK', 'FITNESS']));
    expect(() => selectCareerEventCoach(state, 'ASSISTANT')).toThrow('employs no assistant coach');
  });
  ...
  it('refuses to re-point a chapter that inherited its coach', () => {
    ...
    const locked: GameState = {
      ...state,
      pendingEvent: { ...state.pendingEvent!, selectedCoachRole: 'HEAD', coachLocked: true },
    };
    expect(() => selectCareerEventCoach(locked, 'HEAD')).toThrow('chosen earlier in the story');
  });
});
...
describe('applying a coach effect', () => {
  it('accumulates a boost across separate stories', () => {
    ...
    state = applyCoachEventEffect(state, 'HEAD', { facet: 'training', amount: 5 });
    state = applyCoachEventEffect(state, 'HEAD', { facet: 'training', amount: 3 });
  ...
  it('changes only the coach the story pointed at', () => {
    ...
    state = applyCoachEventEffect(state, 'ASSISTANT', { facet: 'tp', amount: 2 });
  ...
  it('retrains the second specialty and leaves the first alone', () => {
    let state = careerWithStaff(coach('head', ['ATTACK', 'FITNESS']));
    state = applyCoachEventEffect(state, 'HEAD', { specialtyTo: 'GOALKEEPING' });
    expect(state.market?.headCoach?.specialties).toEqual(['ATTACK', 'GOALKEEPING']);
  });

  it('refuses a retraining that would duplicate a specialty', () => {
    const state = careerWithStaff(coach('head', ['ATTACK', 'GOALKEEPING']));
    expect(() => applyCoachEventEffect(state, 'HEAD', { specialtyTo: 'GOALKEEPING' }))
