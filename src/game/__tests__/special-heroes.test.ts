...
const specialsIn = (state: GameState) => state.players.filter(player => isSpecialHeroId(player.id));
const overall = (player: CareerPlayer) => roleOverall(player.role, player.attrs);
...
describe('the special hero roster', () => {
  ...
  it('places eleven and reserves four for the scout', () => {
    const placed = ([5, 4, 3, 2, 1] as const).flatMap(division => specialHeroesForDivision(division));
    expect(placed).toHaveLength(11);
    expect(scoutOnlySpecialHeroes()).toHaveLength(4);
    expect(([5, 4, 3, 2, 1] as const).map(d => specialHeroesForDivision(d).length)).toEqual([1, 1, 2, 3, 4]);
    ...
    for (const division of [5, 4, 3, 2, 1] as const) {
      ...
      expect(heroes.map(hero => hero.order)).toEqual(heroes.map((_, index) => index + 1));
    }
  });
});
...
describe('special hero ratings', () => {
        ...
        for (let order = 1; order <= count; order += 1) {
          ...
          for (const role of ['FWD', 'MID', 'DEF'] as const) {
            ...
            if (roleOverall(role, attrs) !== target) {
              failures.push(`${role} base=${base} count=${count} order=${order}`);
            }
          }
        }
      }
  ...
  it('clears the best ordinary player by eight and spaces the ranks by three', () => {
    expect([1, 2, 3, 4].map(order => specialHeroTargetOverall(50, 4, order))).toEqual([67, 64, 61, 58]);
  ...
  it('spikes the attribute the role is actually judged on', () => {
    const peak = (role: 'FWD' | 'MID' | 'DEF') => {
      ...
      return OUTFIELD.reduce((best, key) => (attrs[key] > attrs[best] ? key : best), OUTFIELD[0]);
    };
    expect([peak('FWD'), peak('MID'), peak('DEF')]).toEqual(['sho', 'pas', 'def']);
  });
...
describe('placement in a career', () => {
  it('gives division five its one hero, on the strongest rival, in the starting eleven', () => {
    ...
    const squad = career.players.filter(player => player.clubId === hero.clubId);
  ...
  it('never lets an ordinary player wear a reserved face', () => {
    ...
    const strays = career.players.filter(player => (
      !isSpecialHeroId(player.id) && player.lookId !== undefined && isReservedFieldLook(player.lookId)
    ));
  ...
  it('rebuilds each season without duplicating a character or leaking into the pyramid', () => {
    const career = runHeadlessFullCareer(createLaunchCareerSetup(20260720), 7);
    const specials = specialsIn(career);
    expect(new Set(specials.map(player => player.id)).size).toBe(specials.length);
