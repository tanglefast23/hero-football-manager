...
describe('facility catalog and grid', () => {
  test('defines the complete M2 catalog on a deterministic 8x6 grid', () => {
    ...
    expect([grid.width, grid.height]).toEqual([
      FACILITY_GRID_WIDTH,
      FACILITY_GRID_HEIGHT,
    ]);
    ...
    expect(
      Object.values(FACILITY_CATALOG).every(
        (entry) =>
          entry.buildWeeks >= 1 &&
    ...
    expect(FACILITY_CATALOG['coaching-office'].weeklyUpkeep).toEqual([
      40, 65, 100,
    ]);
    expect(
      Object.fromEntries(
  ...
  test('builds immutably, charges the catalog cost, and generates stable IDs', () => {
    ...
    expect(() =>
      build(first.grid, 'dorm', { x: 3, y: 1 }, first.cashAfter),
  ...
  test('rejects locked, unaffordable, out-of-bounds, fractional, and overlapping builds', () => {
    const grid = finishConstruction(
      build(createFacilityGrid(), 'training-pitch', { x: 0, y: 0 }).grid,
    );

    expect(() =>
      buildFacility(grid, 'training-pitch', { x: 4, y: 0 }, 1_000_000),
    ).toThrow(/Training Pitch is already built/);
    expect(() => buildFacility(grid, 'gym', { x: 3, y: 3 }, 6_999)).toThrow(
      /not affordable/,
    );
    expect(() =>
      buildFacility(grid, 'youth-field', { x: 7, y: 5 }, 1_000_000),
    ).toThrow(/outside/);
    expect(() =>
      buildFacility(grid, 'gym', { x: 2.5, y: 2 }, 1_000_000),
    ).toThrow(/integer grid coordinates/);
    expect(() =>
      buildFacility(grid, 'medical-bay', { x: 1, y: 1 }, 1_000_000),
    ).toThrow(/overlaps/);
  });
  ...
  test('rejects a second copy while the first copy is still under construction', () => {
    ...
    expect(() => buildFacility(grid, 'gym', { x: 2, y: 0 }, 1_000_000)).toThrow(
      /Gym is already built/,
    );
  });
  ...
  test('allows three income buildings of each type and rejects a fourth', () => {
    ...
    for (let index = 0; index < 3; index += 1) {
      shops = finishConstruction(
        build(shops, 'fan-shop', { x: index, y: 0 }).grid,
      );
    }
    ...
    expect(() => build(shops, 'fan-shop', { x: 3, y: 0 })).toThrow(
      /Fan Shop limit reached.*up to 3/,
    );

    let stands = createFacilityGrid();
    for (let index = 0; index < 3; index += 1) {
      stands = finishConstruction(
        build(stands, 'stadium-stand', { x: index * 2, y: 0 }).grid,
      );
    }
    expect(() => build(stands, 'stadium-stand', { x: 6, y: 0 })).toThrow(
      /Stadium Stand limit reached.*up to 3/,
    );
  });

  test('asks repeatable income buildings to wait while the works crew is busy', () => {
    const firstShop = build(createFacilityGrid(), 'fan-shop', {
      x: 0,
      y: 0,
    }).grid;

    expect(() => build(firstShop, 'fan-shop', { x: 1, y: 0 })).toThrow(
      /only one facility construction project/,
    );
  });
});
...
describe('facility upgrades and upkeep', () => {
  ...
  test('validates upgrade identity and affordability', () => {
    const grid = finishConstruction(
      build(createFacilityGrid(), 'medical-bay', { x: 0, y: 0 }).grid,
    );

    expect(() => upgradeFacility(grid, 'missing', 100_000)).toThrow(
      /unknown facility/,
    );
    expect(() => upgradeFacility(grid, 'facility-1', 12_499)).toThrow(
      /not affordable/,
    );
  });
  ...
  test('upgrades one income building without changing its other copies', () => {
    let grid = finishConstruction(
      build(createFacilityGrid(), 'fan-shop', { x: 0, y: 0 }).grid,
    );
    grid = finishConstruction(build(grid, 'fan-shop', { x: 1, y: 0 }).grid);

    const upgrading = finishConstruction(
      upgradeFacility(grid, 'facility-2', 100_000).grid,
    );

    expect(
      upgrading.buildings.map((building) => [building.id, building.level]),
    ).toEqual([
      ['facility-1', 1],
      ['facility-2', 2],
    ]);
  ...
  test('rejects persisted investment outside the non-negative safe-integer invariant', () => {
    ...
    expect(() => weeklyFacilityUpkeep(withInvestment(-1))).toThrow(
      /capital invested must be a non-negative safe integer/,
    );
    expect(() => weeklyFacilityUpkeep(withInvestment(1.5))).toThrow(
      /capital invested must be a non-negative safe integer/,
    );
  });
});
...
describe('closing a facility', () => {
  test('refunds half of the build and every finished upgrade, and frees the square', () => {
    ...
    const upgraded = finishConstruction(
      upgradeFacility(ready, 'facility-1', 30_000).grid,
    );

    // 7,000 built + 18,000 to reach Level 2.
    expect(facilityInvestment(upgraded.buildings[0])).toBe(25_000);
    const closed = closeFacility(upgraded, 'facility-1', 1_000);

    expect(closed).toMatchObject({ cost: -12_500, cashAfter: 13_500 });
  ...
  test('gives nothing back for a founding facility the club never paid for', () => {
    ...
    expect(closeFacility(seeded, 'facility-1', 500)).toMatchObject({
      cost: 0,
      cashAfter: 500,
    });
  });
  ...
  test('pays out while the club is in the red, where spending is still refused', () => {
    const grid = finishConstruction(
      build(createFacilityGrid(), 'gym', { x: 0, y: 0 }, 30_000).grid,
    );
    const closed = closeFacility(grid, 'facility-1', -10_000);

    expect(closed).toMatchObject({ cost: -3_500, cashAfter: -6_500 });
    expect(closed.grid.buildings).toEqual([]);
    expect(() => upgradeFacility(grid, 'facility-1', -10_000)).toThrow(
      /non-negative safe integer/,
    );
    expect(() => buildFacility(grid, 'dorm', { x: 4, y: 4 }, -10_000)).toThrow(
      /non-negative safe integer/,
    );
    expect(() =>
      relocateFacility(grid, 'facility-1', { x: 4, y: 4 }, -10_000),
    ).toThrow(/non-negative safe integer/);
    // A credit still refuses nonsense; only the floor was lifted.
    expect(() => closeFacility(grid, 'facility-1', 1.5)).toThrow(
      /safe integer/,
    );
  });
  ...
  test('refuses to demolish whatever the crew is currently on', () => {
    ...
    expect(() => closeFacility(built.grid, 'facility-1', 1_000)).toThrow(
      /cannot close while construction is active/,
    );
    expect(() =>
      closeFacility(finishConstruction(built.grid), 'missing', 1_000),
    ).toThrow(/unknown facility/);
  });

  test('rebuilding costs full price, so closing is never a discount', () => {
    const built = build(createFacilityGrid(), 'gym', { x: 0, y: 0 }, 30_000);
    const closed = closeFacility(
      finishConstruction(built.grid),
      'facility-1',
      built.cashAfter,
    );
...
describe('facility relocation and adjacency', () => {
  test('relocates for the explicit fee without changing level or identity', () => {
    ...
    expect(moved.grid.buildings[0]).toEqual({
      ...
      type: 'shooting-range',
      level: 2,
      capitalInvested: 27_500,
      x: 7,
      y: 4,
    });
    expect(() =>
      relocateFacility(moved.grid, 'facility-1', { x: 6, y: 5 }, 1_000),
    ).toThrow(/outside/);
    expect(() =>
      relocateFacility(moved.grid, 'facility-1', { x: 7, y: 4 }, 1_000),
  ...
  test('discovers all three orthogonal pairings, applies effects once, and remembers discoveries', () => {
    ...
    grid = finishConstruction(
      build(grid, 'stadium-stand', { x: 1, y: 2 }).grid,
    );
    grid = finishConstruction(build(grid, 'medical-bay', { x: 4, y: 0 }).grid);
    grid = finishConstruction(
      build(grid, 'training-pitch', { x: 5, y: 0 }).grid,
    );

    expect(activeFacilityAdjacencies(grid)).toEqual(
      FACILITY_ADJACENCIES.map((item) => item.id),
    );
    expect(grid.discoveredAdjacencies).toEqual(
      FACILITY_ADJACENCIES.map((item) => item.id),
    );
    ...
    expect(moved.grid.discoveredAdjacencies).toEqual(
      FACILITY_ADJACENCIES.map((item) => item.id),
    );
  ...
  test('charges $230 a week for the four Level-1 opening facilities', () => {
    ...
    grid = finishConstruction(
      build(grid, 'coaching-office', { x: 2, y: 0 }).grid,
    );
    grid = finishConstruction(build(grid, 'fan-shop', { x: 3, y: 0 }).grid);
    grid = finishConstruction(
      build(grid, 'stadium-stand', { x: 4, y: 0 }).grid,
    );
  ...
  test('rejects relocations that overlap another footprint or cannot be afforded', () => {
    let grid = finishConstruction(
      build(createFacilityGrid(), 'youth-field', { x: 0, y: 0 }).grid,
    );
    grid = finishConstruction(build(grid, 'gym', { x: 3, y: 0 }).grid);

    expect(() =>
      relocateFacility(grid, 'facility-2', { x: 1, y: 1 }, 1_000),
    ).toThrow(/overlaps/);
    expect(() =>
      relocateFacility(grid, 'facility-2', { x: 4, y: 0 }, 349),
