...
describe('the agent has a mood, and it cannot be rerolled', () => {
  ...
  it('varies by player, by season and by round', () => {
    const byRound = [1, 2].map((round) =>
      askWobblePercent(7, 'renewal-s1-a', round),
    );
    const byPlayer = ['renewal-s1-a', 'renewal-s1-b', 'renewal-s2-a'].map(
      (id) => askWobblePercent(7, id, 1),
    );
  ...
  it('is silent in the final round, where the agent has named his price', () => {
    for (let seed = 1; seed <= 50; seed += 1) {
      expect(
        askWobblePercent(seed, `renewal-s1-p${seed}`, FINAL_NEGOTIATION_ROUND),
      ).toBe(0);
    }
  });
});
...
describe('the wage the screen tells you to offer is the wage that is accepted', () => {
    ...
    for (const personality of PERSONALITIES) {
      for (const perk of PERKS) {
        const negotiation = talks({
          personality,
          id: `renewal-s1-${personality}-${perk}`,
        });
        ...
        expect({ personality, perk, status: result.status }).toEqual({
          personality,
          perk,
          status: 'ACCEPTED',
        });
  ...
  it('is the LOWEST accepted wage — a dollar less is refused', () => {
    ...
    expect(needed).toBeGreaterThan(
      insultingOfferFloor(negotiation.weeklyAsk) + 1,
    );
    expect(
      submitContractOffer(negotiation, {
        weeklyWage: needed - 1,
        termSeasons: 3,
  ...
  it('never names a wage that would end talks as an insult', () => {
    const negotiation = talks();
    expect(
      requiredWeeklyWage(negotiation, 1, 'JERSEY_10'),
  ...
  it('falls when the manager improves the term or the promise', () => {
    ...
    const threeSeasonStarter = requiredWeeklyWage(
      negotiation,
      3,
      'GUARANTEED_STARTER',
    );
  ...
  it('falls further when a loved card is on the table', () => {
    ...
    const withCard = requiredWeeklyWage(
      negotiation,
      2,
      'CAPTAINCY',
      'STRAIGHT_TALK',
...
describe('a promise is worth what the player thinks of it', () => {
  it('is worth least to the mercenary and most to the man who wants it', () => {
    ...
    expect(contractPerkPercent('CAPTAINCY', 'GREEDY')).toBeLessThan(
      contractPerkPercent('CAPTAINCY'),
    );
    expect(contractPerkPercent('CAPTAINCY', 'FIERY')).toBeGreaterThan(
      contractPerkPercent('CAPTAINCY'),
    );
    // The armband is a burden to a TIMID player and a prize to a FIERY one.
    expect(contractPerkPercent('CAPTAINCY', 'TIMID')).toBeLessThan(
      contractPerkPercent('CAPTAINCY', 'FIERY'),
    );
    expect(contractPerkPercent('GUARANTEED_STARTER', 'TIMID')).toBeGreaterThan(
      contractPerkPercent('GUARANTEED_STARTER', 'FIERY'),
    );
  });
    ...
    for (const personality of PERSONALITIES) {
      for (const perk of PERKS) {
        expect({
          personality,
          perk,
          worth: contractPerkPercent(perk, personality),
  ...
  it('is what the offer is actually scored on', () => {
    ...
    expect(contractOfferValue(offer, 'FIERY')).toBeGreaterThan(
      contractOfferValue(offer, 'GREEDY'),
    );
  });
});
