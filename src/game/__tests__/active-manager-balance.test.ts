...
function winnerScore(f: LeagueFixture, user: string) {
  if (f.homeClubId === user)
    return { fixtureId: f.id, homeGoals: 3, awayGoals: 0 };
  if (f.awayClubId === user)
    return { fixtureId: f.id, homeGoals: 0, awayGoals: 3 };
...
describe('active-manager economy rail', () => {
  test.each(SEEDS)(
    'a winning, actively-training manager stays bounded (seed %i)',
    (seed) => {
      ...
      while (!(state.phase === 'season-end' && state.season === SEASONS)) {
        ...
        if (state.phase === 'manage') {
          ...
          state = completeMatchday(
            state,
            md.fixtures.map((f) => winnerScore(f, state.userClubId)),
          );
