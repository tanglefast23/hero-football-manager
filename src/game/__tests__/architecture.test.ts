...
const BANNED_RUNTIME = /\b(?:Math\.(?:exp|hypot|pow|sin|cos|tan|log|random|atan2|atan|asin|acos|cbrt|log2|log10|log1p|expm1|sinh|cosh|tanh|fround)|Date\.now|new Date|performance\.now)\s*\(/g;
...
function sourceFiles(root = GAME_DIR): string[] {
  ...
  for (const entry of readdirSync(root, { withFileTypes: true })) {
    if (entry.isDirectory()) {
      if (entry.name !== '__tests__') files.push(...sourceFiles(join(root, entry.name)));
...
function runtimeViolations(root = GAME_DIR): string[] {
  ...
  for (const file of sourceFiles(root)) {
    ...
    if (source.includes('**')) violations.push(`${file}: exponentiation operator **`);
  }
...
describe('game architecture boundary', () => {
  ...
  it('resolves nested game and sim imports without allowing outward dependencies', () => {
    ...
    expect(isAllowedGameImport(nestedFile, '../../ui/screens/example')).toBe(false);
  ...
  it('recursively inspects production modules in nested folders', () => {
    ...
    try {
      ...
      writeFileSync(join(nestedDir, 'bad.ts'), 'export const value = Math.random();\n');
