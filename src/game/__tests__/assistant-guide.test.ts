...
describe('assistant guide milestones', () => {
  ...
  test('records the matchday condition lesson only once per career', () => {
    ...
    expect(
      hasAssistantGuideMilestone(once, 'match-condition-warning-seen'),
    ).toBe(true);
    expect(twice).toBe(once);
  });
  ...
  test('offers the roster sort hint from Week 12 until any tap dismisses it forever', () => {
    ...
    expect(shouldShowSquadSortHint({ ...career, season: 2, week: 1 })).toBe(
      true,
    );

    const dismissed = completeAssistantGuideMilestone(
      weekTwelve,
      'squad-sort-seen',
    );
    expect(shouldShowSquadSortHint(dismissed)).toBe(false);
    expect(completeAssistantGuideMilestone(dismissed, 'squad-sort-seen')).toBe(
      dismissed,
    );
  });
  ...
  test('persists M2 firsts in insertion order and completes them idempotently', () => {
    ...
    const queuedAgain = queueAssistantGuideSequence(
      queued,
      'head-coach-market',
    );
    ...
    const completedAgain = completeAssistantGuideSequence(
      completed,
      'scout-mission',
    );
    expect(hasAssistantGuideSequenceCompleted(completed, 'scout-mission')).toBe(
      true,
    );
    expect(pendingAssistantInboxGuideSequences(completed)).toEqual([
      'head-coach-market',
    ]);
  ...
  test('treats the migrated continuity briefing as the one-time Sponsor Desk introduction', () => {
    ...
    expect(
      hasAssistantGuideSequenceCompleted(completed, 'sponsor-desk-continuity'),
    ).toBe(true);
    expect(hasAssistantGuideSequenceCompleted(completed, 'sponsor-desk')).toBe(
      true,
    );
    expect(completeAssistantGuideSequence(completed, 'sponsor-desk')).toBe(
      completed,
    );
  });
  ...
  test('delivers at most three firsts and holds the fourth for a later week', () => {
    ...
    const afterThreeReads = firstWeek.guideSequenceIds.reduce(
      (career, sequenceId) =>
        completeAssistantGuideSequence(career, sequenceId),
      firstWeek.state,
    );
    ...
    expect(
      nextWeek.state.eventFlags.some((flag) => flag.includes(':w1:')),
    ).toBe(false);
  });
  ...
  test('gives urgent product alerts priority and remains byte-idempotent', () => {
    ...
    expect(planned.productAlertIds).toEqual([
      'injury-captain',
      'board-warning',
    ]);
  ...
  test('permanently dismisses a one-time product lesson after its hand-off', () => {
    ...
    expect(
      isAssistantInboxProductPermanentlyDismissed(dismissed, 'emergency-loan'),
    ).toBe(true);
    expect(
  ...
  test('lets a newly urgent alert displace, but not erase, a scheduled guide', () => {
    ...
    expect(interrupted.guideSequenceIds).toEqual([
      'facility-placement',
      'facility-upgrade',
    ]);
    expect(interrupted.deferredGuideSequenceIds).toEqual([
      'facility-adjacency',
    ]);
  ...
  test('persists a one-shot product notice through the weekly cap and save/load', () => {
    ...
    const crowded = scheduleAssistantInboxWeek(state, {
      productAlerts: [
        ...
        {
          id: 'board-resolution:board-s1-w1',
          priority: 'normal',
          oneShot: true,
        },
      ],
    ...
    expect(crowded.productAlertIds).toEqual([
      'injury-one',
      'injury-two',
      'transfer-request',
    ]);
    expect(crowded.deferredProductAlertIds).toEqual([
      'board-resolution:board-s1-w1',
    ]);

    const reloaded = JSON.parse(
      JSON.stringify(crowded.state),
    ) as typeof crowded.state;
    ...
    expect(
      nextWeek.state.eventFlags.some((flag) =>
  ...
  test('bounds persisted one-shot queue and acknowledgement flags', () => {
    ...
    for (let week = 1; week <= 12; week += 1) {
      const planned = scheduleAssistantInboxWeek(
        { ...state, week },
        {
    ...
    expect(
      state.eventFlags.filter((flag) => flag.includes('pending-product:'))
        .length,
    ).toBeLessThanOrEqual(24);
    expect(
      state.eventFlags.filter((flag) => flag.includes('acknowledged-product:'))
        .length,
    ).toBeLessThanOrEqual(24);
  });
});
