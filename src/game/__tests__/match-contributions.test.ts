...
function row(
  ...
  return {
    playerId, goals: 0, assists: 0, tacklesWon: 0, saves: 0, passesCompleted: 0, ...counts,
  };
}
...
describe('contributionsFrom', () => {
  ...
  it('credits a live clone to the player it was copied from', () => {
    const rows = contributionsFrom(matchWith([
      {
        t: 20, kind: 'TACKLE', by: HOME_DECOY_INDEX, on: 15,
        won: true, style: 'standing', contact: true,
      },
  ...
  it('credits a popped clone to its own source, not the one standing at full time', () => {
    const rows = contributionsFrom(matchWith([
      ...
      {
        t: 30, kind: 'DECOY_POP', player: 6, clone: HOME_DECOY_INDEX, source: 10,
        pos: { x: 0, y: 0 }, reason: 'expired',
      },
