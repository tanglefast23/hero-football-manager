...
export function sponsorBaselineShares(
  ...
  return Array.from({ length: capacity }, (_, slot) => base + (slot === 0 ? remainder : 0));
}
...
export function createProvisionalSponsorPortfolio(
  ...
  return sponsorBaselineShares(nominalTotal, capacity).map((nominalMonthlyFee, slot) => ({
    contractId: `continuity-s${season}-slot${slot}`,
    sponsorContentId: 'continuity',
    sponsorName: capacity === 1 ? 'Current Sponsor' : `Current Sponsor ${slot + 1}`,
...
export function createSeasonSponsorship(context: SponsorSeasonContext): SponsorshipState {
...
export function generateSponsorOffers(
  ...
  const provisionalContracts = [...context.activeContracts]
    .filter(contract => contract.season === context.season && contract.provisional)
    .sort((left, right) => left.slot - right.slot || compareIds(left.contractId, right.contractId));
  ...
  for (const contract of provisionalContracts) {
    ...
    for (let candidateIndex = 0; candidateIndex < SPONSOR_PROFILES.length; candidateIndex += 1) {
      ...
      const objectiveDefinition = context.rules.objectives[
...
export function acceptSponsorOffer(
  ...
  if (typeof context.offerId !== 'string' || context.offerId.trim().length === 0) {
    throw new Error('sponsor offer ID must be a non-empty string');
  }
  if (sponsorship.portfolioSeason !== context.season || sponsorship.offerSeason !== context.season) {
    throw new Error('sponsor offer season does not match the active portfolio');
  }

  const offer = sponsorship.offers.find(candidate => candidate.offerId === context.offerId);
  if (offer === undefined || offer.season !== context.season) {
    throw new Error(`sponsor offer ${context.offerId} is no longer available`);
  }
  const contractsForSlot = sponsorship.activeContracts.filter(contract => (
    contract.season === context.season && contract.slot === offer.slot
  ));
  if (contractsForSlot.length !== 1) {
    throw new Error(`sponsor slot ${offer.slot} does not have exactly one active contract`);
  }
  ...
  return {
    ...
    offers: sponsorship.offers.filter(candidate => candidate.slot !== offer.slot),
  };
}
...
export function expireSponsorOfferWindow(
  ...
  if (sponsorship.portfolioSeason !== season) {
    throw new Error('cannot expire offers for a different sponsor portfolio season');
  }

  const hasOffers = sponsorship.offers.some(offer => offer.season === season);
  const hasProvisional = sponsorship.activeContracts.some(contract => (
...
export function nominalSponsorPortfolioIncome(
  ...
  return contracts.reduce(
    (total, contract) => checkedAdd(total, contract.nominalMonthlyFee, 'nominal sponsor portfolio'),
    0,
  );
}
...
export function actualSponsorPortfolioIncome(
  ...
  return allocateSponsorPortfolioPayment(contracts, sponsorIncomePercent).reduce(
    (total, allocation) => checkedAdd(total, allocation.actualAmount, 'actual sponsor portfolio'),
    0,
  );
}
...
export function sponsorObjectiveProgressFromFixtures(
  ...
  if (typeof userClubId !== 'string' || userClubId.trim().length === 0) {
    throw new Error('sponsor objective user club ID must be a non-empty string');
  }

  const table = leagueTableFromFixtures(fixtures, season);
  const user = table.find(row => row.clubId === userClubId);
  if (user === undefined) throw new Error(`league fixtures do not contain user club ${userClubId}`);

  const value = objective.kind === 'LEAGUE_WINS'
    ? user.won
    : objective.kind === 'LEAGUE_GOALS'
      ? user.goalsFor
      : user.position;
  const met = objective.kind === 'LEAGUE_FINISH'
...
export function settleSponsorObjectivesAtWeek30(
  ...
  if (sponsorship.portfolioSeason !== season) {
    throw new Error('sponsor objective season does not match the active portfolio');
  }

  const unresolved = sponsorship.activeContracts.filter(contract => (
  ...
  const progressByContractId = new Map(unresolved.map(contract => [
    contract.contractId,
    sponsorObjectiveProgressFromFixtures(contract.objective!, fixtures, userClubId, season),
  ]));
  const met = unresolved.filter(contract => progressByContractId.get(contract.contractId)!.met);
...
function allocateSponsorAmounts(
  ...
  if (!Number.isSafeInteger(sponsorIncomePercent)
  ...
  const ordered = [...amounts].sort(
    (left, right) => left.slot - right.slot || compareIds(left.contractId, right.contractId),
  );
  ...
  const working = ordered.map(amount => {
    ...
    if (!Number.isSafeInteger(amount.slot) || amount.slot < 0) {
      throw new Error(`${amount.contractId} sponsor slot must be a nonnegative safe integer`);
    }
    if (typeof amount.contractId !== 'string' || amount.contractId.trim().length === 0) {
      throw new Error('sponsor contract ID must be a non-empty string');
    }
    if (slots.has(amount.slot)) throw new Error(`duplicate sponsor slot ${amount.slot}`);
    ...
    nominalTotal = checkedAdd(nominalTotal, amount.nominalAmount, 'nominal sponsor portfolio');
  ...
  const actualTotal = Math.floor(portfolioNumerator / MONEY_PERCENT_DENOMINATOR);
  const flooredTotal = working.reduce(
    (total, amount) => checkedAdd(total, amount.actualAmount, 'floored sponsor portfolio'),
    0,
  );
  let dollarsToAllocate = actualTotal - flooredTotal;
  const largestRemainders = [...working].sort((left, right) => (
    right.remainder - left.remainder
    || left.slot - right.slot
    || compareIds(left.contractId, right.contractId)
  ));
  const receivesDollar = new Set<string>();
  for (let index = 0; index < largestRemainders.length && dollarsToAllocate > 0; index += 1) {
  ...
  return working.map(amount => ({
    ...
    actualAmount: amount.actualAmount + (receivesDollar.has(amount.contractId) ? 1 : 0),
  }));
}
...
function chooseSponsorBrand(
  ...
  const ordered = context.rules.brands
    ...
    .sort((left, right) => left.rank - right.rank || compareIds(left.brand.id, right.brand.id));
  const unused = ordered.find(candidate => !usedBrandIds.has(candidate.brand.id));
  return (unused ?? ordered[0]).brand;
}

function deterministicSponsorIndex(
  context: Pick<SponsorOfferGenerationContext, 'careerSeed' | 'season' | 'division'>,
  slot: number,
...
function leagueTableFromFixtures(
  ...
  const ensureClub = (clubId: string): MutableRow => {
    ...
    const created = { clubId, played: 0, won: 0, goalsFor: 0, goalsAgainst: 0, points: 0 };
  ...
  for (const fixture of fixtures) {
    ...
    if (seenFixtureIds.has(fixture.id)) throw new Error(`duplicate league fixture ${fixture.id}`);
    ...
    if (fixture.score === undefined) throw new Error(`played fixture ${fixture.id} is missing its score`);
  ...
  if (rows.size === 0) throw new Error(`league fixtures do not contain season ${season}`);
...
function validateOfferGenerationContext(context: SponsorOfferGenerationContext): void {
  validateOfferContextBase(context);
  if (context.rules.brands.length === 0) throw new Error('sponsor rules require at least one brand');
  ...
  const kinds = new Set(context.rules.objectives.map(objective => objective.kind));
  ...
  for (const contract of context.activeContracts) {
    validateMoney(contract.nominalMonthlyFee, `${contract.contractId} monthly sponsor fee`);
    if (!Number.isSafeInteger(contract.slot) || contract.slot < 0 || contract.slot > 2) {
      throw new Error(`${contract.contractId} sponsor slot must be 0, 1, or 2`);
    }
    if (contract.season === context.season) {
      if (slots.has(contract.slot)) throw new Error(`duplicate sponsor slot ${contract.slot}`);
...
function validateDivision(value: number, label: string): asserts value is DivisionLevel {
...
function percentOfMoney(value: number, percent: number, label: string): number {
  ...
  return Math.round(checkedMultiply(value, percent, `${label} numerator`) / 100);
}
