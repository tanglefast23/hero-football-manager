...
export function validateTypedName(value: string, subject: string): string {
  ...
  if (name.length < TYPED_NAME_MIN_LENGTH || name.length > TYPED_NAME_MAX_LENGTH) {
    throw new Error(
...
function validateOptionalTypedName(value: string | undefined, subject: string): string | undefined {
