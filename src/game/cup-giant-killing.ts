 ...
 */
export function cupUpsetCopyKey(
  divisionGap: number,
  part: 'title' | 'body',
): string {
...
export function cupGiantKillingCelebration(
  ...
  const copy =
    CUP_UPSET_COPY_BY_GAP[divisionGap as keyof typeof CUP_UPSET_COPY_BY_GAP];
...
export function completeCupGiantKillingCelebration(
  state: GameState,
): GameState {
  const pending = state.pendingCupGiantKillingCelebrations ?? [];
  if (pending.length === 0)
    throw new Error('there is no Cup giant-killing celebration to complete');
