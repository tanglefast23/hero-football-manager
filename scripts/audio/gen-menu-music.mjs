...
function sequenceBar(pattern, clock, voice, options = {}) {
  const parts = pattern.map(([name, beats]) =>
    name
      ? voice(name, beats, clock, options)
      : new Float32Array(clock.beats(beats)),
  );
...
function padBar(chordName, clock, { brightness = 1500, pulse = false } = {}) {
  ...
  const env = adsr(clock.barN, {
    attack: 0.06,
    decay: 0.1,
    sustain: pulse ? 0.42 : 0.56,
    release: 0.16,
  });
...
function arpeggioBar(chordName, clock, { reverse = false, soft = false } = {}) {
  ...
  return resize(
    concat(order.map((index) => arpNote(tones[index], clock, { soft }))),
    clock.barN,
  );
}
...
function bassNote(name, beats, clock, { soft = false } = {}) {
  ...
  const raw = mix([
    ...
    {
      buf: lowpass(oscSaw(soundN, frequency), soft ? 380 : 540),
      gain: soft ? 0.1 : 0.22,
    },
  ]);
  const shaped = applyEnv(
    raw,
    decayEnv(soundN, { attack: 0.004, decay: soft ? 0.38 : 0.3 }),
  );
  return resize(shaped, totalN);
}
...
function snare(rng, soft = false) {
  ...
  return applyEnv(
    mix([
      { buf: noise, gain: soft ? 0.62 : 0.82 },
      { buf: body, gain: soft ? 0.16 : 0.28 },
    ]),
...
function clap(rng, soft = false) {
  ...
  return mix(
    [0, 0.018, 0.036].map((seconds, index) => ({
      buf: applyEnv(
...
function titleDrums(
  rng,
  clock,
  { intensity = 1, stadium = false, fill = false } = {},
) {
  const layers = [];
  const place = (buf, beat, gain) =>
    layers.push({ buf, gain, offset: clock.beats(beat) });
  const kickBeats =
    intensity < 0.7
      ? [0, 2]
      : intensity > 1.05
  ...
  for (let eighth = 0; eighth < 8; eighth++) {
    place(
      hat(rng, fill && eighth === 7),
      eighth * 0.5,
      (eighth % 2 ? 0.21 : 0.14) * intensity,
    );
  }
  if (fill) {
...
function managementDrums(
  rng,
  clock,
  { intensity = 1, shaker = true, fill = false } = {},
) {
  const layers = [];
  const place = (buf, beat, gain) =>
    layers.push({ buf, gain, offset: clock.beats(beat) });
...
function titleBar(index, rng, clock) {
  ...
  const layers = [
    {
      buf: padBar(chordName, clock, { brightness: cycle === 3 ? 1300 : 1700 }),
      gain: index === 23 ? 0.15 : cycle === 3 ? 0.14 : 0.2,
    },
    {
      buf: arpeggioBar(chordName, clock, {
        reverse: cycle % 2 === 1,
        soft: cycle === 3,
      }),
      gain: cycle === 3 ? 0.18 : 0.3,
    },
  ];
  ...
  if (cycle === 0) {
    ...
    layers.push({
      buf: titleDrums(rng, clock, { intensity: 0.62, fill: index === 3 }),
      gain: 0.58,
    });
    if (barInCycle >= 2)
      layers.push({
        buf: sequenceBar(TITLE_HOOK[barInCycle], clock, titleLead, {
          softer: true,
        }),
        gain: 0.48,
      });
    ...
    layers.push({
      buf: sequenceBar(TITLE_ANSWER[barInCycle], clock, titleLead),
      gain: 0.64,
    });
    layers.push({
      buf: bassBar(chordName, clock, { walking: true }),
      gain: 0.58,
    });
    layers.push({
      buf: titleDrums(rng, clock, {
    ...
    layers.push({
      buf: fanfareStabs(chordName, clock, { quiet: true }),
      gain: 0.28,
    });
    ...
    layers.push({
      buf: sequenceBar(TITLE_LIFT[barInCycle], clock, titleLead),
      gain: 0.7,
    });
    layers.push({
      buf: sequenceBar(TITLE_HOOK[barInCycle], clock, titleLead, {
...
function managementBar(index, rng, clock) {
  ...
  const layers = [
    {
      buf: padBar(chordName, clock, {
        brightness: section === 3 ? 1050 : 1350,
      }),
      gain: section === 3 ? 0.22 : 0.27,
    },
    {
      buf: arpeggioBar(chordName, clock, {
        reverse: cycle % 2 === 1,
      ...
      gain: section === 3 ? 0.22 : 0.3,
    },
    {
      buf: bassBar(chordName, clock, {
        walking: section === 2 || section === 5,
        soft: true,
      }),
      gain: section === 3 ? 0.4 : 0.48,
    },
  ];
  ...
  if (section === 0) {
    ...
    if (cycle === 1)
      layers.push({
        buf: sequenceBar(MANAGEMENT_HOOK[barInCycle], clock, warmPluck, {
          softer: true,
        }),
        gain: 0.38,
      });
    ...
    layers.push({
      buf: sequenceBar(phrase[barInCycle], clock, warmPluck),
      gain: 0.54,
    });
    layers.push({
      buf: managementDrums(rng, clock, { intensity: 0.74, fill: index === 15 }),
      gain: 0.5,
    });
  } else if (section === 2) {
    ...
    layers.push({
      buf: sequenceBar(phrase[barInCycle], clock, warmPluck, { bell: true }),
      gain: 0.5,
    });
    layers.push({
      buf: managementDrums(rng, clock, { intensity: 0.88, fill: index === 23 }),
      gain: 0.52,
    });
  } else if (section === 3) {
    ...
    layers.push({
      buf: sequenceBar(phrase[barInCycle], clock, warmPluck, { softer: true }),
      gain: 0.12,
    });
    layers.push({
      buf: managementDrums(rng, clock, { intensity: 0.78, fill: index === 39 }),
      gain: 0.5,
    });
  } else if (section === 5) {
    ...
    layers.push({
      buf: managementDrums(rng, clock, { intensity: 0.9, fill: index === 47 }),
      gain: 0.54,
    });
  } else {
...
function eventBar(index, rng, clock) {
  ...
  const layers = [
    ...
    {
      buf: bassBar(chordName, clock, {
        walking: cycle >= 2 && cycle !== 4,
        soft: quiet,
      }),
      gain: quiet ? 0.46 : 0.58,
    },
  ];
  ...
  if (cycle === 0) {
    ...
    layers.push({
      buf: titleDrums(rng, clock, { intensity: 0.58, fill: index === 3 }),
      gain: 0.56,
    });
    if (barInCycle >= 2) {
      layers.push({
        buf: sequenceBar(EVENT_WARNING[barInCycle], clock, titleLead, {
          octaveShift: -1,
          softer: true,
        }),
        gain: 0.44,
    ...
    layers.push({
      buf: sequenceBar(EVENT_HOOK[barInCycle], clock, titleLead),
      gain: 0.66,
    });
    layers.push({
      buf: titleDrums(rng, clock, { intensity: 0.88, fill: index === 7 }),
      gain: 0.64,
    });
  } else if (cycle === 2) {
    layers.push({
    ...
    layers.push({
      buf: sequenceBar(EVENT_ANSWER[barInCycle], clock, titleLead),
      gain: 0.68,
    });
    layers.push({
      buf: titleDrums(rng, clock, {
...
function renderTrack(entry, built, outDir) {
  ...
  console.log(
    `${entry.name}.wav  ${ms.toFixed(0)}ms  ${built.bpm} BPM  peak ${entry.targetDb}dBFS`,
  );
  try {
    ...
    console.warn(
      `AAC encoders unavailable or failed, skipping ${entry.name}.m4a (${err.message})`,
    );
  }
...
export function generateMenuMusic(outDir = OUT_DIR) {
  ...
  const openingEntry = MUSIC_CATALOG.find(
    (entry) => entry.name === 'opening-theme',
  );
  const managementEntry = MUSIC_CATALOG.find(
    (entry) => entry.name === 'management-theme',
  );
  const eventEntry = MUSIC_CATALOG.find(
    (entry) => entry.name === 'event-theme',
  );
  if (!openingEntry || !managementEntry || !eventEntry)
    throw new Error('menu music entries missing from MUSIC_CATALOG');
  return [
    renderTrack(openingEntry, buildTitleTheme(), outDir),
    renderTrack(
      managementEntry,
      buildManagementTheme(managementEntry.loopFadeMs),
      outDir,
    ),
