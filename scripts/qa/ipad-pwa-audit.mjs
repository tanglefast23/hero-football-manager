...
const pass = (severity, id, what, detail) => record('PASS', severity, id, what, detail);
const fail = (severity, id, what, detail) => record('FAIL', severity, id, what, detail);
const manual = (severity, id, what, detail) => record('MANUAL', severity, id, what, detail);
const accepted = (severity, id, what, detail) => record('ACCEPTED', severity, id, what, detail);
...
async function main() {
  ...
  if (css === '') {
    console.error(`No built CSS under ${distDir}/_expo/static/css — is this a complete export?`);
    process.exit(2);
  }

  // ---- S2/S6: gestures the browser would otherwise steal --------------------
  cssCheck(css, 'P1', 'R2.1', 'touch-action:manipulation',
    'double-tap zoom cannot fire on rapid taps (training)');
  cssCheck(css, 'P1', 'R2.2', '-webkit-touch-callout:none',
    'the iPadOS text callout cannot race InfoTip\'s hold');
  cssCheck(css, 'P3', 'R6.3', 'overscroll-behavior:none',
    'a rubber-banded list cannot expose the dark body');

  // ---- S3: the installed web app shell -------------------------------------
  htmlCheck('P2', 'R3.3', 'viewport-fit=cover', 'safe-area insets report real numbers');
  htmlCheck('P2', 'R3.4', 'name="apple-mobile-web-app-capable"', 'iOS launches it as a web app');
  htmlCheck('P2', 'R3.4', 'name="mobile-web-app-capable"', 'the standard capability meta is present');
  htmlCheck('P2', 'R3.4', 'name="apple-mobile-web-app-status-bar-style"', 'the status bar is styled for a dark shell');
  htmlCheck('P2', 'R3.4', 'name="theme-color"', 'the browser chrome matches the game');
  htmlCheck('P2', 'R3.1', 'rel="apple-touch-icon"', 'the home screen shows the icon, not a screenshot');
  htmlCheck('P2', 'R3.2', 'rel="manifest"', 'the manifest is linked');
  htmlCheck('P2', 'R3.5', 'background: #241f2e', 'the shell paints dark before the stylesheet lands');
  ...
  for (const [id, file] of [
    ...
    if (await exists(path.join(distDir, file))) pass('P2', id, `${file} shipped`);
    else fail('P2', id, `${file} shipped`, 'linked but not in the export');
  }

  const manifestPath = path.join(distDir, 'manifest.json');
  if (!(await exists(manifestPath))) {
    fail('P2', 'R3.2', 'manifest.json shipped', 'the link has nothing to resolve to');
  } else {
    const manifest = JSON.parse(await readFile(manifestPath, 'utf8'));
    manifestCheck(manifest, 'display', 'standalone', 'R3.2', 'it launches without browser chrome');
    // Native locks portrait; the tablet plays in landscape, so web must not inherit that.
    manifestCheck(manifest, 'orientation', 'any', 'R6.4', 'both tablet orientations stay allowed');
    manifestCheck(manifest, 'theme_color', '#241f2e', 'R3.4', 'the manifest colour matches the shell');
    manifestCheck(manifest, 'background_color', '#241f2e', 'R3.5', 'the launch background is dark');
  ...
  if (await exists(path.join(distDir, 'canvaskit.wasm'))) {
    pass('P1', 'R9.1', 'canvaskit.wasm is in the export, so match day can draw');
  } else {
    fail('P1', 'R9.1', 'canvaskit.wasm is in the export', 'match day would not draw');
  }

  // ---- S8: delivery --------------------------------------------------------
  const hasServiceWorker = (await readdir(distDir)).some(name => /^(sw|service-worker)\.js$/.test(name));
  accepted('P3', 'R8.1',
  ...
  if (probeUrl === undefined) {
    manual('P3', 'R8.2', 'the deployment must not cache index.html',
  ...
  manual('P2', 'R3.6', 'the home screen launch is full-screen, and the in-Safari path still works');
  manual('P1', 'R2.5', 'no control is trapped in an iPadOS system gesture edge (all four edges)');
  manual('P2', 'R6.2', 'rotating across the 1100pt two-column breakpoint keeps state and reflows cleanly');
  manual('P2', 'R7.2', 'the software keyboard does not cover the name or search field');
  manual('P1', 'R9.2', 'a match returned to after a long background still draws (not a blank pitch)');
  manual('P1', 'R9.4', 'every match-day control over the canvas responds to touch');

  report();

  function htmlCheck(severity, id, needle, what) {
    if (html.includes(needle)) pass(severity, id, what);
    else fail(severity, id, what, `\`${needle}\` is missing from the exported index.html`);
  }

  function manifestCheck(manifest, key, expected, id, what) {
    if (manifest[key] === expected) pass('P2', id, what);
    else fail('P2', id, what, `manifest ${key} is ${JSON.stringify(manifest[key])}, expected ${JSON.stringify(expected)}`);
  }
}
...
async function probeCacheHeaders(origin) {
  ...
  try {
    const response = await fetch(`${base}/index.html`, { method: 'HEAD', redirect: 'follow' });
    const cacheControl = response.headers.get('cache-control') ?? '';
    const revalidates = /no-store|no-cache|max-age=0|must-revalidate/.test(cacheControl);
    if (revalidates) {
      pass('P3', 'R8.2', 'the deployment revalidates index.html, so a relaunch gets the new build',
        cacheControl);
    } else {
      fail('P3', 'R8.2', 'the deployment revalidates index.html',
        `cache-control: ${cacheControl || '(none)'} — a stale shell would pin an old build`);
    }
  } catch (error) {
    manual('P3', 'R8.2', 'the deployment revalidates index.html',
...
function report() {
  ...
  for (const { status, severity, id, what, detail } of results) {
    ...
    console.log(detail === undefined ? line : `${line}\n${' '.repeat(11)}↳ ${detail}`);
  }
  ...
  if (manuals.length > 0) {
    ...
    for (const { severity, id, what, detail } of manuals) {
      console.log(`  ${severity} ${id}  ${what}${detail === undefined ? '' : ` (${detail})`}`);
    }
  }
