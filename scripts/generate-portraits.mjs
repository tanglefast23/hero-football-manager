...
const launchClubs = JSON.parse(
  readFileSync(resolve(scriptDirectory, '../content/clubs.json'), 'utf8'),
).clubs;
...
console.log(
  `wrote ${Object.keys(sprites).length} portraits for ${looks.length} unique player looks`,
);
...
function launchLookAssignments(clubs) {
  ...
  for (const club of clubs)
    for (const player of club.players) {
      ...
      if (look === undefined)
        throw new Error(
          `not enough ${player.role === 'GK' ? 'goalkeeper' : 'field'} looks for launch rosters`,
        );
...
function validateSprites(candidateSprites) {
  ...
  const structuralLooks = careerLooks.map(
    (look) =>
      `${look.role}|${look.feature}|${look.face}|${look.build}|${look.eyes ?? ''}|${look.mouth ?? ''}`,
  );
  if (new Set(structuralLooks).size !== careerLooks.length) {
    throw new Error(
      'a palette swap alone cannot count as a unique player look',
    );
  }
  const uniqueResting = new Set();
  for (const look of looks) {
    const expressionRows = expressions.map(
      (expression) => candidateSprites[`${look.id}:${expression}`],
    );
    if (
      new Set(expressionRows.map((rows) => JSON.stringify(rows))).size !==
      expressions.length
    ) {
  ...
  if (uniqueResting.size !== looks.length)
    throw new Error('player resting portraits must all be unique');
  for (const [key, rows] of Object.entries(candidateSprites)) {
    if (rows.length !== PORTRAIT_CELL.h)
      throw new Error(`${key} has the wrong height`);
    for (const row of rows) {
      if (row.length !== PORTRAIT_CELL.w)
        throw new Error(`${key} has the wrong width`);
