...
const importFor = file => {
  ...
  const to = file.startsWith('App.tsx') ? './src/i18n' : `${'../'.repeat(depth)}i18n`;
  return `import { useCopy } from '${to}';`;
};

for (const file of process.argv.slice(2)) {
  const original = fs.readFileSync(file, 'utf8');
  if (!/\bt\('/.test(original)) { console.log(`  ${file}: no t() calls`); continue; }

  const sf = ts.createSourceFile(file, original, ts.ScriptTarget.Latest, true);
  const inserts = [];

  const scan = (node, holder) => {
    const isFn = ts.isFunctionDeclaration(node) || ts.isArrowFunction(node)
      || ts.isFunctionExpression(node);
    const next = isFn && node.body && ts.isBlock(node.body) ? node : holder;
    if (ts.isCallExpression(node) && node.expression.getText(sf) === 't' && next) {
      inserts.push(next);
    }
  ...
  const bodies = [...new Set(inserts)].filter(fn =>
    !inserts.some(other => other !== fn
      && other.getStart(sf) < fn.getStart(sf) && other.getEnd() > fn.getEnd()));

  let out = original;
  for (const fn of bodies.sort((a, b) => b.body.getStart(sf) - a.body.getStart(sf))) {
  ...
  if (!/from '[^']*i18n'/.test(out)) {
    ...
    const last = lines.reduce((best, l, i) =>
      (l.startsWith('import ') || l.startsWith('} from ')) ? i : best, 0);
    lines.splice(last + 1, 0, importFor(file));
    out = lines.join('\n');
  } else if (!/useCopy/.test(out.split('\n').filter(l => l.includes('i18n')).join('\n'))) {
    out = out.replace(/import \{ ([^}]*) \} from ('[^']*i18n')/,
