...
let whole=0, fragment=0, a11y=0;
for(const f of files){ const sf=ts.createSourceFile(f,fs.readFileSync(f,'utf8'),ts.ScriptTarget.Latest,true);
 const v=(node)=>{
  if(ts.isJsxText(node)&&prose(node.text)){
    const sibs = node.parent && node.parent.children ? node.parent.children : [];
    const hasExpr = sibs.some(c=>ts.isJsxExpression(c));
    if(hasExpr) fragment++; else whole++;
  }
  if(ts.isJsxAttribute(node)&&/^accessibility(Label|Hint)$/.test(node.name.getText(sf))&&node.initializer&&ts.isStringLiteral(node.initializer)&&prose(node.initializer.text)) a11y++;
  ts.forEachChild(node,v); }; ts.forEachChild(sf,v); }
