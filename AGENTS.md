...
3. **Never seize input.** Drive the browser with the pane / remote-debugging tools and the Simulator with headless screenshot and tap actions. Do not use desktop mouse or keyboard control on this project. Front a window with `osascript` only when a real user gesture is required, keep the activation as short as possible, and hand focus back.
