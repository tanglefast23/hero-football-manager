...
- Service: EAS Update, `@joseph-vu/hero-football-manager`, project ID `f6e36042-4f3b-4a96-a441-263b2570acfb`. `app.json` is the configuration source. Do not substitute another project's Expo account or endpoint.
- Native releases use the `production` channel and `fingerprint` runtime policy. Keep the embedded game, zero startup wait, and Expo's recovery protections. Updates download on launch and apply on the next restart; never force a reload during play.
- Test an update on `preview` using the production environment before publishing it to `production`. Publication is a separate release action; ordinary commits must not publish updates automatically.
- OTA is for compatible fixes and presentation changes allowed by Apple. Native dependencies, permissions, substantial feature changes, or incompatible save migrations need a new reviewed build. Fingerprints guard native compatibility, not save rollback safety. Replay-affecting changes still follow the engine-version rule.
- EAS sends installation identifiers and technical update/crash data. Keep the in-game privacy copy, privacy manifest, public policy, and App Store disclosures accurate. Do not restore the old claims of no network requests or no collected data.
- Native folders are generated and ignored. After update configuration or dependencies change, regenerate iOS and install pods before archiving. Inspect CLI-generated configuration: `eas update:configure` has duplicated privacy/localization arrays and inserted unwanted Android permissions here.
...
- Steam requires new macOS apps to be notarized, so the Mac depot is signed and notarized; Windows stays unsigned. `CSC_IDENTITY_AUTO_DISCOVERY=false npm run pack` (CI and local QA) stays unsigned. For the Mac depot, run `npm run pack` on a Mac whose login keychain holds `Developer ID Application: Joseph Anh Hai Vu (647S42DUW3)`, with `APPLE_API_KEY` (the `.p8` path), `APPLE_API_KEY_ID`, and `APPLE_API_ISSUER` exported from the `ASC_*` values in `~/.claude/secrets.env`: electron-builder signs with the hardened runtime and `desktop/entitlements.mac.plist`, notarizes, and staples. Without those three variables it signs but skips notarization. Verify with `spctl -a -vvv -t exec` (expects `source=Notarized Developer ID`) and `xcrun stapler validate`.
- Steam upload: app 5323960, depot 5323961 (Windows, `desktop/out/win-unpacked` unzipped from the `desktop.yml` artifact) and depot 5323962 (macOS, `desktop/out/mac-universal` from the signed local pack). From the repo root run `steamcmd +login tanglefast +run_app_build "$PWD/desktop/steam/app_build.vdf" +quit`; the first login needs Joe's password and Steam Guard, later ones reuse the cached login. Then set the build live on the `default` branch in Steamworks → SteamPipe → Builds and publish.
...
The latest agent-installed phone build (2026-09-08) is **Release with embedded JS**, including the iOS audio timing patch and six restored 3x effect groups. Changes need a new build and install to reach that app. Verify the installed build before assuming Metro reloads apply. The Debug workflow below applies only after a Debug build is installed:
...
- Matches auto-play, 3–4 real minutes watched; heroes build Heat and enter "the Zone" when their role-aware bar fills. **Hero power control defaults to MANUAL. AUTO fires in-context at 85%.** In MANUAL, a charged outfield hero shows faded, disabled `ARMED` until their authored useful context is live; only then does `FIRE!` accept a press at 100%. Stale or early presses are recorded no-ops and keep the charge. `ELASTIC_KEEPER` and `GIANT_GK` are the save-power exception: `FIRE!` appears when an enemy attack reaches the keeper's defending third or shoots from anywhere, and a press starts a ten-second full-strength save window. Its overhead bar drains in simulation time; expiry spends Heat and scrolls `WASTED POWER`. A goalkeeper carrying GUST follows the ordinary outfield contract. **MANUAL is watched-only; Quick Result is always automatic.** Engine m2.9 records these input rules. Teammate powers advance independently and may overlap; the match HUD presents one to four simultaneous power tiles as the Hero License cap grows. The desktop hero rail stays at 100% while a Zone or save window remains armed.
- Powers: 17 ship at launch (Magnet Touch cut at M4 by measurement; catalog in content/powers.json is canon). Hero License capacity starts at 2; promotion earns 3 at D3 and 4 at D1. Permits 3/4 can be bought early; more than four requires having reached D1. Purchased capacity is a floor, not added to earned capacity. GK Resolve prevents one-shot goals, wind-ups are interruptible, cut-ins skippable after first view. Timing-sensitivity principle: effects are visible possession/geometry spikes, never stat smears.
