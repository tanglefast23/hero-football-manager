...
- Matches auto-play, 3–4 real minutes watched; heroes build Heat, bank it until an authored opportunity, then enter "the Zone" (a full-intensity glow that holds until the context arrives — m1.27 removed the countdown, so an unused Zone never fades or refunds Heat). **Powers fire automatically by default, in-context at 85%.** A HERO POWER MANUAL/AUTO setting on the match-day screen is remembered per device (removed 2026-07-25, reinstated 2026-08-20). MANUAL puts a fire button on the pitch per charged hero: a press in context fires at 100%, a press outside it arms a 2-second window that fires at 90% or expires and costs one of that hero’s three Zones. **Goalkeepers always fire automatically** whatever the setting says (engine m2.7, `firePolicyForRole`) — their Zone opens minutes before the shot it is for, so a hand-fired keeper is a wasted keeper. **MANUAL is watched-only; Quick Result is always automatic**, because nobody is there to tap. Teammate powers advance independently and may overlap; the match HUD presents one to four simultaneous power tiles as the Hero License cap grows (the HUD caps at four; the LICENCE cap does not — `heroLicensePurchaseCost` sells permits 5, 6, 7 and up, so the fire dock is built for eleven). The player's live inputs are Formation, Playstyle, Swap, and Energy Use — all recorded, so a watched match stays deterministic (same seed + same inputs) without being predetermined. Quick Result runs the same engine and resolves identically to an unattended AUTO watch — that parity holds for AUTO only.
