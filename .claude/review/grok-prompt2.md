OUTPUT FORMAT — OBEY EXACTLY. Your reply is being length-capped, so be dense:
- No preamble, no restating the spec.
- At most SIX findings, hardest first.
- Each finding: one bold title line, then AT MOST 90 words. No sub-bullets.
- Skip anything you'd rate minor.
- Final line, alone: APPROVED or REVISE

Already known and accepted, do NOT spend a finding on these:
(a) PREFERENCES_SCHEMA_VERSION is already 9; the spec now bumps to 10. Fixed.
(b) fontFamily via CSS var IS verified working in react-native-css-interop 0.2.6 (font-family is in validProperties, routes through parseUnparsed -> var descriptor -> resolve-value.ts). Don't re-litigate whether vars() works.
(c) The measured-advance tables throw on unmeasured strings and per-locale short forms must be a hard Phase-2 dependency. Accepted.
