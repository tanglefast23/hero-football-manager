...
- For each candidate contact, compute the **entry fraction `t ∈ [0,1]`** along the tick using *relative motion* (ball start/end vs player start/end — players move too; a defender stepping across the path mid-tick is a real contact). Player proximity, ground contact (`z` crossing 0), goal-line crossing, and pitch-boundary crossing are all expressed as fractions in this same time domain.
- Sort candidates by `(t, playerIndex)` and process the **earliest event first**; an early contact consumes the tick's remaining travel (a ball caught at `t = 0.4` never reaches the goal line that tick).
...
- **Touchline wrap (canon, doc 03):** when the segment exits a side boundary, split it at the boundary and continue the remainder from the wrapped entry point — never test the unsplit segment (a wrapped ball would otherwise draw a false collision line across the whole pitch).
...
- **Lofted passes get their own solver** (an airborne ball obeys `AIR_DRAG` + bounces, not `ROLL_FRICTION`): choose hang time `m` from distance, `vz0 = GRAVITY × m / 2` (symmetric arc landing at `m` ticks), then simulate the airborne rules (+ first bounce) to solve horizontal `v0` for the landing point. Used when the ground solver's `v0` exceeds the kicker's leg-speed cap (from `pas`) — chipped long balls emerge from the physics, not a script.
...
`vz0` is **solved, not drawn** (external review, accepted — the blocking find of round 1: an independent `vz` band never connected the vertical error model to the vertical flight, and under the semi-implicit integrator `vz = 40` tops out at 60 cm, so shots could never clear the 300 cm bar). Compute horizontal flight ticks `n` from `v0` and distance, then pick the integer `vz0` whose simulated `z` at the crossing tick lands nearest `targetZ` — same simulate-the-integrator technique as the pass solver, gravity included exactly. `targetZ ≈ 0` → a driven roller (`vz0 = 0`); `targetZ > 300` → the ball honestly sails over. The ball may still legitimately skip off the turf en route (bounce handling is automatic).
...
- **One control gate for everyone:** no player may control a ball above `CONTROL_H` (150 cm) — receivers and loose-ball pickups included (a receiver cannot "trap" a ball five metres over their head; it flies on and comes down first). Cutting a pass mid-flight additionally requires `z ≤ INTERCEPT_H` (250 cm) at the crossing. This is *why* chipped through-balls beat a flat defensive line — and the price is built in: lofted balls hang longer and come down hot.
...
- Events (external review, accepted — `PASS.ok` cannot exist when outcomes are emergent): `PASS` becomes `{from, to}` at launch (drop `ok`), and **every flight ends in exactly one** `PASS_RESULT {from, to, outcome: 'complete' | 'intercepted' | 'loose', by?}` (`by` = actual controller when someone controls it). One ball means flights never overlap, so launch/terminal pairs correlate trivially and the completion rail reads straight off the stream. `MISS` gains `flavor: 'wide-left' | 'wide-right' | 'over'`; `SHOT` unchanged externally; `SAVE` gains `held: boolean` (catch vs parry).
