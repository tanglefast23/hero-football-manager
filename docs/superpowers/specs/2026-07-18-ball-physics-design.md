...
- Sort by **`(t, eventTypePriority, playerIndex)`** — non-player events have no index, so ties need a declared type order: keeper plane → player contact → goal line → ground → boundary (possession/save moments beat passive physics at the same instant). Process in order, with **continuation rules** (round-2 review):
  ...
  - **After any velocity- or path-changing event (bounce, reflection, wrap), recompute the candidate list for the residual fraction** — the initial sort is stale the moment the path bends (round-3 review). A deterministic per-tick contact cap (4) guards against pathological reflect/wrap loops; on hitting the cap the ball simply completes the residual travel uncontested.
...
- **Boundary rules, complete** (round-2/3 review — non-shot balls reach end lines too): touchlines (x = 0 / `PITCH_W`) **wrap** (canon, doc 03). End lines (y = 0 / `PITCH_H`) *outside* the goal mouth **reflect** — but **only for non-shot balls** (a ball in the `shot` kind that crosses outside the mouth or above the bar resolves as `MISS` + restart per §3, never a reflection; the two rules cannot both fire). Reflection bounces the ball back into play off a low invisible wall — no corners/goal-kick ceremony exists in canon, and wrap would teleport the ball the length of the pitch. Any ball — shot or not — fully crossing *inside* the mouth (under bar, between posts) is a **GOAL** for the team attacking that end, `by` = **`lastTouch`** (§5); a defender's spill trickling in is an own goal, and the parry vector always points into the pitch so a keeper can't wrap one into his own net.
...
`vz0` is **solved, not drawn** (round-1 blocking find: an independent `vz` band never connected the vertical error model to the vertical flight, and under the semi-implicit integrator `vz = 40` tops out at 60 cm, so shots could never clear the 300 cm bar). Round-2/3 refinement — the solve has a circular dependency (flight time depends on airborne-vs-rolling drag, which depends on `vz0`), so it's a plain **enumeration**: for each candidate integer `vz0` in a small band, simulate the complete 2.5D path (drag switching, bounces and all) **hypothetically through the goal line**, and keep the candidate whose `zAtLine` is nearest `targetZ` — the target and the miss check live on the same plane (round-3: selecting on `zAtPlane` while missing on `zAtLine` was a plane mismatch). The simulation records `vAtPlane` in passing — the speed at the *keeper* plane, where the save physically happens. `targetZ ≈ 0` → a driven roller (`vz0 = 0` wins); `targetZ > 300` → the ball honestly sails over. Bounded cost: ~a dozen candidates × ≤ 20 ticks of integer math per shot.

The save contest's `power` input becomes the ball's **arrival speed at the keeper plane** (`power = vAtPlane / K_POWER` — the plane is where the save happens, round-3 alignment). This is the physical version of today's hand-rolled `sho − dist/200`: launch speed encodes `sho`, drag encodes distance — the linear penalty falls out of the ballistics instead of being scripted. The `SHOT` event keeps `power` (now the derived value) for gauge/commentary compatibility.
...
- **Ball-kind contact matrix (round-3 review — who can touch what):** `pass` → opponents attempt cuts, the receiver (then any teammate once slow) attempts traps; `loose` → anyone attempts a trap; **`shot` → keeper plane and boundaries only** — outfielders cannot block or control a ball in the `shot` kind this milestone (deflected/blocked shots are a deliberate m0.7+ extension; shot candidate generation simply skips player contacts).
...
- `BallState`: `loose` / `pass` / `shot` all gain `z`, `vz` (and `pass` gains real `vel`); `pass` keeps intent fields `from/to`, gains immutable `targetPos` (§2) and the 22-bit `attemptMask` (§4), and **drops `willSucceed`/`interceptor`** (outcomes are emergent); `shot` keeps `by` (+ derived `power`). `held` unchanged. `MatchState` gains **`lastTouch: number`** (round-3 review — the any-ball goal rule needs stored ownership), updated on every kick/release, trap success *and* bobble, tackle spill, parry, and deliberate deflection; tested through a defender-spill own goal and a parry→bobble→goal attribution chain.
