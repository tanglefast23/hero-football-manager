...
2. **Match day** (if scheduled): pre-match screen (starting eleven, bench swaps, hero licenses — formation, Playstyle and Energy Use are live match controls, not pre-match ones) → Play → the post-match **Financial Report** (itemized, Kairosoft-style: tickets, sponsor fees, prize, minus wages — revealed row by row as slot reels).
...
- **D5 · District League → D4 · County League → D3 · Regional League → D2 · National League → D1 · Global League**: 10 teams each; top 2 promoted, bottom 2 relegated. Honest raw squad-strength bands are D5 **40–50**, D4 **51–63**, D3 **69–77**, D2 **81–91**, and D1 **107–120** — each promotion step sized to what one season of growth can hold (`src/game/pyramid.ts` is the source of truth). The ordinary-support anchors are 40/54/65/77/103, each club's DEF/MID/FWD specialist has an explicit 94/111/133/159/212 role focus, and goalkeeper REF is 80/94/113/135/180. A new club begins at 40 against the same authored opening sequence: 50 at home, 45 away, 46 at home, 43 away, then 42 at home. Each division up means better sponsors, bigger gates, stronger opponents — and more opposing heroes.
- **Hero Cup**: all-divisions knockout, entered every season. Its rounds settle in Weeks **10, 14, 18, 22, 26, and 29**, after the opening league run. League fixtures move around those dates, so every Cup tie has its own matchday. The draw freezes every entrant's division. In the Round of 32, a surviving D5 club draws D2 while D1 clubs draw D3 or D4, avoiding an automatic D1-v-D5 wall. From D2 onward, the player's club draws only a surviving D1-D3 club in the Round of 32 and Round of 16 when a safe candidate exists; later rounds keep the normal draw. A deterministic long-tailed performance model keeps same-division ties competitive while letting a lower club win roughly **5–10%** of one-division-gap ties and **1–2%** of two-plus-division gaps. The **first** player-controlled giant-killing of a career gets Bert's post-result walk-on: enthusiastic for one division, full **GIANT-KILLERS!** treatment for two or more. Later upsets pass without the interruption, guarded by `m4:cup-giant-killing-seen`. AI-only upsets never interrupt the player.
...
- **Chairman mode** (default): a harder game, not just a leaner budget — Season-1 subsidy cut to 40% (vs 50%), sponsor income at 80%, a half-size 10,000 emergency loan base, the board steps in after 2 negative weeks (vs 4), rivals grow 5%/season with an 800 attribute cap (vs 2.5%/700), the balance may sink to a −30,000 floor (vs −15,000) before board rescues clamp it, and harsher sponsor objectives (`chairmanDelta` in `content/sponsors.json`). For Kairosoft veterans who found Pocket League Story too easy (a common review complaint — we keep real money tension available).
