...
- **Hero Cup**: all-divisions knockout, entered every season. Its rounds settle in Weeks **10, 14, 18, 22, 26, and 29**, after the opening league run. League fixtures move around those dates, so every Cup tie has its own matchday. The draw freezes every entrant's division. A deterministic long-tailed performance model keeps same-division ties competitive while letting a lower club win roughly **5–10%** of one-division-gap ties and **1–2%** of two-plus-division gaps. Every player-controlled giant-killing gets Bert's post-result walk-on: enthusiastic for one division, full **GIANT-KILLERS!** treatment for two or more. AI-only upsets never interrupt the player.
