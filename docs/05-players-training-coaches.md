...
- **999 warning**: when training reaches 999, the manager gets a one-shot alert stating the exact player and stat and that it cannot go higher. The maxed option is greyed out and disabled; drill options show only the current stat value, never a cap. If the repeating plan still targets it next week, Advance Week opens a blocking prompt to choose another stat or player; no TP is wasted.
