...
- **Instant drills (the TP decision)**: tap **+** on any player, pick a stat in the drill popup, and choose **1–9 consecutive runs**; the picker stops at the number the current TP bank can afford. Every run resolves separately with its own gain, SUPER roll, injury roll, and skippable presentation before the next run begins. There is **no weekly plan, no slot limit, and no settlement step** — the only limits are the TP bank and the conditioning gamble. Training costs TP only, never money. The seven paths are Sprints→PAC, Finishing→SHO, Rondo→PAS, Duels→DEF, First Touch→TEC, Circuit→STA, and Keeper Drills→REF. A player is only offered the paths their role can use: outfielders never see Keeper Drills, and a keeper sees neither Finishing, Duels nor First Touch — the engine reads none of SHO, DEF or TEC for a goalkeeper, so those four cost TP for nothing. Each path has five tiers, a higher tier trading more TP per tap for a bigger gain (the per-tier gains and TP costs are `content/training.json`). Tier 1 starts in D5 and every tier above it goes on sale one division higher — tier 2 in D4 through tier 5 in D1. Reaching a division is permanent and only puts the upgrade on the shelf: the club still buys it path by path, at $3,000 / $8,000 / $18,000 / $40,000.
...
- **TP income**: match results award no TP. A fresh career starts with 24 TP and an empty grounds grid, plus enough extra cash to build the $8,000 Level 1 Training Pitch as its guided first project. Every weekly settlement pays a **20 TP** baseline whether or not a pitch exists, and the pitch itself pays nothing until it opens; from then on each settlement adds **+23 TP per completed facility level**. Employed coaches add more: a head coach pays **10/12/13/15/16** by level and an assistant **5/6/7/8/8**. **Every** TP the club is ever given — those three weekly rates, the launch grant, and the one-off rewards authored into events — is its pre-2026-08-07 figure cut to **80%, rounded up** (`TRAINING_POINT_SCALE_PERCENT`). Nothing that pays TP is exempt. A Level 1 pitch therefore takes the week from 20 TP to 43 — two basic 10-TP drills to four. Losing never slows training income, while hiring and upgrading staff or grounds creates a visible development budget.
...
- **Effects**: a head coach adds +10%/level to matching training gains and generates `ceil(0.8 × (10 + 2× level))` TP weekly — 10/12/13/15/16 by level; an assistant adds +5%/level and generates `ceil(0.8 × (5 + level))` — 5/6/7/8/8. The +10%/+5% training percentages are untouched by that weekly cut; they multiply a drill's stat gain, not TP income. Fractional coach growth is banked per player and attribute until it becomes a whole stat point, so a Level 1 bonus cannot disappear to weekly rounding. Attack→SHO, Defense→DEF, Fitness→PAC+STA, Technique→PAS+TEC, Goalkeeping→REF. Motivator instead slows morale loss and speeds Hero Gauge fill by +5%/head level or +2.5%/assistant level.
