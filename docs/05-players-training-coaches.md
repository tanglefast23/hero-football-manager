...
- **Effects**: a head coach adds +10%/level to matching training gains and generates `ceil(0.4 × (10 + 2× level))` TP weekly — 5/6/7/8/8 by level; an assistant adds +5%/level and generates `ceil(0.4 × (5 + level))` — 3/3/4/4/4. The +10%/+5% training percentages are untouched by that weekly cut; they multiply a drill's stat gain, not TP income. Fractional coach growth is banked per player and attribute until it becomes a whole stat point, so a Level 1 bonus cannot disappear to weekly rounding. Attack→SHO, Defense→DEF, Fitness→PAC+STA, Technique→PAS+TEC, Goalkeeping→REF. Motivator instead slows morale loss and speeds Hero Gauge fill by +5%/head level or +2.5%/assistant level.
