...
- **Match recovery**: a starter costs 4 / 6 / 8 / 10 / 12 condition in D5 / D4 / D3 / D2 / D1 before the weekly recovery tick. A substitute costs half: 2 / 3 / 4 / 5 / 6. An unused bench player pays nothing. A player appearing in two matches pays twice. Computer clubs use the same costs, receive the best Dorm recovery available in their division, and favor fresher same-role players when picking a lineup.
...
- **Effects**: a head coach adds +10%/level to matching training gains and generates **5/6/7/8/9 TP weekly** by level; Level 5 gets the final capstone TP after scaling. An assistant adds +5%/level and generates `ceil(0.4 × (5 + level))` — 3/3/4/4/4. The +10%/+5% training percentages are untouched by that weekly cut; they multiply a drill's stat gain, not TP income. Fractional coach growth is banked per player and attribute until it becomes a whole stat point, so a Level 1 bonus cannot disappear to weekly rounding. Attack→SHO, Defense→DEF, Fitness→PAC+STA, Technique→PAS+TEC, Goalkeeping→REF. Motivator instead slows morale loss and speeds Hero Gauge fill by +5%/head level or +2.5%/assistant level.
