...
- **Full gauge** → 8-second window: tap the hero's chip to fire at **100%**, aimed at the current situation. If the window lapses, the hero auto-fires at **75%** at the next useful context (hard deadline: +4 more seconds). Pre-match, each hero can be set to **Fire when ready** — the AI fires automatically at the next useful context at **85%** (or at 75% after the deadline if no context ever appears — no policy wastes a full charge). The attention ladder is deliberate: your tap > hero's instincts > wasted lapse, so watching earns an edge and simming stays respectable.
