...
**Quick Result** always runs every hero on the automatic firing policy, and the parity below therefore holds for an AUTO watched match only. Under MANUAL a hero holds their Zone until the manager fires it, so an unattended MANUAL watch would fire nothing while Quick Result fires everything — which is exactly why MANUAL is a watched-match setting and Quick Result never takes it. Quick Result runs the same engine with every hero on the same automatic firing policy an AUTO watched match uses — a fair playing-out of the match, not a prediction. It is exactly an unattended watched match: the user's saved opening formation and Auto Subs choice are preserved, no live Formation, Playstyle, Swap, or Energy Use inputs are invented, and only the opponent is auto-coached. Because powers no longer depend on attention, those identical inputs resolve identically. Watching earns its value from spectacle and from the real coaching calls the player chooses to make, never from a hidden simulation bonus.
...
- Each fielded hero builds **Heat** from involvement events (touches, tackles won, shots at launch, saves) plus a small role-aware trickle. At the role-aware threshold, the hero enters **the Zone** immediately and resets Heat. The Zone then waits for the power's authored useful context.
...
- **The Zone does not expire.** Under AUTO, a charged hero waits for useful context and fires at **85%**. Under MANUAL, an outfield hero shows disabled `ARMED` outside context and pressable `FIRE!` inside it. A legal press fires at **100%**; an early or stale press is a recorded no-op. `ELASTIC_KEEPER` and `GIANT_GK` instead accept `FIRE!` when an enemy attack reaches the keeper's defending third or shoots from anywhere. Their press opens a ten-second full-strength save window. Expiry resets Heat to zero and emits `WASTED POWER`. Quick Result always uses AUTO.
- **Powers can stack.** Every hero's Heat, Zone, wind-up, and active effect advances independently while teammates' powers run. The match HUD tracks up to four live activations at once: the phone coaching dock shows one takeover card for the newest power plus a "+N MORE HEROES LIVE" count, while the desktop control rail lists up to four persistent hero tiles. Later heroes retain their own opportunities instead of freezing behind the first activation.
...
- Match HUD: a fixed bottom-left name + live energy card shows the current carrier and retains the last carrier while the ball travels. In MANUAL, each charged hero keeps normal sprite size, flashes slowly, and gets a solid power-coloured oval at their feet plus a `First (Power)` label. The corner control shows the first name instead of a star. The desktop hero rail stays at 100% until the Zone fires or a save window expires. The first coaching row exposes Formation, Playstyle, and Swap; a second full-width row directly selects Save Energy, Balanced, or All Out and shows Team Energy. Speed/pause remain by the scoreboard. Rival heroes always run automatically.
