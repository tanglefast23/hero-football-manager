...
- **The Zone window** lasts ~7 seconds, glow fading as it runs out. Every hero on both sides fires automatically at the next useful context at **85%**, or at **75%** in the window's final seconds if no context appears. There is no manual tap and no `M`/`A` switch (removed 2026-07-25). Powers that require a victim never fire targetless — those windows expire instead, making an opponent's glow a threat you can starve.
