...
1. **Pre-match** (one screen): formation pitch with drag-swap lineup and hero license slot picker. One big **Play** button starts the watched match. The three quick-cycle formations live in persistent Settings, reachable from both the title screen and management shell.
2. **Live match** (portrait vertical pitch): scoreboard bug top with speed and pause; Seasons 1–2 expose **1× / 2×** playback, then the first watched match from Season 3 onward pauses for Bert's two-line veteran-coach introduction and permanently adds **3×**. A compact name + energy card stays fixed at bottom-left and retains the last carrier during passes, shots, and loose balls. AUTO heroes fire in context. MANUAL heroes stay normal size, flash slowly, and use a solid power-coloured foot oval with a `First (Power)` label. Their corner control names the hero, shows disabled `ARMED` outside context, and shows pressable `FIRE!` only when the press cannot be wasted. Save powers enable `FIRE!` once an enemy attack reaches the defending third; their ten-second window drains above the keeper and scrolls `WASTED POWER` if unused. The first coaching row has **Formation**, **Playstyle**, and **Swap**. A second full-width **Energy Use** row selects **Save Energy / Balanced / All Out** and shows Team Energy. A home power activation temporarily replaces the phone coaching dock—or the desktop left control rail—with a large player + power title in that team's jersey colour. Rival activations use compact red threat banners. All on-pitch power effects follow the Track-B pixel-art rules in doc 11.

The first eligible Bert-led MANUAL match teaches this control once per career. The first real `ARMED` state pauses and dims everything except the actual disabled power button and a Continue card. The card says that `ARMED` means charged and to wait for `FIRE!`. After Continue, the same hero's first later `FIRE!` state pauses again. Only the real `FIRE!` button remains active, with a `TAP NOW` or `CLICK NOW` arrow. The lesson completes only when that button queues its accepted power input. Other pause reasons remain active, and an interrupted attempt retries in a later match.
