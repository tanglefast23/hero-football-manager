...
Colorblind-safe kit palette pairs (never red-vs-green matches); text scaling respected on management screens; reduce-motion keeps the team-colour power title but removes its movement and sheen; timing gives a 7-second Zone followed by a fixed 2-second armed window when the player commits early; left/right-hand HUD flip.
