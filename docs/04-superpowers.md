...
- **Powers fire automatically by default; MANUAL is opt-in** (removed 2026-07-25, reinstated 2026-08-20 at the owner's decision). AUTO fires at the next useful context at **85%** (`CONTEXT_AUTO_STRENGTH`). Under MANUAL the manager's heroes run `SAVE_FOR_TAP` in a WATCHED match. A charged outfield hero shows faded, disabled `ARMED` until the authored useful context is live. Only then does `FIRE!` accept a press at **100%** (`TAP_STRENGTH`). An early or stale press is a recorded no-op that keeps the Zone. The old two-second outfield window is gone. Quick Result is always automatic.
