...
- **Powers fire automatically by default; MANUAL is opt-in** (removed 2026-07-25, reinstated 2026-08-20 at the owner's decision). AUTO fires at the next useful context at **85%** (`CONTEXT_AUTO_STRENGTH`). Under MANUAL the manager's heroes run `SAVE_FOR_TAP` in a WATCHED match and a fire button appears for each armed hero: a press inside the authored context fires at **100%** (`TAP_STRENGTH`), a press outside it commits a 2-second armed window that fires at 90% or expires and costs one of the hero's three Zones. Quick Result is always automatic.
- **Goalkeepers are hand-fireable since m2.8**, and their rules key on the POWER, never the role. `ELASTIC_KEEPER` and `GIANT_GK` take the shot-save contract: the button greys to `HOLD` while the ball is in the opponent's half, a press opens a **ten-second** window instead of two, there is no `FIRE!` state (a shot in flight is far too brief to react to), and a landed press is worth **100%** — anything less makes a manual keeper strictly worse than an automatic one, and nobody would use the control. **A keeper carrying GUST is not a save keeper** (`ROLE_POOL.GK` includes GUST) and takes the ordinary outfield contract. A MANUAL keeper does **not** save automatically: press, or the power goes unused.
...
- **A lost charge refunds five sixths of that ROLE's threshold** (`heatRefund`) — 50 for an outfield hero, 4 for a keeper. The old flat 50 was ten times a keeper's own line, so a keeper who lost a charge re-armed on the very next tick and could burn all three Zones in half a minute.
...
- **Powers fire automatically by default; MANUAL is opt-in** (removed 2026-07-25, reinstated 2026-08-20 at the owner's decision). With HERO POWER set to MANUAL, the manager's outfield heroes run `SAVE_FOR_TAP` in a WATCHED match and a fire button appears on the pitch for each charged hero. A press inside the authored context fires at **100%** (`TAP_STRENGTH`); a press outside it commits a 2-second armed window that fires at 90% or expires and costs one of the hero's three Zones. **Goalkeepers are exempt and always fire automatically** (engine m2.7, `firePolicyForRole`): a keeper's Zone opens at a Heat threshold of 5 and its only useful moment is an enemy shot already in flight, so automatic play already does what a tap was reaching for, and a hand-fired keeper is a wasted keeper. Quick Result is always automatic. Every hero on both sides runs the fire-when-ready policy: the power fires at the next useful context at **85%** (`CONTEXT_AUTO_STRENGTH`), the only grade reachable in the shipped game. A watched match and Quick Result therefore resolve identically, and the player's live decisions are Formation, Playstyle, Swap, and Energy Use rather than reflex. **Exception: powers that require a victim (Super Strength, Shadow Mark's ambush, Web Trap's springing) never fire targetless** — they hold their charge instead, for as long as it takes. A rival's glow is therefore a threat you can play around: keep the ball away from them and the charge never converts. Because the Zone no longer expires, that denial has to hold for the rest of the match. That's counterplay, not a bug.
