...
- **Watched-match AUTO switch** (match-wide, OFF by default): when ON, the first legally available home Zone is activated immediately as a normal recorded **100% tap**. It does not bypass knockouts or the one-power-per-team gate; a teammate's frozen Zone fires as soon as the current power clears. Turning it OFF restores manual chips immediately. This is deliberately distinct from the contextual 85% pre-match policy above: AUTO guarantees speed, while manual play can wait for a better moment.
