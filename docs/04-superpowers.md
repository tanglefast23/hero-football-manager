...
- **The window**: ~7 seconds, glow fading as it runs out.
- **Powers always fire automatically** (decision 2026-07-25, superseding the manual tap). Every hero on both sides runs the fire-when-ready policy: the power fires at the next useful context at **85%**, or at **75%** in the window's final seconds if no context appears. A watched match and Quick Result therefore resolve identically, and the player's live decisions are Formation, Playstyle, Swap, and Energy Use rather than reflex. **Exception: powers that require a victim (Super Strength, Shadow Mark's ambush, Web Trap's springing) never fire targetless** — their windows simply expire. A rival's glow is therefore a threat you can play around: keep the ball away from him and you starve the zone. That's counterplay, not a bug.
...
3. **Cutscene trigger deck**: every awakening uses the same collapse → discovery → ascension structure. Discovery causes come from a 17-entry comedy bank; every entry is used once before free random repeats begin. The 10.3-second pre-rise music asset begins with the limp, fades over its final second if the player waits on the story prompt, and stops the instant the ascension/rise beat starts.
