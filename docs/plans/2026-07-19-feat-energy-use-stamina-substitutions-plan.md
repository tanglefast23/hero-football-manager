...
This is a replay-affecting match-engine change. It was implemented against
`m1.3` as `m1.4`; final integration after the separate 2.5D-flight `m1.4`
change ships the combined behavior as `ENGINE_VERSION` **m1.5**. Regenerate the
Node/Hermes goldens only after the new behavior and balance rails pass, and
never treat a snapshot update as a substitute for the version decision.
