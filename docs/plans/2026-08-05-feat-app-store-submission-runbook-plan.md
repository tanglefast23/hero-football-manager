...
| Build number              | Durable `expo.ios.buildNumber` is `2`; App Store Connect has one valid uploaded build numbered `1`, but no build is selected for version 1.0 as of August 25, 2026                                                                                                                                                                                                                                      |                              `VERIFIED_REPO` and `VERIFIED_ACCOUNT` | Verify build `2` survives native generation and matches the signed archive and processed build before selection                                                                                                                                               |
...
| Content rights            | The rights ledger covers shipped assets. Joe approved retaining the 15 named special-hero homages on 2026-08-25 and accepts the residual App Review/IP risk; this records a product decision, not legal clearance                                                                                                                                                                                        |                                                   `OWNER_CONFIRMED` | Re-run the final archive inventory, retain commercial-use evidence and notices, then Joe truthfully reconfirms the declaration                                                                                                                               |
| Screenshots               | App Store Connect contains ten iPhone and ten iPad screenshots. Joe approved retaining both sets on 2026-08-25 because the player-visible game remains materially the same                                                                                                                                                                                                                            |                              `OWNER_CONFIRMED` and `VERIFIED_ACCOUNT` | Confirm processed build `2` still matches them materially; recapture only if that check finds real drift                                                                                                                                                     |
| Release archive           | The full archive → export → validate path was proved on 2026-08-17 from unfrozen `main`: Apple returned **VERIFY SUCCEEDED with no errors** for a distribution-signed IPA. That dry-run archive was deleted. App Store Connect contains older build `1`; final build `2` is not uploaded                                                                                                                | `VERIFIED_BUILD` for the path, `BLOCKED` for the submission archive | Fresh signed `.xcarchive` from the approved final release commit, revalidated, uploaded, and tested on physical devices                                                                                                                                      |
...
- **Still open:** upload/process/TestFlight QA for build `2` and build selection.
  The existing ten iPhone and ten iPad screenshots are uploaded and owner-approved
  for retention as of 2026-08-25, subject to a final material-match check. Joe
  also approved retaining the 15 named special-hero homages and accepted the
  residual App Review/IP risk; this is a product decision, not legal clearance.
...
- current iPad support is retained without approved iPad screenshots or portrait/landscape/multitasking QA;
...
6. “Should version 1.0 support iPad? Keeping it requires approved 13-inch iPad screenshots plus portrait, both landscapes, resizing, and multitasking QA. Joe approved retaining the uploaded iPad set on 2026-08-25; physical-device QA remains open.”
...
- `security find-identity` shows **no** distribution certificate. Xcode fetches
  the account's existing _Distribution Managed_ certificate at export time
...
14. Upload through Xcode Organizer or Transporter.
15. Wait for processing and inspect Apple email/status. Do not immediately upload a duplicate if processing is slow.
16. Assign the processed build to internal TestFlight without selecting it for version 1.0. Repeat the critical clean-install, offline, reviewer-path, and device-family smoke tests on physical iPhone and iPad.
17. Present the archive identity, processed-build identity, TestFlight evidence, and unresolved warnings to Joe. Obtain the binary gate approval.
18. Select the processed build for version 1.0 only after it passes those gates and the retained screenshots still match materially.
...
- Supply **1–10** screenshots per required device set and localization. Hero Football Manager 1.0 uses the approved 10-screen set below.
...
#### Final approved 10-screenshot storyboard

Use this exact order and title-card text for both required device sets:

| #   | Title card text           | What the screenshot must show                                                                                                |
| --- | ------------------------- | ---------------------------------------------------------------------------------------------------------------------------- |
| 1   | **HEROES CHANGE MATCHES** | A hero power cut-in or comic FX moment, such as the giant goalkeeper power or the most visually impressive power activation. |
| 2   | **CONTRACT RENEWALS**     | The season-end renewal queue with at least one hero wage cliff: old wage crossed out and the new hero rate visible.          |
| 3   | **COACH LIVE**            | A live match with the title and arrows beside the Formation and Playstyle controls.                                          |
| 4   | **TRAIN WHAT MATTERS**    | A player's detailed stat screen with all six visible stats clearly shown.                                                    |
| 5   | **FACILITIES PAY OFF**    | The facility grid with multiple buildings placed and adjacency bonuses visible.                                              |
| 6   | **STORIES WITH CONSEQUENCES** | A story event card that requires the player to choose a team member.                                                     |
| 7   | **PLAYER REQUESTS**       | A player-request decision with the request and its loyalty or morale effect visible.                                         |
| 8   | **SPONSORS WANT MORE**    | A choice of two or three sponsor offers with each payment and objective visible.                                             |
| 9   | **5 DIVISIONS + CUP**     | A filled Hero Cup bracket with named clubs and matchups.                                                                     |
| 10  | **FINANCIAL REPORT**      | The post-match Financial Report with its slot-reel income and expense reveal.                                                |

The title cards are owner-approved. The captured screens still require final approval before upload. Never use “Kairosoft-style,” another game/developer name, real clubs, or trademark stuffing in public copy.
...
- [ ] Asset rights ledger current for the shipped assets; named special-hero retention approved on 2026-08-25, with only the final archive inventory still open
- [ ] iPhone/iPad/Mac/Vision support decisions reflected in binary and availability
- [ ] Unique `expo.ios.buildNumber` `2` persisted in source; verify it as the final archive and processed build `CFBundleVersion`
...
- [x] Existing ten iPhone and ten iPad screenshots approved for retention by Joe on 2026-08-25; recheck material accuracy against processed build `2`
...
- [ ] Review contact/notes verified (contact complete, sign-in not required); build `2` upload, TestFlight QA, and selection remain pending
