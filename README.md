...
- **Developer Mode (every build, including static web exports and TestFlight):** Settings → Developer mode → ON, which reveals the save/load rail in the management header: weekly slots 1–5 written on each Advance Week, plus manual slots A–E. It is gated on `DEVELOPER_MODE_AVAILABLE` in `src/ui/release-surface.ts`, not on `__DEV__`, because the builds where jumping to a mid-career state matters most are exactly the ones where `__DEV__` is false. **Flip that constant to `false` before an App Store archive** — `npm run release:check` fails while it is `true`, so a release cannot ship the rail by omission.
